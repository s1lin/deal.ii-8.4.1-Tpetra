...
#error unexpected value of __TBB_BIG_ENDIAN
            ...
            for (atomic_backoff b;; b.pause()) {
                const uint32_t surroundings =
                        *aligned_ptr & ~mask; // reload the aligned_ptr value which might change during the pause
        ...
        template<>
        inline uint16_t
        __TBB_CompareAndSwapGeneric<2, uint16_t>(volatile void *ptr, uint16_t value, uint16_t comparand) {
#if __TBB_USE_GENERIC_PART_WORD_CAS
        ...
        template<>
        inline uint32_t
        __TBB_CompareAndSwapGeneric<4, uint32_t>(volatile void *ptr, uint32_t value, uint32_t comparand) {
            // Cast shuts up /Wp64 warning
        ...
        template<>
        inline uint64_t
        __TBB_CompareAndSwapGeneric<8, uint64_t>(volatile void *ptr, uint64_t value, uint64_t comparand) {

            ...
            public:
                ...
                bool check_rehashing_collision(const hashcode_t h, hashcode_t m_old, hashcode_t m) const {
                    ...
                    if ((h & m_old) != (h & m)) { // mask changed for this hashcode, rare event
                        ...
                        for (++m_old; !(h &
                                        m_old); m_old <<= 1) // at maximum few rounds depending on the first block size
                            ;
    ...
    @ingroup containers */
        ...
        public:
            ...
            size_type count(const Key &key) const {
                return const_cast<concurrent_hash_map *>(this)->lookup(/*insert*/false, key, NULL, NULL, /*write=*/
                                                                                 false);
            }
            ...
            bool find(const_accessor &result, const Key &key) const {
                result.release();
                return const_cast<concurrent_hash_map *>(this)->lookup(/*insert*/false, key, NULL, &result, /*write=*/
                                                                                 false);
            }
...
#pragma warning( disable: 4127 )
        ...
        template<typename I>
        std::pair<I, I>
        concurrent_hash_map<Key, T, HashCompare, A>::internal_equal_range(const Key &key, I end_) const {
            hashcode_t h = my_hash_compare.hash(key);
        ...
        void concurrent_hash_map<Key, T, HashCompare, A>::rehash(size_type sz) {
            ...
            int current_size = int(my_size), buckets =
                    int(mask) + 1, empty_buckets = 0, overpopulated_buckets = 0; // usage statistics
...
#if TBB_USE_PERFORMANCE_WARNINGS || __TBB_STATISTICS
            int current_size = int(my_size), buckets =
                    int(m) + 1, empty_buckets = 0, overpopulated_buckets = 0; // usage statistics
            static bool reported = false;
#endif
            bucket *bp = 0;
            // check consistency

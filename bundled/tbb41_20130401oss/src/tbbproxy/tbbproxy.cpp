...
// Error reporting utility. Behavior depends on mode.
                static tbb::runtime_loader::error_code
                error(tbb::runtime_loader::error_mode mode, tbb::runtime_loader::error_code err, char const *format,
                      ...) {
                    va_list args;
                    va_start(args, format);
...
// One attempt to load library (dll_name can be a full path or just a file name).
                static tbb::runtime_loader::error_code _load(char const *dll_name, int min_ver, int max_ver) {
                    ...
                    */
                    ...
                    if (_handle == NULL) {
                        char *msg = dlerror();
                        code = error(mode, tbb::runtime_loader::ec_no_lib, "Loading \"%s\" failed; system error: %s",
                                     dll_name, msg);
                        goto error;
                    } // if
                    ...
                    */
                    ...
                    if (get_ver_func == NULL) {
                        code = error(mode, tbb::runtime_loader::ec_bad_lib,
                                     "Symbol \"%s\" not found; library rejected.", get_ver_name);
                        goto error;
                    } // if
                    _version = get_ver_func();
                    if (!(min_ver <= _version && _version <= max_ver)) {
                        code = error(mode, tbb::runtime_loader::ec_bad_ver,
                                     "Version %d is out of requested range; library rejected.", _version);
                        goto error;
                    } // if
                        ...
                        if (addr != NULL) {
                            if (symbol.type == st_object) {
                                if (strncmp(symbol.name, "_ZTS", 4) == 0) {
                                    ...
                                    __TBB_ASSERT(strlen(name) + 1 == size_t(symbol.size),
                                                 "Unexpected size of typeinfo name");
                                    __TBB_ASSERT(strcmp(symbol.name + 4, name) == 0,
                                                 "Unexpected content of typeinfo name");
                ...
                static tbb::runtime_loader::error_code
                load(tbb::runtime_loader::error_mode mode, char const *path[], int min_ver, int max_ver) {
                    // Check arguments first.
                    if (min_ver <= 0) {
                        return error(mode, tbb::runtime_loader::ec_bad_arg,
                                     "tbb::runtime_loader::load(): Invalid value of min_ver argument: %d.", min_ver);
                    } // if
                    if (max_ver <= 0) {
                        return error(mode, tbb::runtime_loader::ec_bad_arg,
                                     "tbb::runtime_loader::load(): Invalid value of max_ver argument: %d.", max_ver);
                    } // if
                    if (min_ver > max_ver) {
                        return error(mode, tbb::runtime_loader::ec_bad_arg,
                                     "tbb::runtime_loader::load(): min_ver and max_ver specify empty range: [%d, %d].",
                                     min_ver, max_ver);
                    } // if
                    if (min_ver == max_ver) {
                    ...
                    if (handle != NULL) {
                        ...
                        if (min_ver <= version && version <= max_ver) {
                            ...
                            return error(mode, tbb::runtime_loader::ec_bad_ver, "Library version %d is already loaded.",
                                         version);
                        } // if
                    } // if
...
// Supress "defined but not used" compiler warnings.
        ...
        tbb::runtime_loader::error_code runtime_loader::load(char const *path[], int min_ver, int max_ver) {
            if (my_loaded) {
                my_status = tbb::interface6::internal::runtime_loader::error(my_mode, ec_bad_call,
                                                                             "tbb::runtime_loader::load(): Library already loaded by this runtime_loader object.");
            } else {
...
int __tbb_internal_runtime_loader_stub() {
    ...
    return tbb::interface6::internal::runtime_loader::error(tbb::interface6::internal::runtime_loader::stub_mode,
                                                            tbb::runtime_loader::ec_no_lib, msg);
} // stub

...
//! Synchronizes dispatch of OpenMP work.
        ...
        public:
            ...
            void produce(omp_client &c, job_type &j, void *cookie_, omp_client::size_type index_
                         PRODUCE_ARG(omp_connection_v2 &s)) {
                __TBB_ASSERT(&j, NULL);
...
/** ref_count holds number of clients that are using this,
    plus 1 if a host thread owns this instance. */
        class server_thread : public ref_count {
            ...
            template<typename Server, typename Client> friend
            class generic_connection;
...
/** One to one map from server threads to jobs, and associated reference counting. */
        ...
        public:
            ...
            thread_map(wait_counter &fc, ::rml::client &client) :
                    ...
                    my_client_ref_count(1), my_client(client), my_factory_counter(fc) { my_unrealized_threads = 0; }
...
/** The internal representation uses inheritance for the server part and a pointer for the client part. */
        ...
        public:
            ...
            tbb_connection_v2(wait_counter &fc, tbb_client &client) : generic_connection<tbb_server, tbb_client>(fc,
                                                                                                                 client) {
                my_slack = 0;
#if RML_USE_WCRM
                nesting = 0;
                ...
                active_tbb_connections = (uintptr_t)
            ...
            inline bool wakeup_next_thread(thread_map::iterator i) {
...
//------------------------------------------------------------------------
        ...
        public:
            ...
            omp_connection_v2(wait_counter &fc, omp_client &client) : generic_connection<omp_server, omp_client>(fc,
                                                                                                                 client) {
#if TBB_USE_ASSERT
                net_delta = 0;
#endif /* TBB_USE_ASSERT */
        ...
        void generic_connection<Server, Client>::request_close_connection(bool) {
            ...
            if (connection_traits<Server, Client>::is_tbb) {
                ...
                if (pred_conn) {
                    ...
                    active_tbb_connections = reinterpret_cast<uintptr_t>(generic_connection<tbb_server, tbb_client>::get_addr(
                            active_tbb_connections)); // release it
                } else
...
//! Wake up some available tbb threads
        void wakeup_some_tbb_threads() {
            ...
            generic_connection<tbb_server, tbb_client> *next_conn_wake_up = generic_connection<tbb_server, tbb_client>::get_addr(
                    active_tbb_connections);
            ...
            if (delta > 0) {
                ...
                if (the_balance > 0 &&
                    generic_connection<tbb_server, tbb_client>::get_addr(active_tbb_connections) != 0)
                    goto start_it_over;
            }
...
#define CREATE_SCHEDULER_POLICY(policy,min_thrs,max_thrs,stack_size) \
    try {                                                                 \
        ...
        extern "C" factory::status_type
        __RML_open_factory(factory &f, version_type &server_version, version_type client_version) {
...
namespace tbb {
    namespace internal {
        namespace rml {
            ...
            extern "C" tbb_factory::status_type
            __TBB_make_rml_server(tbb_factory &f, tbb_server *&server, tbb_client &client) {
...
namespace __kmp {
    namespace rml {
        ...
        extern "C" omp_factory::status_type
        __KMP_make_rml_server(omp_factory &f, omp_server *&server, omp_client &client) {

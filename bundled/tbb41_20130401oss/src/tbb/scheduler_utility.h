    ...
    segment is twice as large as the previous one. **/
        ...
        public:
            fast_reverse_vector(T *initial_segment, size_t segment_size)
                    : m_cur_segment(initial_segment), m_cur_segment_size(segment_size), m_pos(segment_size),
                      m_num_segments(0), m_size(0) {
            ...
            void push_back(const T &val) {
                if (!m_pos) {
                    ...
                    m_segments[m_num_segments++] = m_cur_segment = (T *) NFS_Allocate(m_cur_segment_size * sizeof(T), 1,
                                                                                      NULL);

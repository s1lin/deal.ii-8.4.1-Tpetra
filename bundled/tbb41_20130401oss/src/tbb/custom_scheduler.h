...
/** The customization can use SchedulerTraits to make decisions without needing a run-time check. */
        ...
        class custom_scheduler : private generic_scheduler {
            ...
            void wait_for_all(task &parent, task *child) {
                static_cast<custom_scheduler *>(governor::local_scheduler())->scheduler_type::local_wait_for_all(parent,
                                                                                                                 child);
            }
        ...
        public:
            ...
            /* override */ task *
            receive_or_steal_task(__TBB_atomic reference_count &completion_ref_count, bool return_if_no_work);
...
//------------------------------------------------------------------------
        ...
        template<typename SchedulerTraits>
        task *
        custom_scheduler<SchedulerTraits>::receive_or_steal_task(__TBB_atomic reference_count &completion_ref_count,
        ...
        void custom_scheduler<SchedulerTraits>::local_wait_for_all(task &parent, task *child) {
            ...
            __TBB_ASSERT(parent.prefix().context || (is_worker() && &parent == my_dummy_task),
                         "parent task does not have context");
                        ...
                        do {
                            ...
                            while (t) {
...
#endif
                                    __TBB_ASSERT(1L << t->state() &
                                                 (1L << task::allocated | 1L << task::ready | 1L << task::reexecute),
                                                 NULL);
                                assert_task_pool_valid();
#if __TBB_TASK_PRIORITY
...
#endif
                                {
                                    ...
                                    GATHER_STATISTIC(
                                            my_counters.avg_arena_concurrency += my_arena->num_workers_active());
                                    GATHER_STATISTIC(
                                            my_counters.avg_assigned_workers += my_arena->my_num_workers_allotted);
#if __TBB_TASK_PRIORITY
                                ...
                                switch (t->state()) {
                                    case task::executing: {
                                        ...
                                        __TBB_ASSERT(t->prefix().ref_count == 0,

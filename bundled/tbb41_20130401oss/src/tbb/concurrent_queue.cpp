...
/** For efficiency, this class has no constructor.
    The caller is expected to zero-initialize it. */
        struct micro_queue {
            ...
            page *make_copy(concurrent_queue_base &base, const page *src_page, size_t begin_in_page, size_t end_in_page,
                            ticket &g_index);
...
/** For efficiency, this class has no constructor.
    The caller is expected to zero-initialize it. */
        ...
        public:
            ...
            char pad1[NFS_MaxLineSize -
                      ((sizeof(atomic<ticket>) + sizeof(concurrent_monitor) + sizeof(atomic<size_t>)) &
                       (NFS_MaxLineSize - 1))];

            atomic<ticket> tail_counter;
            concurrent_monitor slots_avail;
            char pad2[
                    NFS_MaxLineSize - ((sizeof(atomic<ticket>) + sizeof(concurrent_monitor)) & (NFS_MaxLineSize - 1))];
...
//------------------------------------------------------------------------
        ...
        micro_queue &micro_queue::assign(const micro_queue &src, concurrent_queue_base &base) {
            ...
            if (srcp) {
                ...
                __TBB_TRY {
                    ...
                    size_t index = modulo_power_of_two(head_counter / concurrent_queue_rep::n_queue,
                                                       base.items_per_page);
                    size_t end_in_first_page = (index + n_items < base.items_per_page) ? (index + n_items)
                                                                                       : base.items_per_page;
                    ...
                    if (srcp != src.tail_page) {
                        ...
                        size_t last_index = modulo_power_of_two(tail_counter / concurrent_queue_rep::n_queue,
                                                                base.items_per_page);
        ...
        concurrent_queue_base::page *
        micro_queue::make_copy(concurrent_queue_base &base, const concurrent_queue_base::page *src_page,
...
//------------------------------------------------------------------------
        ...
        void concurrent_queue_base_v3::assign(const concurrent_queue_base &src) {
            ...
            __TBB_ASSERT(my_rep->head_counter == src.my_rep->head_counter &&
                         my_rep->tail_counter == src.my_rep->tail_counter,
        ...
        public:
            ...
            bool get_item(void *&item, size_t k) {
                if (k == my_queue.my_rep->tail_counter) {
                    ...
                    item = static_cast<unsigned char *>(static_cast<void *>(p)) + offset_of_last +
                           my_queue.item_size * i;

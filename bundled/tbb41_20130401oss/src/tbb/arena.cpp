...
namespace tbb {
    namespace internal {
        ...
        void arena::process(generic_scheduler &s) {
            ...
            unsigned index =
                    s.my_arena_index < my_num_slots ? s.my_arena_index : s.my_random.get() % (my_num_slots - 1) + 1,
                    end = index;
            ...
            // Find a vacant slot
            for (;;) {
                if (!my_slots[index].my_scheduler &&
                    __TBB_CompareAndSwapW(&my_slots[index].my_scheduler, (intptr_t) &s, 0) == 0)
                    break;
            ...
            for (;;) {
                ...
                __TBB_ASSERT(s.my_arena_slot->task_pool == EmptyTaskPool,
                             "Empty task pool is not marked appropriately");
            ...
            if (s.my_offloaded_tasks) {
                ...
                } while (__TBB_CompareAndSwapW(&my_orphaned_tasks, (ptrdiff_t) s.my_offloaded_tasks,
                                               (ptrdiff_t) orphans) != (ptrdiff_t) orphans);
        ...
        arena::arena(market &m, unsigned max_num_workers) {
            ...
            __TBB_ASSERT(sizeof(my_slots[0]) % NFS_GetLineSize() == 0,
                         "arena::slot size not multiple of cache line size");
            __TBB_ASSERT((uintptr_t) this % NFS_GetLineSize() == 0, "arena misaligned");
#if __TBB_TASK_PRIORITY
            __TBB_ASSERT(!my_reload_epoch && !my_orphaned_tasks && !my_skipped_fifo_priority,
            ...
            my_default_ctx =
                    new(NFS_Allocate(sizeof(task_group_context), 1, NULL)) task_group_context(
        ...
        arena &arena::allocate_arena(market &m, unsigned max_num_workers) {
            ...
            memset(storage, 0, n);
            return *new(storage + num_slots_to_reserve(max_num_workers) * sizeof(mail_outbox)) arena(m,
                                                                                                     max_num_workers);
        }
        ...
        void arena::free_arena() {
            ...
            for (intptr_t i = 0; i < num_priority_levels; ++i)
                __TBB_ASSERT(my_task_stream[i].empty() && my_task_stream[i].drain() == 0,
...
// TODO: This function seems deserving refactoring, e.g. get rid of 's'
        inline bool
        arena::may_have_tasks(generic_scheduler *s, arena_slot &slot, bool &tasks_present, bool &dequeuing_possible) {
                    ...
                    case SNAPSHOT_FULL: {
                        ...
                        if (my_pool_state.compare_and_swap(busy, SNAPSHOT_FULL) == SNAPSHOT_FULL) {
                            ...
                            if (work_absent) {
                                ...
                                if (s && __TBB_CompareAndSwapW(&my_slots[0].my_scheduler, (intptr_t) LockedMaster,
                                                               (intptr_t) s) ==
                                         (intptr_t) s) { //TODO: remove need to lock
                                ...
                                for (k = 1; work_absent && k < n; ++k)
                                    work_absent = !may_have_tasks(my_slots[k].my_scheduler, my_slots[k], tasks_present,
                                                                  dequeuing_possible);
                                ...
                                if (work_absent) {
                                    ...
                                    if (top_priority > my_bottom_priority) {
                                        if (my_market->lower_arena_priority(*this, top_priority - 1, top_priority)
                                            ...
                                            atomic_update(my_skipped_fifo_priority, top_priority,
                                                          std::less<intptr_t>());
                                        }

...
//------------------------------------------------------------------------
    ...
    void task_group_context::init() {
        ...
        __TBB_ASSERT (sizeof(task_group_context) == 2 * NFS_MaxLineSize,
                      "Context class has wrong size - check padding and members alignment");
        __TBB_ASSERT ((uintptr_t(this) & (sizeof(my_cancellation_requested) - 1)) == 0,
    ...
    void task_group_context::bind_to(generic_scheduler *local_sched) {
        ...
        if (my_parent->my_parent) {
            ...
            uintptr_t local_count_snapshot = __TBB_load_with_acquire(
                    my_parent->my_owner->my_context_state_propagation_epoch);
        ...
        ForEachArena(a)
                {
                    ...
                    if (s && __TBB_CompareAndSwapW(&slot.my_scheduler, (intptr_t) LockedMaster, (intptr_t) s) ==
                             (intptr_t) s) { //TODO: remove need in lock
    ...
    bool task_group_context::cancel_group_execution() {
        ...
        governor::local_scheduler()->my_arena->propagate_task_group_state(
                &task_group_context::my_cancellation_requested, *this, (uintptr_t) 1);
        return true;
    }
...
// IMPORTANT: It is assumed that this method is not used concurrently!
    ...
    void task_group_context::set_priority(priority_t prio) {
        __TBB_ASSERT(prio == priority_low || prio == priority_normal || prio == priority_high,

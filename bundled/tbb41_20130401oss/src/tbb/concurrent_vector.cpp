...
namespace tbb {
    ...
    namespace internal {
        ...
        public:
            ...
            inline static bool
            incompact_predicate(size_type size) { // assert size != 0, see source/test/test_vector_layout.cpp
                return size < page_size ||
                       ((size - 1) % page_size < page_size / 2 && size < page_size * 128); // for more details
            }
            ...
            acquire_segment(concurrent_vector_base_v3 &v, size_type index, size_type element_size, bool owner) {
                ...
                if (s.array <=
                    internal::vector_allocation_error_flag) // check for internal::vector_allocation_error_flag
                    throw_exception(
                            eid_bad_last_alloc); // throw custom exception, because it's hard to recover after internal::vector_allocation_error_flag correctly
                return s;
            }
            ...
            helper(segment_t *segments, size_type fb, size_type esize, size_type index, size_type s,
                   size_type f) throw()
        ...
        concurrent_vector_base_v3::helper::enable_segment(concurrent_vector_base_v3 &v,
            ...
            if (!k) {
                ...
                __TBB_TRY {
                    publish_segment(s[0], allocate_segment(v, segment_size(v.my_first_block)));
                        ...
                        ...) { // intercept exception here, assign internal::vector_allocation_error_flag value, re-throw exception
            ...
            if (k < v.my_first_block) {
                ...
                if (array0 <=
                    internal::vector_allocation_error_flag) { // check for internal::vector_allocation_error_flag of initial segment
                    publish_segment(s[k],
                                    internal::vector_allocation_error_flag); // and assign internal::vector_allocation_error_flag here
                    throw_exception(eid_bad_last_alloc); // throw custom exception
                }
                publish_segment(s[k],
        ...
        void concurrent_vector_base_v3::helper::cleanup() {
            ...
            while (sz <= finish) { // there is still work for at least one segment
                ...
                if (array > internal::vector_allocation_error_flag)
                    std::memset(static_cast<char *>(array) + element_size * start, 0,
                                ((sz < finish ? sz : finish) - start) * element_size);
                else
                    __TBB_ASSERT(array == internal::vector_allocation_error_flag, NULL);
            }
        }
        ...
        concurrent_vector_base_v3::~concurrent_vector_base_v3() {
            ...
            if (s != my_storage) {
                ...
                for (segment_index_t i = 0; i < pointers_per_long_table; i++)
                    __TBB_ASSERT(my_segment[i].array <= internal::vector_allocation_error_flag,
                                 "Segment should have been freed. Please recompile with new TBB before using exceptions.");
#endif
        ...
        void concurrent_vector_base_v3::internal_resize(size_type n, size_type element_size, size_type max_size,
                                                        const void *src,
        ...
        concurrent_vector_base_v3::segment_index_t
        concurrent_vector_base_v3::internal_clear(internal_array_op1 destroy) {
            __TBB_ASSERT(my_segment, NULL);
            size_type j = my_early_size;
            my_early_size = 0;
            helper for_each(my_segment, my_first_block, 0, 0, 0,
                            j); // element_size is safe to be zero if 'start' is zero
        ...
        void *
        concurrent_vector_base_v3::internal_compact(size_type element_size, void *table, internal_array_op1 destroy,
                                                    internal_array_op2 copy) {
        ...
        void concurrent_vector_base_v3::internal_swap(concurrent_vector_base_v3 &v) {
            ...
            size_type tmp = my_first_block;
            my_first_block = v.my_first_block;
            v.my_first_block = tmp;
            ...
            if (my_short && v_short) { // swap both tables
                ...
                my_segment = v.my_segment;
                v.my_segment = v.my_storage;
            } else if (v_short) { // v -> my
                memcpy(my_storage, v.my_storage, pointers_per_short_table * sizeof(segment_t));
                v.my_segment = my_segment;
                my_segment = my_storage;
            } else {
                segment_t *ptr = my_segment;
                my_segment = v.my_segment;
                v.my_segment = ptr;
            }
            my_early_size = v_sz;
            v.my_early_size = my_sz;
        }

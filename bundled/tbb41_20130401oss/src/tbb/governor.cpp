...
//! Table describing how to link the handlers.
        ...
        rml::tbb_server *governor::create_rml_server(rml::tbb_client &client) {
            ...
            if (!UsePrivateRML) {
                ...
                if (status != ::rml::factory::st_success) {
                    UsePrivateRML = true;
                    runtime_warning("rml::tbb_factory::make_server failed with status %x, falling back on private rml",
                                    status);
                }
            }
            if (!server) {
        ...
        void governor::setBlockingTerminate(const task_scheduler_init *tsi) {
            __TBB_ASSERT(!IsBlockingTermiantionInProgress,
...
#if TBB_USE_ASSERT
                if (BlockingTSI) {
                    __TBB_ASSERT(BlockingTSI == tsi_ptr,
                                 "For blocking termiantion last terminate_scheduler must be blocking.");
            ...
            if (s && s->my_auto_initialized) {
                if (!--(s->my_ref_count)) {
                    ...
                    if (!theTLS.get())
                        ...
                        __TBB_ASSERT(s == theTLS.get(), NULL);
                    s->cleanup_master();
        ...
        __cilk_tbb_retcode governor::stack_op_handler(__cilk_tbb_stack_op op, void *data) {
            ...
            switch (op) {
                ...
                case CILK_TBB_STACK_ORPHAN: {
                    __TBB_ASSERT(current == s && s->my_cilk_state == generic_scheduler::cs_running,
                                 "invalid orphaning");
#if TBB_USE_ASSERT
...
/** Left out-of-line for the sake of the backward binary compatibility **/
    ...
    void task_scheduler_init::initialize(int number_of_threads, stack_size_type thread_stack_size) {
        ...
        if (number_of_threads != deferred) {
            ...
            internal::generic_scheduler *s = governor::init_scheduler(number_of_threads,
                                                                      thread_stack_size, /*auto_init=*/false);
#if __TBB_TASK_GROUP_CONTEXT && TBB_USE_EXCEPTIONS
            if (s->master_outermost_level()) {
                ...
                vt = new_mode & propagation_mode_exact ? vt | task_group_context::exact_exception
                                                       : new_mode & propagation_mode_captured ? vt &
                                                                                                ~task_group_context::exact_exception
                                                                                              : vt;

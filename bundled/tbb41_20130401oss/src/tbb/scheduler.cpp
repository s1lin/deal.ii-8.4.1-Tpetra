...
#pragma warning(disable:4355)
        ...
        generic_scheduler::generic_scheduler(arena *a, size_t index)
                ...
                , my_ref_top_priority(NULL), my_offloaded_tasks(NULL), my_offloaded_task_list_tail_link(NULL),
                  my_ref_reload_epoch(NULL), my_local_reload_epoch(0), my_pool_reshuffling_pending(false)
    ...
    by the context object. **/
        void generic_scheduler::cleanup_local_context_list() {
            ...
            my_local_ctx_list_update.store<relaxed>(1);
            {
                ...
                if (my_nonlocal_ctx_list_update.load<relaxed>() ||
                ...
                while (node != &my_context_list_head) {
                    ...
                    __TBB_ASSERT(ctx.my_kind != task_group_context::binding_required,
                                 "Only a context bound to a root task can be detached");
                    node = node->my_next;
                    __TBB_ASSERT(is_alive(ctx.my_version_and_traits),
                                 "Walked into a destroyed context while detaching contexts from the local list");
                    ...
                    if ((task_group_context::kind_type) (uintptr_t) __TBB_FetchAndStoreW(&ctx.my_kind,
                                                                                         task_group_context::detached) ==
                        task_group_context::dying)
        ...
        task &generic_scheduler::allocate_task(size_t number_of_bytes,
            ...
            if (number_of_bytes <= quick_task_size) {
                ...
                if ((t = my_free_list)) {
                    ...
                    t = (task *) ((char *) NFS_Allocate(task_prefix_reservation_size + quick_task_size, 1, NULL) +
                ...
                t = (task *) ((char *) NFS_Allocate(task_prefix_reservation_size + number_of_bytes, 1, NULL) +
                              task_prefix_reservation_size);
#if __TBB_COUNT_TASK_NODES
                ++my_task_node_count;
#endif /* __TBB_COUNT_TASK_NODES */
        ...
        size_t generic_scheduler::prepare_task_pool(size_t num_tasks) {
            ...
            __TBB_ASSERT(!my_arena_slot->my_task_pool_size || my_arena_slot->my_task_pool_size >= min_task_pool_size,
                         NULL);
            ...
            for (;;) {
                ...
                if (my_arena_slot->task_pool != LockedTaskPool &&
                    __TBB_CompareAndSwapW(&my_arena_slot->task_pool, (intptr_t) LockedTaskPool,
                                          (intptr_t) my_arena_slot->task_pool_ptr) ==
                    (intptr_t) my_arena_slot->task_pool_ptr) {
            ...
            for (;;) {
                ...
                if (victim_task_pool != LockedTaskPool &&
                    __TBB_CompareAndSwapW(&victim_arena_slot->task_pool,
                                          (intptr_t) LockedTaskPool, (intptr_t) victim_task_pool) ==
...
/** Conceptually, this method should be a member of class scheduler.
    But doing so would force us to publish class scheduler in the headers. */
        ...
        task *generic_scheduler::reload_tasks(task *&offloaded_tasks, task **&offloaded_task_list_link,
                                              intptr_t top_priority) {
        ...
        task *generic_scheduler::reload_tasks() {
            ...
            if (my_offloaded_tasks &&
                (my_arena->my_bottom_priority >= top_priority || !my_arena->my_num_workers_requested)) {
        ...
        void generic_scheduler::cleanup_master() {
            ...
            if (in_arena()) {
                ...
                if (my_arena_slot->task_pool == EmptyTaskPool ||
                    ...
                    __TBB_ASSERT(!in_arena(), NULL);
                    __TBB_ASSERT (governor::is_set(this),
                                  "Other thread reused our TLS key during the task pool cleanup");
                }
            }
            __TBB_ASSERT(s.my_market, NULL);

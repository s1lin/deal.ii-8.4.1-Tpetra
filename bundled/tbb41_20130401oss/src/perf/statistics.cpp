...
StatisticsCollector::TestCase StatisticsCollector::SetTestCase(const char *name, const char *mode, int threads) {
    ...
    switch (SortMode) {
        case ByThreads:
            KeyName += Format("_%02d_%s", threads, mode);
            break;
        default:
        case ByAlg:
            KeyName += Format("_%s_%02d", mode, threads);
            break;
    }
...
void StatisticsCollector::Print(int dataOutput, const char *ModeName) {
        ...
        if ((OutputFile = fopen((Name + file_suffix + file_ext).c_str(), append ? "at" : "wt")) == NULL) {
            ...
            if (dataOutput & StatisticsCollector::PivotMode) {
                ...
                for (Statistics_t::iterator i = Statistics.begin(); i != Statistics.end(); i++) {
                    string RowHead;
                    if (SortMode == ByThreads)
                        RowHead = Format("\n%s%s%d%s%s%s", i->second->Name.c_str(), delim, i->second->Threads, delim,
                                         i->second->Mode.c_str(), delim);
                    else
                        RowHead = Format("\n%s%s%s%s%d%s", i->second->Name.c_str(), delim, i->second->Mode.c_str(),
                                         delim, i->second->Threads, delim);
                    for (size_t k = 0; k < RunInfo.size(); k++)
                ...
                for (Statistics_t::iterator i = Statistics.begin(); i != Statistics.end(); i++) {
                    if (SortMode == ByThreads)
                        fprintf(OutputFile, "\n%s%s%d%s%s", i->second->Name.c_str(), delim, i->second->Threads, delim,
                                i->second->Mode.c_str());
                    else
                        fprintf(OutputFile, "\n%s%s%s%s%d", i->second->Name.c_str(), delim, i->second->Mode.c_str(),
                                delim, i->second->Threads);
                    for (size_t k = 0; k < RunInfo.size(); k++)
                    ...
                    for (Analysis_t::iterator a = analisis.begin(); a != analisis.end(); ++t) {
                        ...
                        if (*t == a->first) {
                            fprintf(OutputFile, "%s", a->second.c_str());
                            ++a;
                        }
                    }
                    //data
    ...
    if (dataOutput & StatisticsCollector::HTMLFile) {
        if ((OutputFile = fopen((Name + file_suffix + ".html").c_str(), "w+t")) == NULL) {
            ...
            //-----------------------
            fprintf(OutputFile,
                    "<table id=\"h\" style=\"position:absolute;top:20\" border=1 cellspacing=0 cellpadding=2>\n");
            ...
            for (Statistics_t::iterator i = Statistics.begin(); i != Statistics.end(); i++) {
                ...
                for (Analysis_t::iterator j = i->second->Analysis.begin(); j != i->second->Analysis.end(); t++) {
                    fprintf(OutputFile, "<td bgcolor=#FFFF99>%s</td>",
                            (*t != j->first) ? " " : (i->second->Analysis[j->first]).c_str());
                    if (*t == j->first) j++;
                }
                //data
                Results_t &r = i->second->Results;
            ...
            //////////////////////////////////////////////////////
            fprintf(OutputFile,
                    "<table id=\"v\" style=\"visibility:hidden;position:absolute;top:20\" border=1 cellspacing=0 cellpadding=2>\n");
            ...
            for (AnalysisTitles_t::iterator t = AnalysisTitles.begin(); t != AnalysisTitles.end(); t++) {
                ...
                for (Statistics_t::iterator i = Statistics.begin(); i != Statistics.end(); i++)
                    fprintf(OutputFile, "<td>%s</td>",
                            i->second->Analysis.count(*t) ? i->second->Analysis[*t].c_str() : " ");
            }
    ...
    if (dataOutput & StatisticsCollector::ExcelXML) {
        if ((OutputFile = fopen((Name + file_suffix + ".xml").c_str(), "w+t")) == NULL) {
            ...
            fprintf(OutputFile, XMLBeginTable,
                    int(RoundTitles.size() + Formulas.size() + AnalysisTitles.size() + COUNT_PARAMETERS + 1/*title*/),
                    int(Statistics.size() + 1));
            ...
            for (Statistics_t::iterator i = Statistics.begin(); i != Statistics.end(); i++) {
                ...
                for (Analysis_t::iterator j = i->second->Analysis.begin(); j != i->second->Analysis.end(); at++) {
                    fprintf(OutputFile, XMLCellAnalysis,
                            (*at != j->first) ? "" : (i->second->Analysis[j->first]).c_str());
                    if (*at == j->first) j++;
                }
                //formulas
                size_t place = 0;
                ...
                for (Formulas_t::iterator f = Formulas.begin(); f != Formulas.end(); f++, place++)
                    fprintf(OutputFile, XMLCellFormula,
                            ExcelFormula(f->second, Formulas.size() - place, v.size(), true).c_str());
                //data
            ...
            fprintf(OutputFile, XMLWorkSheetProperties, 1, 1, 3, 3,
                    int(RoundTitles.size() + AnalysisTitles.size() + Formulas.size() + COUNT_PARAMETERS));
            fprintf(OutputFile, XMLAutoFilter, 1, 1, 1,
                    int(AnalysisTitles.size() + Formulas.size() + COUNT_PARAMETERS));

...
public:
    ...
    coarse_grained_raii_lru_cache(value_function_type f, std::size_t number_of_lru_history_items) : my_value_function(
            f), my_number_of_lru_history_items(number_of_lru_history_items) {}
    ...
    handle_object operator[](key_type k) {
        ...
        if (it == my_map_storage.end()) {
            it = my_map_storage.insert(it,
                                       std::make_pair(k, map_value_type(value_type(), 0, my_lru_list.end(), false)));
...
private:
    struct handle_move_t : tbb::internal::no_assign {
        ...
        handle_move_t(coarse_grained_raii_lru_cache &cache_ref, typename map_storage_type::reference value_ref)
                : my_cache_ref(cache_ref), my_value_ref(value_ref) {};
    };
    ...
    public:
        handle_object(coarse_grained_raii_lru_cache &cache_ref, typename map_storage_type::reference value_ref)
                : my_cache_pointer(&cache_ref), my_value_ref(value_ref) {}

    ...
    namespace internal {
        ...
        struct TestResults {
            ...
            TestResults(const TestResults &tr)
                    : my_test(tr.my_test), my_availableMethods(0), my_testName(tr.my_testName),
...
#define RES_STAT(res) (res != 0 ? "failed" : "ok")
        ...
        bool AffinitizeTBB(int p, int affMode) {
            ...
            tbb::task_list tl;
            for (int i = 0; i < p; ++i) {
                tbb::task &t = *new(tbb::task::allocate_root()) AffinitySetterTask(
                        affMode == amSparse ? i * NumCpus / p : i);
    ...
    if ( theSettings.my_opts & UseTaskScheduler && init.is_active() ) {     \
        ...
        public:
            TimingFunctor(Test *test, RunConfig *cfg, RunMemFnPtr fnRun,
                    ...
                    : my_test(test), my_cfg(cfg), my_fnRun(fnRun), my_numRuns(numRuns), my_numRepeats(nRepeats),
                      my_availableMethods(availableMethods) {}
        ...
        void RunTest(TestResults &tr, int mastersRange, int w, int p, int m, int a, int &numTests) {
            ...
            printf("Running tests: %04.1f%%\r", ++numTests * 100. / TotalConfigs);
            fflush(stdout);
        }
        ...
        void PrepareTests() {
            ...
            for (size_t i = 0; i < theSession.size(); ++i) {
                ...
                if (t.MaxNumMasters() > 1) {
                    ASSERT(theSettings.my_opts & UseTaskScheduler,
                           "Multiple masters mode is only valid for task scheduler tests");
        ...
        void PrintResults() {
            ...
            for (size_t i = 0; i < theSession.size(); ++i) {
                ...
                for (size_t j = 0; j < tr.my_results.size(); ++j) {
                    ...
                    if (tr.my_availableMethods & idRunSerial) {
                        duration_t serialTime =
                                (tr.my_serialBaselines[w].my_avgTime - baselineTime) / rc.my_maxConcurrency;
                        Report("%-*.1f ", OvhdFieldLen, 100 * (cleanTime - serialTime) / serialTime);
                    } else
                        Report("%*s%*s ", OvhdFieldLen / 2, "-", OvhdFieldLen - OvhdFieldLen / 2, "");
                    Report("%-*.2e %-*.2e ", RateFieldLen, ts.my_minTime - baselineTime, RateFieldLen,
                           ts.my_maxTime - baselineTime);
                    Report("\n");
                }
            }
            delete[] TlsTimings;

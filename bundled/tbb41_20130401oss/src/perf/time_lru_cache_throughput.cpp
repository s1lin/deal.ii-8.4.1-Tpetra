...
struct parameter_pack {
    ...
    parameter_pack(
            size_t a_time_window_sec, size_t a_time_check_granularity_ops, size_t a_cache_lru_history_size,
            size_t a_time_of_item_use_usec, size_t a_cache_miss_percent, int a_threads_number,
            size_t a_weight_of_initiation_call_usec, bool a_use_serial_initiation_function
    ) :
            time_window_sec(a_time_window_sec), time_check_granularity_ops(a_time_check_granularity_ops),
            cache_lru_history_size(a_cache_lru_history_size), time_of_item_use_usec(a_time_of_item_use_usec),
            cache_miss_percent(a_cache_miss_percent), threads_number(a_threads_number),
...
struct return_size_t {
    ...
    return_size_t(size_t a_weight_of_initiation_call_usec, bool a_use_serial_initiation_function)
            : m_weight_of_initiation_call_usec(a_weight_of_initiation_call_usec),
              use_serial_initiation_function(a_use_serial_initiation_function) {}
...
struct throughput {
    ...
    throughput(parameter_pack a_parameter_pack)
            : m_parameter_pack(a_parameter_pack), per_thread_sample_size(
            m_parameter_pack.cache_lru_history_size * (1 + m_parameter_pack.cache_miss_percent / 100)),
              m_access_sequence(m_parameter_pack.threads_number * per_thread_sample_size),
              m_cache(return_size_t(m_parameter_pack.weight_of_initiation_call_usec,
                                    m_parameter_pack.use_serial_initiation_function),
                      m_parameter_pack.cache_lru_history_size) {
        loops_count = 0;
    ...
    size_t operator()() {
        struct _ {
            static void retrieve_from_cache(self_type *_this, size_t thread_index) {
                parameter_pack &p = _this->m_parameter_pack;
                access_sequence_type::iterator const begin_it =
                        _this->m_access_sequence.begin() + thread_index * _this->per_thread_sample_size;
                ...
                do {
                    size_t part_of_the_sample_so_far =
                            (local_loops_count * p.time_check_granularity_ops) % _this->per_thread_sample_size;
                    ...
                    access_sequence_type::iterator const iteration_end_it = iteration_begin_it +
                                                                            (std::min)(p.time_check_granularity_ops,
                                                                                       _this->per_thread_sample_size -
                                                                                       part_of_the_sample_so_far);
...
int main(int argc, const char **args) {
    ...
    parameter_pack p(time_window_sec, time_check_granularity_ops, cache_lru_history_size, time_of_item_use_usec,
                     cache_miss_percent, threads_number, weight_of_initiation_call_usec,
                     use_serial_initiation_function);
    ...
    utility::parse_cli_arguments(argc, args, utility::cli_argument_pack()
            ...
            .arg(p.time_of_item_use_usec, "time-of-item-use",
                 "time between  consequent requests to the cache, in microseconds")
            .arg(p.cache_miss_percent, "cache-miss-percent", "cache miss percent ")
            .arg(p.weight_of_initiation_call_usec, "initiation-call-weight",
                 "time occupied by a single call to initiation function, in microseconds")
            .arg(p.use_serial_initiation_function, "use-serial-initiation-function",
                 "limit lock-based serial initiation function")

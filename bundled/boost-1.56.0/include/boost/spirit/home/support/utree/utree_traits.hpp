...
namespace boost {
    namespace spirit {
        ...
        typedef spirit::basic_string<iterator_range<T const*>, I>
        string;
    ...
    static void
            call(variant < BOOST_VARIANT_ENUM_PARAMS(T) >
    const& val,
    utree &attr
    ) {
...
struct compute_compatible_component_variant<utree, Sequence, mpl::false_, typename enable_if<
        fusion::traits::is_sequence < Sequence> >::type>
: mpl::true_
{
typedef iterator_range <utree::const_iterator> compatible_type;
...
static type pre(iterator_range < Iterator >
const& t)
{

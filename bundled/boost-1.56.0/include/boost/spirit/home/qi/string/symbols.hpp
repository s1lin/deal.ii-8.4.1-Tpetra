...
namespace boost {
    namespace spirit {
        namespace qi {
            ...
            struct symbols
                    ...
                    >::type, symbols<Char, T, Lookup, Filter>
            >
        ...
        symbols(symbols<Char, T, Lookup, Filter_>
        const& syms)
        :
    ...
    template<typename Filter_>
    symbols &
            operator=(symbols < Char, T, Lookup, Filter_ >
    const& rhs) {
    name_ = rhs.name_;
    *
    lookup = *rhs.lookup;
...
bool
parse(Iterator &first, Iterator const &last, Context & /*context*/, Skipper const &skipper, Attribute &attr_) const {
...
typedef typename mpl::if_<
        no_case, symbols < Char, T, Lookup, nc_filter>
, reference_>
::type
        result_type;

...
namespace boost {
    ...
    template<typename Parser, typename Attribute>
    struct pass_sequence_attribute<Parser, Attribute, typename enable_if_c<(Parser::is_pass_through_unary)>::type>
    ...
    struct partition_attribute {
        ...
        static r_part right(Attribute &s) {
            return r_part(
                    fusion::advance_c<l_size>(fusion::begin(s)), fusion::end(s));
        }
    };
    ...
    struct get_sequence_types {
        typedef
        mpl::vector<
                typename traits::attribute_of<L, C>::type, typename traits::attribute_of<R, C>::type
        >
                type;
    };
    ...
    typedef typename
    mpl::push_back<
            typename get_sequence_types<LL, LR, C>::type, typename traits::attribute_of<R, C>::type
    >::type
            type;
};
...
typedef typename
mpl::push_front<
        typename get_sequence_types<RL, RR, C>::type, typename traits::attribute_of<L, C>::type
>::type
        type;
};
...
typedef
mpl::joint_view<
        typename get_sequence_types<LL, LR, C>::type, typename get_sequence_types<RL, RR, C>::type
>
        type;
};
...
struct attribute_of_sequence {
    ...
    typedef typename
    mpl::copy_if<
            all_types, mpl::not_ < is_same < mpl::_1, unused_type>>
...
bool parse_sequence(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, traits::tuple_attribute) {
...
bool parse_sequence(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, traits::plain_attribute) {
    typedef typename Parser::left_type Left;
    typedef typename Parser::right_type Right;
...
bool parse_sequence(
        Left const &left, Right const &right, Iterator &first, Iterator const &last, Context const &context,
        RContext &rcontext, Attribute &attr, traits::container_attribute);

template<typename Parser, typename Iterator, typename Context, typename RContext, typename Attribute>
bool parse_sequence(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, traits::container_attribute) {
    Iterator save = first;
...
bool parse_sequence_assoc(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, mpl::false_ /*should_split*/) {
...
bool parse_sequence_assoc(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, mpl::true_ /*should_split*/) {
    Iterator save = first;
...
bool parse_sequence(
        Parser const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
        Attribute &attr, traits::associative_attribute) {
...
static bool call(
        parser_type const &parser, Iterator &first, Iterator const &last, Context const &context, RContext &rcontext,
    ...
    Attribute attr_;
    if (!parse_sequence(parser, first, last, context, rcontext, attr_, traits::container_attribute())) {
        return false;
    }

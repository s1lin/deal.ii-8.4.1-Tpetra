...
namespace boost {
    namespace spirit {
        namespace x3 {
            ...
            parse_rule(
                    rule<ID, Attribute> rule_, Iterator &first, Iterator const &last, Context const &context,
    ...
    namespace spirit {
        namespace x3 {
            namespace detail {
                ...
                struct has_on_error<ID, Iterator, Context,
                        typename disable_if_substitution_failure<
                                ...
                                std::declval<ID>().on_error(
                                        ...
                                        std::declval < expectation_failure < Iterator >> (), std::declval<Context>()
                                )
                                )>::type
                >
                        : mpl::true_ {
                ...
                struct has_on_success<ID, Iterator, Context, Attribute,
                        typename disable_if_substitution_failure<
                                ...
                                std::declval<ID>().on_success(
                                        std::declval<Iterator &>(), std::declval<Iterator>(),
                                        std::declval<Attribute &>(), std::declval<Context>()
                                )
                                )>::type
                >
                        : mpl::true_ {
                ...
                Context const &
                make_rule_context(RHS const &rhs, Context const &context, mpl::false_ /* is_default_parse_rule */) {
                    return context;
                }
                ...
                struct rule_parser {
                    ...
                    static bool call_on_success(
                            Iterator &first, Iterator const &last, Context const &context, ActualAttribute &attr,
                    ...
                    static bool call_on_success(
                        ...
                        bool pass = true;
                        ID().on_success(
                                first, last, attr, make_context<parse_pass_context_tag>(pass, context)
                        );
                        return pass;
                    }
                    ...
                    static bool parse_rhs_main(
                            RHS const &rhs, Iterator &first, Iterator const &last, Context const &context,
                            RContext &rcontext, ActualAttribute &attr, mpl::false_) {
                        // see if the user has a BOOST_SPIRIT_DEFINE for this rule
                        typedef
                        decltype(parse_rule(
                                rule<ID, Attribute>(), first, last, make_unique_context<ID>(rhs, context), attr))
                                parse_rule_result;
                        ...
                        Iterator i = first;
                        bool r = rhs.parse(
                                i, last, make_rule_context<ID>(rhs, context, is_default_parse_rule()), rcontext, attr
                        );
                    ...
                    static bool parse_rhs_main(
                            RHS const &rhs, Iterator &first, Iterator const &last, Context const &context,
                            RContext &rcontext, ActualAttribute &attr, mpl::true_ /* on_error is found */) {
                        for (;;) {
                            try {
                    ...
                    static bool parse_rhs_main(
                        ...
                        return parse_rhs_main(
                                rhs, first, last, context, rcontext, attr, has_on_error<ID, Iterator, Context>()
                        );
                    }
                    ...
                    static bool parse_rhs(
                            RHS const &rhs, Iterator &first, Iterator const &last, Context const &context,
                            RContext &rcontext, ActualAttribute &attr, mpl::true_) {
                    ...
                    static bool call_rule_definition(
                            RHS const &rhs, char const *rule_name, Iterator &first, Iterator const &last,
                            Context const &context, ActualAttribute &attr, ExplicitAttrPropagation) {
...
#endif
                            ok_parse = parse_rhs(rhs, first, last, context, attr_, attr_, mpl::bool_
                                    <(RHS::has_action

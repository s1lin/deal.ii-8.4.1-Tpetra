    ...
    namespace property_tree {
        ...
        struct basic_ptree<K, D, C>::subs {
            ...
            static by_name_index &assoc(self_type *s) {
                return ch(s).BOOST_NESTED_TEMPLATE
                get<by_name>();
            }
        ...
        typename boost::enable_if<detail::is_translator < Translator>, Type>
        ...
        typename boost::disable_if<detail::is_translator < Type>, Type>
        ...
        basic_ptree<K, D, C>::get(const path_type &path,
                                  Translator tr) const {
            return get_child(path).BOOST_NESTED_TEMPLATE
            get_value<Type>(tr);
        }
        ...
        optional <Type> basic_ptree<K, D, C>::get_optional(
            ...
            if (optional<const self_type &> child = get_child_optional(path))
                return child.get().BOOST_NESTED_TEMPLATE
            get_value_optional<Type>();
            else
            return optional<Type>();
        }

...
namespace boost {
    namespace concepts {
        BOOST_concept(CycleVisitor, (Visitor)(Path)
        (Graph)) {
 ...
 */
    struct min_max_cycle_visitor {
        ...
        inline void cycle(const Path &p, const Graph &g) {
            ...
            minimum = min
            BOOST_PREVENT_MACRO_SUBSTITUTION(minimum, len);
            maximum = max
            BOOST_PREVENT_MACRO_SUBSTITUTION(maximum, len);
        }
    ...
    find_min_max_cycle(std::size_t &min_, std::size_t &max_) { return min_max_cycle_visitor(min_, max_); }
    ...
    namespace detail {
        ...
        can_wrap_path(const Graph &g, const Path &p) {
            ...
            Vertex
                    ...
                    v = p.front();
            OutIterator i, end;
            for (boost::tie(i, end) = out_edges(u, g);
            i != end;
            ++i) {
                if ((target(*i, g) == v)) {
                    return true;
    ...
    tiernan_all_cycles(const Graph &g,
        ...
        for (boost::tie(i, end) = vertices(g);
        i != end;
        ++i) {
    ...
    template<typename Graph>
    inline std::size_t
    tiernan_girth(const Graph &g) { return tiernan_girth_and_circumference(g).first; }

    template<typename Graph>
    inline std::size_t
    tiernan_circumference(const Graph &g) { return tiernan_girth_and_circumference(g).second; }

   ...
   compressed_sparse_row_graph<bidirectionalS, VertexProperty, EdgeProperty, \
    ...
    namespace detail {
        ...
        struct default_construct_iterator
                : public boost::iterator_facade<default_construct_iterator<T>, T, boost::random_access_traversal_tag, const T &> {
 ...
 */
    ...
    inherited_vertex_properties (numverts), m_property(prop) {
        m_forward.assign_unsorted_multi_pass_edges(edge_begin, edge_end, numverts,
    ...
    inherited_vertex_properties (numverts), m_forward(), m_property(prop) {
        m_forward.assign_unsorted_multi_pass_edges(edge_begin, edge_end, ep_iter, numverts,
                                                   typed_identity_property_map<vertices_size_type>(), keep_all());
    }
    ...
    inherited_vertex_properties (numlocalverts), m_forward(), m_property(prop) {
        m_forward.assign_unsorted_multi_pass_edges(edge_begin, edge_end, ep_iter, numlocalverts, global_to_local,
                                                   source_pred);
    }
    ...
    m_property(prop)
            {
                    m_forward.assign_from_sorted_edges(edge_begin, edge_end, ep_iter, global_to_local, source_pred,
                                                       numverts, 0);
            inherited_vertex_properties::resize(numverts);
            }
    ...
    & sources,
    std::vector<vertex_descriptor> &targets,
            std::vector<typename forward_type::inherited_edge_properties::edge_bundled>
    & edge_props,
    vertices_size_type numverts,
    const GraphProperty &prop = GraphProperty()
    )
    ...
    inherited_vertex_properties (numverts), m_property(prop) {
        m_forward.assign_sources_and_targets_global(sources, targets, edge_props, numverts,
                                                    boost::typed_identity_property_map<vertices_size_type>());
    }
    ...
    add_edges_sorted_internal(
            ...
            BidirectionalIteratorOrig last_sorted,
            EPIterOrig ep_iter_sorted) {
        m_forward.add_edges_sorted_internal(first_sorted, last_sorted, ep_iter_sorted,
    ...
    add_edges_sorted_internal_global(
            ...
            const GlobalToLocal &global_to_local) {
        m_forward.add_edges_sorted_internal(first_sorted, last_sorted,
                                            detail::default_construct_iterator<edge_bundled>(), global_to_local);
    }
    ...
    const graph_bundled &operator[](graph_bundle_t) const { return get_property(*this); }
...
inherited_vertex_properties (numverts), m_property(prop) {
    m_forward.assign_unsorted_multi_pass_edges(edge_begin, edge_end, numverts, typed_identity_property_map<Vertex>(),
                                               keep_all());
...
inline Vertex
        add_vertex(BOOST_DIR_CSR_GRAPH_TYPE &g,
                   typename BOOST_DIR_CSR_GRAPH_TYPE::vertex_bundled
const& p) {
Vertex old_num_verts_plus_one = g.m_forward.m_rowstart.size();
g.m_forward.m_rowstart.
push_back(g
...
inline Vertex
        add_vertex(BOOST_BIDIR_CSR_GRAPH_TYPE &g,
                   typename BOOST_BIDIR_CSR_GRAPH_TYPE::vertex_bundled
const& p) {
Vertex old_num_verts_plus_one = g.m_forward.m_rowstart.size();
g.m_forward.m_rowstart.
push_back(g
...
g.m_backward.m_rowstart.
push_back(g
.m_backward.m_rowstart.
...
inline void
        set_property(BOOST_CSR_GRAPH_TYPE &g, Tag
tag,
const Value &value
)
{
get_property_value(g
.m_property, tag) =
value;
}
...
get(Tag tag, BOOST_CSR_GRAPH_TYPE &g) {
    return typename property_map<BOOST_CSR_GRAPH_TYPE, Tag>::type(tag,
                                                                  get(typename property_map<BOOST_CSR_GRAPH_TYPE, Tag>::all_tag(),
                                                                      g));
}
...
get(Tag tag, const BOOST_CSR_GRAPH_TYPE &g, typename property_map<BOOST_CSR_GRAPH_TYPE, Tag>::key_type k) {
    ...
    return lookup_one_property<const typename property_traits<outer_pm>::value_type, Tag>::lookup(get(all_tag(), g, k),
                                                                                                  tag);
}
...
put(Tag tag,
    ...
    typedef typename property_map<BOOST_CSR_GRAPH_TYPE, Tag>::all_tag all_tag;
    lookup_one_property<typename property_map<BOOST_CSR_GRAPH_TYPE, Tag>::plist_type, Tag>::lookup(get(all_tag(), g, k),
                                                                                                   tag) = val;
}

template<BOOST_CSR_GRAPH_TEMPLATE_PARMS>
struct property_map<BOOST_CSR_GRAPH_TYPE, vertex_index_t, typename disable_if<
        detail::is_distributed_selector < Vertex> >::type>
{
...
template<BOOST_CSR_GRAPH_TEMPLATE_PARMS>
struct property_map<BOOST_CSR_GRAPH_TYPE, edge_index_t, typename disable_if<
        detail::is_distributed_selector < Vertex> >::type>
{
...
template<BOOST_CSR_GRAPH_TEMPLATE_PARMS>
struct property_map<BOOST_CSR_GRAPH_TYPE, vertex_all_t, typename disable_if<
        detail::is_distributed_selector < Vertex> >::type>
{
...
template<BOOST_CSR_GRAPH_TEMPLATE_PARMS>
struct property_map<BOOST_CSR_GRAPH_TYPE, edge_all_t, typename disable_if<
        detail::is_distributed_selector < Vertex> >::type>
{
...
template<BOOST_CSR_GRAPH_TEMPLATE_PARMS>
struct property_map<BOOST_CSR_GRAPH_TYPE, graph_all_t, typename disable_if<
        detail::is_distributed_selector < Vertex> >::type>
{

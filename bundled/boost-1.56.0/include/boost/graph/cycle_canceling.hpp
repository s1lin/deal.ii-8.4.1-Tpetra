...
namespace boost {
    ...
    namespace detail {
        ...
        public:
        RecordEdgeMapAndCycleVertex(PredEdgeMap
        pred,
        Vertex &v
        ) :
...
void cycle_canceling(const Graph &g, Weight weight, Reversed rev, ResidualCapacity residual_capacity, Pred pred,
    ...
    while (!bellman_ford_shortest_paths(gres, N,
                                        weight_map(weight).
                                                ...
                                                visitor(
                                                detail::RecordEdgeMapAndCycleVertex<Pred, vertex_descriptor>(pred,
                                                                                                             cycleStart)))) {
...
//setting default distance map
    ...
    void cycle_canceling_dispatch2(
        ...
        cycle_canceling(g, weight, rev, residual_capacity, pred,
                        make_iterator_property_map(d_map.begin(), choose_const_pmap(get_param(params, vertex_index), g,
                                                                                    vertex_index)));
    }

...
namespace boost {
    ...
    namespace detail {
        ...
        stoer_wagner_min_cut(const UndirectedGraph &g, WeightMap weights, ParityMap parities,
            ...
            for (boost::tie(u_iter, u_end) = vertices(g);
            u_iter != u_end;
            ++u_iter) {
    ...
    stoer_wagner_min_cut(const UndirectedGraph &g, WeightMap weights, ParityMap parities,
        ...
        BOOST_CONCEPT_ASSERT(
                (boost::Convertible<typename boost::graph_traits<UndirectedGraph>::directed_category, boost::undirected_tag>));
    ...
    namespace graph {
        namespace detail {
            ...
            struct stoer_wagner_min_cut_impl {
                ...
                result_type operator()(const UndirectedGraph &g, WeightMap weights, const ArgPack &arg_pack) const {
                    ...
                    typename boost::result_of<gen_type(const UndirectedGraph &, const ArgPack &)>::type pq = gen(g,
                                                                                                                 arg_pack);

                    return boost::stoer_wagner_min_cut(g,
                                                       weights,
                                                       arg_pack[_parity_map | boost::dummy_property_map()],
                                                       boost::detail::make_property_map_from_arg_pack_gen<tag::vertex_assignment_map, vertex_descriptor>(
                                                               vertex_descriptor())(g, arg_pack),
                                                       pq,
                                                       boost::detail::override_const_property(arg_pack,
                                                                                              _vertex_index_map, g,
                                                                                              vertex_index)
                    );
                }
        ...
        BOOST_GRAPH_MAKE_FORWARDING_FUNCTION(stoer_wagner_min_cut,
        2,4)
    }

    // Named parameter interface
    BOOST_GRAPH_MAKE_OLD_STYLE_PARAMETER_FUNCTION(stoer_wagner_min_cut,
    2)
    namespace graph {

...
namespace boost {
    namespace detail {
        enum bm_case_t {
            BM_NO_CASE_CHOSEN, BM_CASE_A, BM_CASE_B, BM_CASE_C, BM_CASE_D, BM_CASE_E
        };
    }
    ...
    struct planar_dfs_visitor : public dfs_visitor<> {
        ...
        void back_edge(const Edge &e, Graph &g) {
            ...
            if (t != get(parent, s)) {
                ...
                put(low, s,
                    min
                BOOST_PREVENT_MACRO_SUBSTITUTION(s_low_df_number,
                                                 t_df_number)
                );

                put(least_ancestor, s,
                    min
                BOOST_PREVENT_MACRO_SUBSTITUTION(s_least_ancestor_df_number,
                                                 t_df_number
                )
                );
        ...
        void finish_vertex(const Vertex &u, Graph &) {
            ...
            if (u_parent != u) {
                put(low, u_parent,
                    min
                BOOST_PREVENT_MACRO_SUBSTITUTION(u_lowpoint,
                                                 u_parent_lowpoint
                )
                );
            }
    ...
    private:
        ...
        void walkup(vertex_t v) {
            ...
            for (boost::tie(oi, oi_end) = out_edges(v, g);
            oi != oi_end;
            ++oi)
            {
                edge_t e(*oi);
        ...
        void clean_up_embedding(graph::detail::store_embedding) {
            ...
            for (boost::tie(xi, xi_end) = vertices(g);
            xi != xi_end;
            ++xi)
            {
        ...
        void extract_kuratowski_subgraph(OutputIterator o_itr, EdgeIndexMap em) {
            ...
            if (chosen_case != detail::BM_CASE_B && chosen_case != detail::BM_CASE_A) {
                ...
                if (chosen_case == detail::BM_CASE_E) {
                    ...
                    for (boost::tie(oei, oei_end) = out_edges(w, g);
                    oei != oei_end;
                    ++oei)
                    {
                        if (!outer_face_edge[*oei])

...
//@{
    ...
    template<typename T>
    inline local_property<T> local(T x) { return local_property<T>(x); }
    ...
    template<typename T>
    inline global_property<T> global(T x) { return global_property<T>(x); }
//@}
...
// explicitly assigned here.
    ...
    public:
        ...
        subgraph(vertices_size_type n, const graph_property_type &p = graph_property_type())
            ...
            for (boost::tie(v, v_end) = vertices(m_graph);
            v != v_end;
            ++v)
            m_global_vertex[i++] = *v;
        }
        ...
        subgraph(const subgraph &x)
                : m_parent(x.m_parent), m_edge_counter(x.m_edge_counter), m_global_vertex(x.m_global_vertex),
        ...
        const subgraph &root() const { return is_root() ? *this : m_parent->root(); }
        ...
        operator[](Descriptor x) { return is_root() ? m_graph[x] : root().m_graph[local_to_global(x)]; }

        template<typename Descriptor>
        typename graph::detail::bundled_result<Graph, Descriptor>::type const &
        operator[](Descriptor x) const { return is_root() ? m_graph[x] : root().m_graph[local_to_global(x)]; }

        // Local property access returns the local property of the given descripor.
        template<typename Descriptor>
        typename graph::detail::bundled_result<Graph, Descriptor>::type &
        operator[](local_property<Descriptor> x) { return m_graph[x.value]; }

        template<typename Descriptor>
        typename graph::detail::bundled_result<Graph, Descriptor>::type const &
        operator[](local_property<Descriptor> x) const { return m_graph[x.value]; }
        ...
        template<typename Descriptor>
        typename graph::detail::bundled_result<Graph, Descriptor>::type &
        operator[](global_property<Descriptor> x) { return (*this)[x.value]; }

        template<typename Descriptor>
        typename graph::detail::bundled_result<Graph, Descriptor>::type const &
        operator[](global_property<Descriptor> x) const { return (*this)[x.value]; }
...
// Functions special to the Subgraph Class
    ...
    add_vertex(typename subgraph<G>::vertex_descriptor u_global,
        ...
        if (is_directed(g)) { // not necessary for undirected graph
            ...
            typename subgraph<G>::out_edge_iterator ei, ei_end;
            for (boost::tie(vi, vi_end) = vertices(r);
            vi != vi_end;
            ++vi) {
                v_global = *vi;
                ...
                for (boost::tie(ei, ei_end) = out_edges(*vi, r);
                ei != ei_end;
                ++ei) {
                    e_global = *ei;
...
// Functions required by the IncidenceGraph concept
    ...
    out_edges(typename graph_traits<G>::vertex_descriptor v, const subgraph<G> &g) { return out_edges(v, g.m_graph); }

    template<typename G>
    typename graph_traits<G>::degree_size_type
    out_degree(typename graph_traits<G>::vertex_descriptor v, const subgraph<G> &g) { return out_degree(v, g.m_graph); }

    template<typename G>
    typename graph_traits<G>::vertex_descriptor
    source(typename graph_traits<G>::edge_descriptor e, const subgraph<G> &g) { return source(e, g.m_graph); }

    template<typename G>
    typename graph_traits<G>::vertex_descriptor
    target(typename graph_traits<G>::edge_descriptor e, const subgraph<G> &g) { return target(e, g.m_graph); }
...
// Functions required by the BidirectionalGraph concept
    ...
    in_edges(typename graph_traits<G>::vertex_descriptor v, const subgraph<G> &g) { return in_edges(v, g.m_graph); }

    template<typename G>
    typename graph_traits<G>::degree_size_type
    in_degree(typename graph_traits<G>::vertex_descriptor v, const subgraph<G> &g) { return in_degree(v, g.m_graph); }

    template<typename G>
    typename graph_traits<G>::degree_size_type
    degree(typename graph_traits<G>::vertex_descriptor v, const subgraph<G> &g) { return degree(v, g.m_graph); }
...
// Functions required by the VertexListGraph concept
    ...
    vertices(const subgraph<G> &g) { return vertices(g.m_graph); }

    template<typename G>
    typename subgraph<G>::vertices_size_type
    num_vertices(const subgraph<G> &g) { return num_vertices(g.m_graph); }
...
// Functions required by the EdgeListGraph concept
    ...
    edges(const subgraph<G> &g) { return edges(g.m_graph); }

    template<typename G>
    typename subgraph<G>::edges_size_type
    num_edges(const subgraph<G> &g) { return num_edges(g.m_graph); }
...
// Functions required by the AdjacencyMatrix concept
    ...
    edge(typename subgraph<G>::vertex_descriptor u,
         ...
         const subgraph<G> &g) { return edge(u, v, g.m_graph); }
...
// contain vertex descriptors u and v.
    ...
    add_edge(typename subgraph<G>::vertex_descriptor u,
             typename subgraph<G>::vertex_descriptor v,
             subgraph<G> &g) { return add_edge(u, v, typename G::edge_property_type(), g); }
...
// Miscellaneous Functions
    ...
    template<typename G>
    typename subgraph<G>::vertex_descriptor
    vertex(typename subgraph<G>::vertices_size_type n, const subgraph<G> &g) { return vertex(n, g.m_graph); }

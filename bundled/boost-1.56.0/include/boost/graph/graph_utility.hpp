...
namespace boost {
    ...
    void print_in_edges(const IncidenceGraph &G, Name name) {
        ...
        for (boost::tie(ui, ui_end) = vertices(G);
        ui != ui_end;
        ++ui) {
            std::cout << get(name, *ui) << " <-- ";
            typename graph_traits<IncidenceGraph>
            ::in_edge_iterator ei, ei_end;
            for (boost::tie(ei, ei_end) = in_edges(*ui, G);
            ei != ei_end;
            ++ei)
    ...
    void print_graph_dispatch(const IncidenceGraph &G, Name name, directed_tag) {
        ...
        ++ui) {
            ...
            for (boost::tie(ei, ei_end) = out_edges(*ui, G);
            ei != ei_end;
            ++ei)
    ...
    inline bool is_connected(const VertexListGraph &g, VertexColorMap color) {
        ...
        ++vi)
        if (*ui != *vi) {
            for (boost::tie(ci, ci_end) = vertices(g);
            ci != ci_end;
            ++ci)
    ...
    std::pair<T1, T2>
    make_list(const T1 &t1, const T2 &t2) { return std::make_pair(t1, t2); }

    template<class T1, class T2, class T3>
    std::pair<T1, std::pair<T2, T3> >
    make_list(const T1 &t1, const T2 &t2, const T3 &t3) { return std::make_pair(t1, std::make_pair(t2, t3)); }

    template<class T1, class T2, class T3, class T4>
    std::pair<T1, std::pair<T2, std::pair<T3, T4> > >
    make_list(const T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4) { return std::make_pair(t1, std::make_pair(t2,
                                                                                                                 std::make_pair(
                                                                                                                         t3,
                                                                                                                         t4)));
    }
    ...
    make_list(const T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4, const T5 &t5) { return std::make_pair(t1,
                                                                                                            std::make_pair(
                                                                                                                    t2,
                                                                                                                    std::make_pair(
                                                                                                                            t3,

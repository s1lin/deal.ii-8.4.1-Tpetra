...
namespace boost {
    ...
    public:
        ...
        vertex_bundled &operator[](vertex_descriptor v) { return get(vertex_bundle, *this, v); }

        const vertex_bundled &operator[](vertex_descriptor v) const { return get(vertex_bundle, *this, v); }

        edge_bundled &operator[](edge_descriptor e) { return get(edge_bundle, *this, e); }

        const edge_bundled &operator[](edge_descriptor e) const { return get(edge_bundle, *this, e); }
    ...
    out_degree(typename adjacency_matrix<D, VP, EP, GP, A>::vertex_descriptor u,
        ...
        for (boost::tie(f, l) = out_edges(u, g);
        f != l;
        ++f)
        ++n;
        return n;
    ...
    in_degree(typename adjacency_matrix<D, VP, EP, GP, A>::vertex_descriptor u,
        ...
        for (boost::tie(f, l) = in_edges(u, g);
        f != l;
        ++f)
        ++n;
        return n;
    ...
    struct adj_mat_pm_helper<D, VP, EP, GP, A, Tag, edge_property_tag> {
        ...
        static single_nonconst_type
        get_nonconst_one(adjacency_matrix<D, VP, EP, GP, A> &g, Tag tag, edge_descriptor e) {
            return lookup_one_property<EP, Tag>::lookup(*static_cast<EP *>(e.get_property()), tag);
        }

        static single_const_type
        get_const_one(const adjacency_matrix<D, VP, EP, GP, A> &g, Tag tag, edge_descriptor e) {

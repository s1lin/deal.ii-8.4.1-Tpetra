...
namespace boost {
    ...
    namespace detail {
        ...
        class Marker {
            ...
            static value_type done() { return (std::numeric_limits<value_type>::max) () / 2; }
        ...
        public:
            ...
            bool is_multiple_tagged(Vertex node) const { return data[get(id, node)] >= multiple_tag; }
        ...
        public:
            ...
            void eliminate(vertex_t node) {
                ...
                adj_iter v, ve;
                for (boost::tie(v, ve) = adjacent_vertices(node, G);
                v != ve;
                ++v) {
                    vertex_t v_node = *v;
            ...
            void update(Stack llist, size_type &min_degree) {
                ...
                while (!llist.empty()) {
                    ...
                    adj_iter i, ie;
                    for (boost::tie(i, ie) = adjacent_vertices(current, G);
                    i != ie;
                    ++i) {
                        vertex_t i_node = *i;
                    ...
                    while (!qxlist.empty()) {
                        ...
                        ++i) {
                            ...
                            if (numbering.is_numbered(i_node)) {
                                adj_iter j, je;
                                for (boost::tie(j, je) = adjacent_vertices(i_node, G);
                                j != je;
                                ++j) {

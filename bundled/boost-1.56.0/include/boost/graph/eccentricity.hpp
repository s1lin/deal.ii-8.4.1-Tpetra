...
namespace boost {
    ...
    all_eccentricities(const Graph &g, const DistanceMatrix &dist, EccentricityMap ecc) {
        ...
        ++i) {
            ...
            r = min
            BOOST_PREVENT_MACRO_SUBSTITUTION(r, e);
            d = max
            BOOST_PREVENT_MACRO_SUBSTITUTION(d, e);
        }
    ...
    radius_and_diameter(const Graph &g, EccentricityMap ecc) {
        ...
        for (i = boost::next(i); i != end; ++i) {
            ...
            radius = min
            BOOST_PREVENT_MACRO_SUBSTITUTION(radius, cur);
            diameter = max
            BOOST_PREVENT_MACRO_SUBSTITUTION(diameter, cur);
        }
    ...
    radius(const Graph &g, EccentricityMap ecc) { return radius_and_diameter(g, ecc).first; }


    template<typename Graph, typename EccentricityMap>
    inline typename property_traits<EccentricityMap>::value_type
    diameter(const Graph &g, EccentricityMap ecc) { return radius_and_diameter(g, ecc).second; }

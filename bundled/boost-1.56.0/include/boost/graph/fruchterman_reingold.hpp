...
namespace boost {
    ...
    struct all_force_pairs {
        ...
        void operator()(const Graph &g, ApplyForce apply_force) {
            ...
            for (boost::tie(v, end) = vertices(g);
            v != end;
            ++v) {
                vertex_iterator u = v;
                for (++u; u != end; ++u) {
    ...
    struct grid_force_pairs {
        ...
        BOOST_STATIC_ASSERT (Point::dimensions
        == 2);
        ...
        do {
            ...
            if (double temp = cool()) {
                ...
                BGL_FORALL_VERTICES_T(v, g, Graph)
                {
                    ...
                    put(position, v, topology.adjust(get(position, v), get(displacement, v) * (min
                    BOOST_PREVENT_MACRO_SUBSTITUTION(disp_size, temp) / disp_size)));

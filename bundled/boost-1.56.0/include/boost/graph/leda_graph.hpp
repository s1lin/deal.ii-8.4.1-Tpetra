...
namespace boost {
    ...
    private:
        ...
        bool equal(const adjacency_iterator &other) const { return base == other.base; }
    ...
    private:
        ...
        bool equal(const in_edge_iterator &other) const { return base == other.base; }
    ...
    private:
        ...
        bool equal(const edge_iterator &other) const { return base == other.base; }
...
namespace boost {
    ...
    template<class vtype, class etype>
    typename graph_traits<leda::GRAPH < vtype, etype> >

    ::vertex_descriptor
    source(typename graph_traits<leda::GRAPH < vtype, etype>
...
::vertex_descriptor
target(typename graph_traits<leda::GRAPH < vtype, etype>
...
return
std::make_pair( Iter(g.first_adj_edge(u, 0), &g), Iter(0, &g)
);
}
...
return
std::make_pair( Iter(g.first_adj_edge(u, 1), &g), Iter(0, &g)
);
}
...
remove_edge(
    ...
    graph_traits<leda::graph>::out_edge_iterator
            i, iend;
    for (boost::tie(i, iend) = out_edges(u, g);
    i != iend;
    ++i)
    if (target(*i, g) == v)
        g.del_edge(*i);
...
template<class vtype, class etype>
inline typename property_map<leda::GRAPH < vtype, etype>, vertex_all_t>
...
template<class vtype, class etype>
inline typename property_map<leda::GRAPH < vtype, etype>, edge_all_t>
...
make_leda_node_property_map(leda::node_array<E>
& a)
{
typedef leda_node_property_map<E, E &, leda::node_array < E>*>
pmap_type;
return
pmap_type(&a);
}
...
make_leda_node_property_map(leda::node_map<E>
& a)
{
typedef leda_node_property_map<E, E &, leda::node_map < E>*>
pmap_type;
return
pmap_type(&a);
}
...
put(PropertyTag p, leda::GRAPH <vtype, etype> &g,
    ...
    typedef typename property_map<leda::GRAPH < vtype, etype>, PropertyTag > ::type
    Map;
    Map pmap = get(p, g);
    put(pmap, key, value);
}
...
make_leda_edge_property_map(leda::edge_array<E>
& a)
{
typedef leda_edge_property_map<E, E &, leda::edge_array < E>*>
pmap_type;
return
pmap_type(&a);
}
...
make_leda_edge_property_map(leda::edge_map<E>
& a)
{
typedef leda_edge_property_map<E, E &, leda::edge_map < E>*>
pmap_type;
return
pmap_type(&a);
}

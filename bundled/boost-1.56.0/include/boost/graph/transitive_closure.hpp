...
namespace boost {
    ...
    namespace detail {
        ...
        union_successor_sets(const std::vector<std::size_t> &s1,
            ...
            for (std::size_t k = 0; k < s1.size(); ++k)
                s3[k] = min
            BOOST_PREVENT_MACRO_SUBSTITUTION(s1[k], s2[k]);
        }
    }                             // namespace detail
    ...
    void transitive_closure(const Graph &g, GraphTC &tc,
        ...
        // Need to add it to transitive closure.
        {
            ...
            ++i)
            {
                ...
                for (boost::tie(ab, ae) = adjacent_vertices(*i, g);
                ab != ae;
                ++ab)
                {
                    if (*ab == *i)
    ...
    void warshall_transitive_closure(G &g) {
        ...
        for (boost::tie(ki, ke) = vertices(g);
        ki != ke;
        ++ki)
        for (boost::tie(ii, ie) = vertices(g);
        ii != ie;
        ++ii)
        if (edge(*ii, *ki, g).second)
            for (boost::tie(ji, je) = vertices(g);
        ji != je;
        ++ji)
    ...
    void warren_transitive_closure(G &g) {
        ...
        for (boost::tie(ic, ie) = vertices(g), ++ic;
        ic != ie;
        ++ic)
        for (boost::tie(kc, ke) = vertices(g);
        *kc != *ic;
        ++kc)
        if (edge(*ic, *kc, g).second)
            for (boost::tie(jc, je) = vertices(g);
        jc != je;
        ++jc)
        ...
        for (boost::tie(ic, ie) = vertices(g), --ie;
        ic != ie;
        ++ic)

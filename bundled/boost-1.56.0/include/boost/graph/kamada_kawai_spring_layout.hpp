...
namespace boost {
    namespace detail {
        namespace graph {
            ...
            T compute_edge_length(const Graph &, DistanceMap, IndexMap,
                                  edge_or_side<true, T> length) { return length.value; }
                            ...
                            for (ui = vertices(g).first, end = vertices(g).second; ui != end; ++ui) {
                                ...
                                if (i != p) {
                                    ...
                                    for (std::size_t i = 0; i < Point::dimensions; ++i) {
                                        for (std::size_t j = 0; j < Point::dimensions; ++j) {
                                            if (i == j) {
                                                dE_d_d[i][i] += k_mi * (1 + (l_mi * (diff[i] * diff[i] - dist_squared) *
                                                                             inv_dist_cubed));
                                            } else {
    ...
    edge_length(T x) { return detail::graph::edge_or_side<true, T>(x); }

    /// States that the given quantity is a display area side length.
    template<typename T>
    inline detail::graph::edge_or_side<false, T>
    side_length(T x) { return detail::graph::edge_or_side<false, T>(x); }

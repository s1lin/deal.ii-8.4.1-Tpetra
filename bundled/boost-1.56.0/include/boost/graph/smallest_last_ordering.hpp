...
namespace boost {
    ...
    smallest_last_vertex_ordering(const VertexListGraph &G, Order order,
        ...
        for (boost::tie(v, vend) = vertices(G);
        v != vend;
        ++v) {
            put(marker, *v, num);
        ...
        while (1) {
            ...
            for (boost::tie(v, vend) = adjacent_vertices(node, G);
            v != vend;
            ++v)
            ...
            if (get(marker, *v) > current_order) { //*v is unordered vertex
                ...
                BOOST_USING_STD_MIN();
                minimum_degree = min
                BOOST_PREVENT_MACRO_SUBSTITUTION(minimum_degree, get(degree, *v));
    ...
    smallest_last_vertex_ordering(const VertexListGraph &G, Order order) {
        ...
        smallest_last_vertex_ordering(G, order,
                                      make_shared_array_property_map(num_vertices(G), degree_size_type(0),
                                                                     get(vertex_index, G)),
                                      make_shared_array_property_map(num_vertices(G), (std::size_t) (0),
                                                                     get(vertex_index, G)));
    }
    ...
    smallest_last_vertex_ordering(const VertexListGraph &G) {
        ...
        smallest_last_vertex_ordering(G, make_iterator_property_map(o.begin(),

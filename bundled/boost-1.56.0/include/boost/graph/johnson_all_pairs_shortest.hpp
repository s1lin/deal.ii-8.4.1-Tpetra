...
namespace boost {
    ...
    johnson_all_pairs_shortest_paths(VertexAndEdgeListGraph &g1,
        ...
        typename Traits2::vertex_descriptor s = *vertices(g2).first;
        {
            ...
            int i = 1;
            for (boost::tie(v, v_end) = vertices(g1);
            v != v_end;
            ++v, ++i) {
            ...
            for (boost::tie(e, e_end) = edges(g1);
            e != e_end;
            ++e) {
        ...
        for (boost::tie(v, v_end) = vertices(g2);
        v != v_end;
        ++v)
        put(d, *v, inf);
        ...
        if (bellman_ford_shortest_paths
            ...
            for (boost::tie(e, e_end) = edges(g2);
            e != e_end;
            ++e) {
            ...
            for (boost::tie(u, u_end) = vertices(g2);
            u != u_end;
            ++u) {

...
namespace boost {
    namespace detail {
        ...
        bool floyd_warshall_dispatch(const VertexListGraph &g,
            ...
            for (boost::tie(k, lastk) = vertices(g);
            k != lastk;
            k++)
            for (boost::tie(i, lasti) = vertices(g);
            i != lasti;
            i++)
            if (d[*i][*k] != inf)
                for (boost::tie(j, lastj) = vertices(g);
            j != lastj;
            j++)
            if (d[*k][*j] != inf)
                d[*i][*j] =
    ...
    bool floyd_warshall_all_pairs_shortest_paths(
        ...
        for (boost::tie(firstv, lastv) = vertices(g);
        firstv != lastv;
        firstv++)
        for (boost::tie(firstv2, lastv2) = vertices(g);
        firstv2 != lastv2;
        firstv2++)
        ...
        for (boost::tie(first, last) = edges(g);
        first != last;
        first++)
        {
    ...
    bool floyd_warshall_all_pairs_shortest_paths(
        ...
        return detail::floyd_warshall_noninit_dispatch(g, d,
                                                       choose_const_pmap(get_param(params, edge_weight), g,
                                                                         edge_weight),
                                                       params);
    }

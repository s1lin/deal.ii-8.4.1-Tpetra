...
namespace boost {
    ...
    namespace detail {
        ...
        min_degree_vertex(Graph &g) {
            ...
            for (boost::tie(i, iend) = vertices(g);
            i != iend;
            ++i)
            if (degree(*i, g) < delta) {
                delta = degree(*i, g);
        ...
        void neighbors(const Graph &g,
            ...
            for (boost::tie(ai, aend) = adjacent_vertices(u, g);
            ai != aend;
            ++ai)
            *result++ = *ai;
        }
    ...
    edge_connectivity(VertexListGraph &g, OutputIterator disconnecting_set) {
        ...
        while (!non_neighbor_S.empty()) { // at most n - 1 times
            ...
            if (alpha_S_k < alpha_star) {
                ...
                S_star.clear();
                for (boost::tie(vi, vi_end) = vertices(flow_g);
                vi != vi_end;
                ++vi)
        ...
        for (si = S_star.begin(); si != S_star.end(); ++si) {
            ...
            for (boost::tie(ei, ei_end) = out_edges(*si, g);
            ei != ei_end;
            ++ei)

...
namespace boost {
    ...
    namespace detail {
        ...
        public:
            ...
            inline bool operator==(const stored_edge &x) const { return m_target == x.get_target(); }
    ...
    in_degree(typename Config::vertex_descriptor u,
              const undirected_graph_helper<Config> &g_) { return degree(u, g_); }
    ...
    struct bidirectional_graph_helper_with_property
        ...
        get_parallel_edge_sublist(typename Config::edge_descriptor e,
                                  const graph_type &g,
                                  void *) { return out_edges(source(e, g), g); }

        std::pair<out_edge_iterator, out_edge_iterator>
        get_parallel_edge_sublist(typename Config::edge_descriptor e,
                                  const graph_type &g,
                                  setS *) { return edge_range(source(e, g), target(e, g), g); }

        std::pair<out_edge_iterator, out_edge_iterator>
        get_parallel_edge_sublist(typename Config::edge_descriptor e,
                                  const graph_type &g,
                                  multisetS *) { return edge_range(source(e, g), target(e, g), g); }

        std::pair<out_edge_iterator, out_edge_iterator>
        get_parallel_edge_sublist(typename Config::edge_descriptor e,
                                  const graph_type &g,
                                  hash_setS *) { return edge_range(source(e, g), target(e, g), g); }
    ...
    remove_out_edge_if(typename Config::vertex_descriptor u, Predicate pred,
        ...
        for (boost::tie(out_i, out_end) = out_edges(u, g);
        out_i != out_end;
        ++out_i)
        if (pred(*out_i)) {
            detail::remove_directed_edge_dispatch
    ...
    public:
        ...
        inline void copy_impl(const adj_list_impl &x_) {
            ...
            for (boost::tie(vi, vi_end) = vertices(x);
            vi != vi_end;
            ++vi) {
            ...
            for (boost::tie(ei, ei_end) = edges(x);
            ei != ei_end;
            ++ei) {
                edge_descriptor e;
                bool inserted;
    ...
    add_edge(typename Config::vertex_descriptor u,
             ...
             vec_adj_list_impl<Graph, Config, Base> &g_) {
        BOOST_USING_STD_MAX();
        typename Config::vertex_descriptor x = max
        BOOST_PREVENT_MACRO_SUBSTITUTION(u, v);

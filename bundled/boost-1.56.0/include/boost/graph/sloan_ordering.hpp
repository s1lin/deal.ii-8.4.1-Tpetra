...
namespace boost {
    ...
    sloan_start_end_vertices(Graph &G,
        ...
        boost::iterator_property_map<vec_iter, VertexID, size_type, size_type &> dist_pmap(dist.begin(),
                                                                                           get(vertex_index, G));
        ...
        do {
            ...
            for (typename std::vector<typename graph_traits<Graph>::vertices_size_type>::iterator iter = dist.begin();
                 iter != dist.end(); ++iter)
                *iter = 0;
            ...
            *permutation++ = u;                      //Puts u to the first position in the permutation-vector
            ...
            ++ei) {
                ...
                if (get(color, v) == Color::green()) {      //tests if the vertex is inactive
                    ...
                    for (boost::tie(ei2, ei2_end) = out_edges(v, g);
                    ei2 != ei2_end;
                    ++ei2) {
                        w = target(*ei2, g);

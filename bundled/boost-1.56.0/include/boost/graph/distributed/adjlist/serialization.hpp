        ...
        namespace parallel {
            ...
            struct graph_loader {
                ...
                graph_loader(Graph &g, Archive &ar)
                        : m_g(g), m_ar(ar), m_pg(g.process_group()), m_requested_vertices(num_processes(m_pg)),
                          m_remote_vertices(num_processes(m_pg)), m_property_ptrs(num_processes(m_pg)) {
                    g.clear();
                    load_prefix();
                    load_vertices();
                    load_edges();
            ...
            private:
                ...
                template<class Anything>
                void add_edge(
                        vertex_descriptor u, vertex_descriptor v, edge_property_type const &property,
                        void *property_ptr, Anything);

                void add_edge(
                        vertex_descriptor u, vertex_descriptor v, edge_property_type const &property,
                        void *property_ptr, vecS);
                ...
                std::vector<std::vector<unsafe_pair < void * , void *> > >
                m_property_ptrs;
            };
...
# endif
            ...
            void graph_loader<Graph, Archive, VertexListS>::add_edge(
                    vertex_descriptor u, vertex_descriptor v, edge_property_type const &property, void *property_ptr,
                    Anything) {
...
# endif
            ...
            void graph_loader<Graph, Archive, VertexListS>::commit_pending_in_edges(bidirectionalS) {
                ...
                BOOST_FOREACH(pending_in_edge const&e, m_pending_in_edges)
                {
                    ...
                    std::vector<unsafe_pair < void * , void *> > ::iterator
                    i = std::lower_bound(
                            m_property_ptrs[owner(u)].begin(), m_property_ptrs[owner(u)].end(),
            ...
            graph_loader<Graph, Archive, VertexListS>::resolve_remote_vertex(
                ...
                typename std::vector<serializable_vertex_descriptor>::const_iterator
                        i = std::lower_bound(
                        m_requested_vertices[owner(u)].begin(), m_requested_vertices[owner(u)].end(),
...
// vertex_in_edges vertex property.
    ...
    void PBGL_DISTRIB_ADJLIST_TYPE::load(std::string const &filename) {
        ...
        std::set_difference(
                disk_files.begin(), disk_files.end(), consumed_files.begin(), consumed_files.end(),

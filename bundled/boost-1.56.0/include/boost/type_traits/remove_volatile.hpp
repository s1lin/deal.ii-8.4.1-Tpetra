        ...
        struct remove_volatile_impl {
            typedef typename remove_volatile_helper<
                    typename cv_traits_imp<BOOST_TT_AUX_CV_TRAITS_IMPL_PARAM(
                            T)>::unqualified_type, ::boost::is_const<T>::value
            >::type type;
        };
...
// * convert a type T to a non-volatile type - remove_volatile<T>
    ...
    BOOST_TT_AUX_TYPE_TRAIT_DEF1(remove_volatile, T,
    typename boost::detail::remove_volatile_impl<T>::type)
    ...
    BOOST_TT_AUX_TYPE_TRAIT_PARTIAL_SPEC1_2(typename T, std::size_t N, remove_volatile, T const volatile[N],
                                            T const type[N])

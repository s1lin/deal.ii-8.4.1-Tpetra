...
typename detail::call_param<T,>::type _
))
:
...
BOOST_FUSION_GPU_ENABLED
        BOOST_PP_CAT(vector_data, N)
&
operator=(BOOST_PP_CAT(vector_data, N)
const& vec)
{
...
N, FUSION_VECTOR_MEMBER_ITER_DECL_VAR, _
...
BOOST_FUSION_GPU_ENABLED
        BOOST_PP_CAT(vector, N)
&
operator=(BOOST_PP_CAT(vector, N)
const& vec)
{
...
BOOST_FUSION_GPU_ENABLED
        BOOST_PP_CAT(vector, N)
&
operator=(BOOST_PP_CAT(vector, N) < BOOST_PP_ENUM_PARAMS(N, U) >
const& vec)
{
BOOST_PP_REPEAT(N, FUSION_VECTOR_MEMBER_ASSIGN, _

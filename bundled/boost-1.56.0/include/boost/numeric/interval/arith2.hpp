...
namespace boost {
    namespace numeric {
        ...
        interval <T, Policies> pow(const interval <T, Policies> &x, int pwr) {
            ...
            if (interval_lib::user::is_neg(x.upper())) {        // [-2,-1]
                ...
                if (pwr & 1) {   // [-1,1]^1
                    ...
                    return I(static_cast<T>(0), pow_up(max
                    BOOST_PREVENT_MACRO_SUBSTITUTION(static_cast<T>(-x.lower()), x.upper()), pwr, rnd), true);
                }
            } else {                                // [1,2]
        ...
        interval <T, Policies> nth_root(interval < T, Policies >
        const &x,
        int k
        ) {
        typedef interval <T, Policies> I;
        if (
        interval_lib::detail::test_input(x)
        ) return
        ...
        return I(-interval_lib::detail::root_aux_up<R>(-x.
    ...
    -interval_lib::detail::root_aux_dn<R>(-x.
...
return
I(interval_lib::detail::root_aux_dn<R>(x.lower(), k), u,
true);
}

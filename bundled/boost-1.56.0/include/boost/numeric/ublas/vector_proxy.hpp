    ...
    namespace numeric {
        namespace ublas {
            ...
            public:
                ...
                operator=(const vector_range &vr) {
            ...
            return
            vector_range<V> (data, range_type(start, stop)
            );
        }
        ...
        subrange(const V &data, typename V::size_type start, typename V::size_type stop) {
            typedef basic_range<typename V::size_type, typename V::difference_type> range_type;
            return vector_range <
            const V> (data, range_type(start, stop));
        }
...
public:
    ...
    operator=(const vector_slice &vs) {
        // ISSUE need a temporary, proxy can be overlaping alias
...
return
vector_slice<V> (data, slice_type(start, stride, size)
);
}
template<class V>
BOOST_UBLAS_INLINE
...
subslice(const V &data, typename V::size_type start, typename V::difference_type stride, typename V::size_type size) {
    typedef basic_slice<typename V::size_type, typename V::difference_type> slice_type;
    return vector_slice <
    const V> (data, slice_type(start, stride, size));
}
...
return
vector_slice<V> (data, s
);
}
template<class V>
BOOST_UBLAS_INLINE
const vector_slice<const V>
...
public:
    ...
    BOOST_UBLAS_INLINE
            vector_indirect
    &
    assign_temporary (vector_indirect
    &vi) {
        // assign elements, proxied container remains the same
...
BOOST_UBLAS_INLINE
        vector_indirect<V, indirect_array < A>
>
project (vector_indirect<V, indirect_array < A>
> &data,
const indirect_array <A> &ia
) {
return data.
...
template<class V, class A>
BOOST_UBLAS_INLINE
const vector_indirect<V, indirect_array < A>
>

project(const vector_indirect<V, indirect_array < A>

        ...
        namespace odeint {
                ...
                void increment() {
                    ...
                    if (++m_t_start != m_t_end) {
                        while (detail::less_with_sign(this->m_t, static_cast<time_type>(*m_t_start), this->m_dt)) {
                            const time_type current_dt = detail::min_abs(this->m_dt,
                                                                         static_cast<time_type>(*m_t_start) -
                                                                         this->m_t);
            ...
            protected:
                ...
                void increment() {
                    if (++m_t_start != m_t_end) {
                        while (detail::less_with_sign(this->m_t, static_cast<time_type>(*m_t_start), this->m_dt)) {
                            if (detail::less_with_sign(static_cast<time_type>(*m_t_start) - this->m_t, this->m_dt,
                                                       this->m_dt)) {
                ...
                void step_loop(time_type &dt) {
                    ...
                    if (trials == max_attempts) {
                        BOOST_THROW_EXCEPTION(std::overflow_error(
                                "Adaptive iterator : Maximal number of iterations reached. A step size could not be found."));
                    }
                }
                ...
                void increment() {
                    ...
                    if (++m_t_start != m_t_end) {
                        ...
                        while (detail::less_with_sign(st.current_time(), this->m_t, this->m_dt)) {
                            ...
                            if (detail::less_with_sign(m_final_time - st.current_time(), st.current_time_step(),

 ...
 */
        ...
        public:
            ...
            void set_reduce(const Reduce &reduce) { property_map.set_reduce(reduce); }
        ...
        get(const caching_property_map<PropertyMap> &pm, const Key &key) { return get(pm.base(), key); }
        ...
        local_put(const caching_property_map<PropertyMap> &pm, const Key &key,
                  const Value &value) { local_put(pm.base(), key, value); }

        template<typename PropertyMap, typename Key, typename Value>
        inline void
        cache(const caching_property_map<PropertyMap> &pm, const Key &key,
              const Value &value) { cache(pm.base(), key, value); }
        ...
        make_caching_property_map(const PropertyMap &pm) { return caching_property_map<PropertyMap>(pm); }

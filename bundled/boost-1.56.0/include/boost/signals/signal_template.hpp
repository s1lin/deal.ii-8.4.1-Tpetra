...
namespace boost {
    ...
    typename F>
    struct caller {
        typedef BOOST_SIGNALS_ARGS_STRUCT
...
Combiner &combiner() { return *unsafe_any_cast<Combiner>(&impl->combiner_); }

const Combiner &combiner() const { return *unsafe_any_cast<const Combiner>(&impl->combiner_); }
...
const slot_type &in_slot,
        BOOST_SIGNALS_NAMESPACE::connect_position
at)
{
...
return impl->
connect_slot(in_slot
.

get_slot_function(), stored_group(),
        in_slot
...
(
slot_call_iterator(notification
.impl->slots_.

...
// this overload picks up heterogeneous units that are not scaled.
            ...
            inline std::string
            to_string_impl(const unit <Dimension, heterogeneous_system<
                    heterogeneous_system_impl < Units, Dimension, dimensionless_type>>
            ...
            std::string str;
            f.
            template append_units_to<Units>(str);
            return(str);
        }
...
/// INTERNAL ONLY
        ...
        inline std::string
        to_string_impl(const unit <dimensionless_type, heterogeneous_system<
...
/// INTERNAL ONLY
    ...
    inline std::string
    to_string_impl(const unit <dimensionless_type, heterogeneous_system<
            heterogeneous_system_impl < dimensionless_type, dimensionless_type, Scale>>
    ...
    std::string str;
    f.
    template append_scale_to<Scale>(str);
    return(str);
}
...
inline std::string
to_string_impl(const unit <Dimension, heterogeneous_system<heterogeneous_system_impl < Units, Dimension, Scale>>
...
std::string without_scale = f(unit < Dimension, heterogeneous_system < heterogeneous_system_impl < Units, Dimension,
                              dimensionless_type > > > ());

if (f.
is_default_string(without_scale, unit<Dimension,
...
inline std::string
to_string_impl(const unit <Dimension, heterogeneous_system<
        heterogeneous_system_impl < list < heterogeneous_system_dim < Unit, static_rational < 1>>, dimensionless_type
...
str +=
f(unit<Dimension,
        heterogeneous_system < heterogeneous_system_impl < list < heterogeneous_system_dim < Unit, static_rational < 1>
>, dimensionless_type>, Dimension, dimensionless_type> > >());
...
to_string_impl(
        ...
        heterogeneous_system<
                ...
                list < heterogeneous_system_dim < scaled_base_unit < Unit, UnitScale>, static_rational<1>>,
        dimensionless_type
...
f(
        ...
        heterogeneous_system <
        heterogeneous_system_impl <
        list < heterogeneous_system_dim < Unit, static_rational < 1>
>, dimensionless_type>,
Dimension,
...
f.template append_units_to <list<
        heterogeneous_system_dim < scaled_base_unit < Unit, UnitScale>, static_rational<1>>, dimensionless_type> >(str);
return(str);
}
...
/// INTERNAL ONLY
    template<class Dimension, class Units, class Scale, class Subformatter>
    inline std::string
    maybe_parenthesize(const unit <Dimension, heterogeneous_system<heterogeneous_system_impl < Units, Dimension, Scale>>
...
template<class Dimension, class Unit, class Scale, class Subformatter>
inline std::string
maybe_parenthesize(const unit <Dimension, heterogeneous_system<
        heterogeneous_system_impl < list < heterogeneous_system_dim < Unit, static_rational < 1>>, dimensionless_type
...
void do_print_prefixed_impl(std::basic_ostream<CharT, Traits> &os, const quantity <Unit, T> &q, F default_) {
    bool prefixed;
    if (detail::find_matching_scale<Prefixes>(q.value(), autoprefix_norm(q.value()),
...
void do_print_prefixed(std::basic_ostream<CharT, Traits> &os, const quantity <unit<Dimension, System>, T> &q) {
    detail::do_print_prefixed<Prefixes>(os,
                                        quantity<unit<Dimension, typename make_heterogeneous_system<Dimension, System>::type>, T>(
                                                q));
}
...
inline std::basic_ostream<Char, Traits> &operator<<(std::basic_ostream<Char, Traits> &os, const quantity <Unit, T> &q) {
    if (units::get_autoprefix(os) == autoprefix_none) {
        ...
        detail::maybe_print_prefixed<detail::engineering_prefixes>(os, q,

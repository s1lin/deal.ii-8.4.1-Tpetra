    ...
    namespace local_time {
        ...
        struct dst_not_valid : public std::logic_error {
            dst_not_valid(std::string const &msg = std::string()) :
                    std::logic_error(
                            std::string("is_dst flag does not match resulting dst for time label given: " + msg)) {}
        };
        ...
        public:
            ...
            enum DST_CALC_OPTIONS {
                EXCEPTION_ON_ERROR, NOT_DATE_TIME_ON_ERROR
            };
            ...
            // dummy value - time_ is set in constructor code
                ...
                if (result == ambiguous) {
                    if (calc_option == EXCEPTION_ON_ERROR) {
                        ...
                        this->time_ = posix_time::posix_time_system::get_time_rep(date_type(date_time::not_a_date_time),
                                                                                  time_duration_type(

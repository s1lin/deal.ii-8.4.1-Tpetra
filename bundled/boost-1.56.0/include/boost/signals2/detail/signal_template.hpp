...
result_type operator()(BOOST_SIGNALS2_SIGNATURE_FULL_ARGS(BOOST_SIGNALS2_NUM_ARGS)) {
    ...
    return detail::combiner_invoker<typename combiner_type::result_type>()
            (
                    ...
                    slot_call_iterator(local_state->connection_bodies().begin(), local_state->connection_bodies().end(),
                                       cache),
                    slot_call_iterator(local_state->connection_bodies().end(), local_state->connection_bodies().end(),
                                       cache)
            );
}
...
detail::BOOST_SIGNALS2_STD_FUNCTIONAL_BASE
    ...
    typedef typename impl_class::slot_call_iterator
            slot_call_iterator;
    typedef typename mpl::identity<BOOST_SIGNALS2_SIGNATURE_FUNCTION_TYPE(
            BOOST_SIGNALS2_NUM_ARGS)>::type signature_type;

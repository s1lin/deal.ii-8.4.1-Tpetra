...
//--------------Implementation of open, is_open and close---------------------//
            ...
            template<typename T, typename Tr, typename Alloc, typename Mode>
            inline bool indirect_streambuf<T, Tr, Alloc, Mode>::is_open() const { return (flags_ & f_open) != 0; }
            ...
            template<typename T, typename Tr, typename Alloc, typename Mode>
            bool indirect_streambuf<T, Tr, Alloc, Mode>::auto_close() const { return (flags_ & f_auto_close) != 0; }
...
//--------------Implementation virtual functions------------------------------//
            ...
            indirect_streambuf<T, Tr, Alloc, Mode>::seek_impl
                ...
                if (gptr() != 0 && way == BOOST_IOS::cur && which == BOOST_IOS::in &&
                    eback() - gptr() <= off && off <= egptr() - gptr()) {   // Small seek optimization
                    gbump(off);
            ...
            inline void indirect_streambuf<T, Tr, Alloc, Mode>::set_next
                    (streambuf_type *next) { next_ = next; }

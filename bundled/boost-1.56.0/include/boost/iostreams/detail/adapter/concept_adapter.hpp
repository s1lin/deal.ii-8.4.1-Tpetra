    ...
    namespace iostreams {
        namespace detail {
            ...
            public:
                ...
                explicit concept_adapter(const T &t) : t_(t) { BOOST_STATIC_ASSERT(!is_std_io<T>::value); }
                ...
                template<typename Device>
                void close(BOOST_IOS::openmode which, Device *dev) { any_impl::close(t_, dev, which); }
...
//------------------Specializations of device_wrapper_impl--------------------//
            ...
            struct device_wrapper_impl<any_tag> {
                ...
                template<typename Device, typename Dummy>
                static void close(Device &dev, Dummy *, BOOST_IOS::openmode which) { iostreams::close(dev, which); }

                template<typename Device, typename Dummy>
                static bool flush(Device &dev, Dummy *) { return iostreams::flush(dev); }
            };
            ...
            struct device_wrapper_impl<input> : device_wrapper_impl<any_tag> {
                ...
                read(Device &dev, Dummy *, typename char_type_of<Device>::type *s,
                     std::streamsize n) { return iostreams::read(dev, s, n); }
            ...
            struct device_wrapper_impl<output> {
                ...
                write(Device &dev, Dummy *, const typename char_type_of<Device>::type *s,
                      std::streamsize n) { return iostreams::write(dev, s, n); }
            };
...
//------------------Specializations of flt_wrapper_impl--------------------//
            ...
            struct flt_wrapper_impl<any_tag> {
                ...
                seek(Filter &f, Device *dev, stream_offset off,
                     ...
                     random_access, any_tag) { return f.seek(*dev, off, way); }

                template<typename Filter, typename Device>
                static std::streampos
                seek(Filter &f, Device *dev, stream_offset off,
                     BOOST_IOS::seekdir way, BOOST_IOS::openmode which,
                     random_access, two_sequence) { return f.seek(*dev, off, way, which); }
                ...
                template<typename Filter, typename Device>
                static bool flush(Filter &f, Device *dev) { return iostreams::flush(f, *dev); }
            };
            ...
            struct flt_wrapper_impl<input> {
                ...
                read(Filter &f, Source *src, typename char_type_of<Filter>::type *s,
                     std::streamsize n) { return iostreams::read(f, *src, s, n); }
            ...
            struct flt_wrapper_impl<output> {
                ...
                write(Filter &f, Sink *snk, const typename char_type_of<Filter>::type *s,
                      std::streamsize n) { return iostreams::write(f, *snk, s, n); }
            };

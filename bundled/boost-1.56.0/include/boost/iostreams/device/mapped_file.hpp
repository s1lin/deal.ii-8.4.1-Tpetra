...
class BOOST_IOSTREAMS_DECL mapped_file_sink
...
inline char *mapped_file::data() const { return (flags() != readonly) ? const_cast<char *>(delegate_.data()) : 0; }
...
mapped_file_sink::mapped_file_sink(
        ...
        boost::intmax_t offset, mapmode flags) { open(path, length, offset, flags); }

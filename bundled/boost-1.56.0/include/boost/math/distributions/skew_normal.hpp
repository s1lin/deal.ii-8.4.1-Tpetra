    ...
    namespace math {
        ...
        public:
            ...
            skew_normal_distribution(RealType l_location = 0, RealType l_scale = 1, RealType l_shape = 0)
                    : location_(l_location), scale_(l_scale),
                      shape_(l_shape) { // Default is a 'standard' normal distribution N01. (shape=0 results in the normal distribution with no skew)
        ...
        range(const skew_normal_distribution<RealType, Policy> & /*dist*/) { // Range of permissible values for random variable x.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const skew_normal_distribution<RealType, Policy> & /*dist*/) { // Range of supported values for random variable x.
        ...
        inline RealType mean(const skew_normal_distribution<RealType, Policy> &dist) {
            ...
            return dist.location() +
                   dist.scale() * dist.shape() / sqrt(pi<RealType>() + pi<RealType>() * dist.shape() * dist.shape()) *
                   root_two<RealType>();
        }
        ...
        inline RealType variance(const skew_normal_distribution<RealType, Policy> &dist) {
            ...
            const RealType delta2 = static_cast<RealType>(1) / (static_cast<RealType>(1) + static_cast<RealType>(1) /
                                                                                           (dist.shape() *
                                                                                            dist.shape()));
        ...
        namespace detail {
            ...
            */
            ...
            template<class RealType, class Policy>
            inline RealType mode_fallback(const skew_normal_distribution<RealType, Policy> &dist) { // mode.
            ...
            struct skew_normal_mode_functor {
                ...
                boost::math::tuple<RealType, RealType> operator()(RealType const &x) {
                    ...
                    RealType dx = static_cast<RealType>(2) * shape * x * normpdf_x * normpdf_ax *
                                  (static_cast<RealType>(1) + shape * shape) + pdf_x + x * fx;
        ...
        inline RealType quantile(const skew_normal_distribution<RealType, Policy> &dist, const RealType &p) {
            ...
            result = tools::newton_raphson_iterate(detail::skew_normal_quantile_functor<RealType, Policy>(dist, p),
                                                   result,
                                                   search_min, search_max, get_digits, m);

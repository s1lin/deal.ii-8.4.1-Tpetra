...
namespace boost {
    namespace math {
        namespace geometric_detail {
            ...
            template<class RealType, class Policy>
            inline bool
            check_dist_and_k(const char *function, const RealType &p, RealType k, RealType *result, const Policy &pol) {
            ...
            template<class RealType, class Policy>
            inline bool
            check_dist_and_prob(const char *function, RealType p, RealType prob, RealType *result, const Policy &pol) {
        ...
        public:
            ...
            geometric_distribution(RealType p) : m_p(p) { // Constructor stores success_fraction p.
                RealType result;
            ...
            RealType successes() const { // Total number of successes r = 1 (for compatibility with negative binomial?).
                return 1;
            }
        ...
        range(const geometric_distribution<RealType, Policy> & /* dist */) { // Range of permissible values for random variable k.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const geometric_distribution<RealType, Policy> & /* dist */) { // Range of supported values for random variable k.
        ...
        template<class RealType, class Policy>
        inline RealType
        mean(const geometric_distribution<RealType, Policy> &dist) { // Mean of geometric distribution = (1-p)/p.
        ...
        template<class RealType, class Policy>
        inline RealType
        mode(const geometric_distribution<RealType, Policy> &) { // Mode of geometric distribution = zero.
        ...
        inline RealType variance(
                const geometric_distribution<RealType, Policy> &dist) { // Variance of Binomial distribution = (1-p) / p^2.
        ...
        inline RealType skewness(
                const geometric_distribution<RealType, Policy> &dist) { // skewness of geometric distribution = 2-p / (sqrt(r(1-p))
            BOOST_MATH_STD_USING // ADL of std functions.
        ...
        template<class RealType, class Policy>
        inline RealType
        kurtosis(const geometric_distribution<RealType, Policy> &dist) { // kurtosis of geometric distribution
        ...
        inline RealType kurtosis_excess(
                const geometric_distribution<RealType, Policy> &dist) { // kurtosis excess of geometric distribution
        ...
        template<class RealType, class Policy>
        inline RealType pdf(const geometric_distribution<RealType, Policy> &dist,
        ...
        template<class RealType, class Policy>
        inline RealType cdf(const geometric_distribution<RealType, Policy> &dist,
                            const RealType &k) { // Cumulative Distribution Function of geometric.
        ...
        template<class RealType, class Policy>
        inline RealType
        cdf(const complemented2_type <geometric_distribution<RealType, Policy>, RealType> &c) { // Complemented Cumulative Distribution Function geometric.
        ...
        template<class RealType, class Policy>
        inline RealType quantile(const geometric_distribution<RealType, Policy> &dist,
                                 const RealType &x) { // Quantile, percentile/100 or Percent Point geometric function.
            ...
            // Special cases.
            if (x == 1) {  // Would need +infinity failures for total confidence.
            ...
            if (x == 0) { // No failures are expected if P = 0.
                return 0; // Total trials will be just dist.successes.
            }
            // if (P <= pow(dist.success_fraction(), 1))
            if (x <= success_fraction) { // p <= pdf(dist, 0) == cdf(dist, 0)
                return 0;
            }
            if (x == 1) {
                return 0;
        ...
        inline RealType quantile(
                const complemented2_type <geometric_distribution<RealType, Policy>, RealType> &c) {  // Quantile or Percent Point Binomial function.
            ...
            // Special cases:
            if (x == 1) {  // There may actually be no answer to this question,
                // since the probability of zero failures may be non-zero,
                return 0; // but zero is the best we can do:
            }
            if (-x <= boost::math::powm1(dist.success_fraction(), dist.successes(),
                                         Policy())) {  // q <= cdf(complement(dist, 0)) == pdf(dist, 0)
                return 0; //
            }
            if (x == 0) {  // Probability 1 - Q  == 1 so infinite failures to achieve certainty.

...
namespace boost {
    namespace math {
        namespace bernoulli_detail {
            ...
            template<class RealType, class Policy>
            inline bool
            check_dist(const char *, const RealType &, RealType *, const Policy & /* pol */, const mpl::false_ &) {
                return true;
            }
            ...
            check_dist(const char *function, const RealType &p, RealType *result, const Policy & /* pol */) {
                return check_dist(function, p, result, Policy(),
                                  typename policies::constructor_error_check<Policy>::type());
            }

            template<class RealType, class Policy>
            inline bool
            check_dist_and_k(const char *function, const RealType &p, RealType k, RealType *result, const Policy &pol) {
                if (check_dist(function, p, result, Policy(), typename policies::method_error_check<Policy>::type()) ==
                    false) {
                    return false;
                }
        ...
        public:
            ...
            bernoulli_distribution(RealType p = 0.5) : m_p(p) { // Default probability = half suits 'fair' coin tossing
        ...
        range(const bernoulli_distribution<RealType, Policy> & /* dist */) { // Range of permissible values for random variable k = {0, 1}.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const bernoulli_distribution<RealType, Policy> & /* dist */) { // Range of supported values for random variable k = {0, 1}.
        ...
        template<class RealType, class Policy>
        inline RealType
        mean(const bernoulli_distribution<RealType, Policy> &dist) { // Mean of bernoulli distribution = p (n = 1).
        ...
        template<class RealType, class Policy>
        inline RealType
        variance(const bernoulli_distribution<RealType, Policy> &dist) { // Variance of bernoulli distribution =p * q.
            return dist.success_fraction() * (1 - dist.success_fraction());
        } // variance

        template<class RealType, class Policy>
        RealType pdf(const bernoulli_distribution<RealType, Policy> &dist,
        ...
        template<class RealType, class Policy>
        inline RealType cdf(const bernoulli_distribution<RealType, Policy> &dist,
                            const RealType &k) { // Cumulative Distribution Function Bernoulli.
        ...
        template<class RealType, class Policy>
        inline RealType
        cdf(const complemented2_type <bernoulli_distribution<RealType, Policy>, RealType> &c) { // Complemented Cumulative Distribution Function bernoulli.
        ...
        template<class RealType, class Policy>
        inline RealType quantile(const bernoulli_distribution<RealType, Policy> &dist,
                                 const RealType &p) { // Quantile or Percent Point Bernoulli function.
            ...
            if (p <= (1 - dist.success_fraction())) { // p <= pdf(dist, 0) == cdf(dist, 0)
                return 0;
            } else {
                return 1;
            }
        ...
        inline RealType quantile(
                const complemented2_type <bernoulli_distribution<RealType, Policy>, RealType> &c) { // Quantile or Percent Point bernoulli function.
            ...
            if (q <= 1 - dist.success_fraction()) { // // q <= cdf(complement(dist, 0)) == pdf(dist, 0)
                return 1;
            } else {
                return 0;
            }

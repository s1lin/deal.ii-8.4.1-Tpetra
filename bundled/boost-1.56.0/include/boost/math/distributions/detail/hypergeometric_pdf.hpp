...
namespace boost {
    namespace math {
        namespace detail {
            ...
            T
            hypergeometric_pdf_lanczos_imp(T /*dummy*/, unsigned x, unsigned r, unsigned n, unsigned N, const Lanczos &,
                                           const Policy &) {
                ...
                BOOST_MATH_INSTRUMENT_VARIABLE(
                        bases[sorted_indexes[0]] * exp(static_cast<T>(base_e_factors[sorted_indexes[0]])));
                ...
                for (unsigned i = 1; (i < 9) && (exponents[sorted_indexes[i]] > 0); ++i) {
                    ...
                    if (exponents[sorted_indexes[i]] == 1)
                        ...
                        result *= sqrt(
                                bases[sorted_indexes[i]] * exp(static_cast<T>(base_e_factors[sorted_indexes[i]])));
                    else
                        result *= pow(bases[sorted_indexes[i]] * exp(static_cast<T>(base_e_factors[sorted_indexes[i]])),
            ...
            T hypergeometric_pdf_lanczos_imp(T /*dummy*/, unsigned x, unsigned r, unsigned n, unsigned N,
                                             const boost::math::lanczos::undefined_lanczos &, const Policy &pol) {
                BOOST_MATH_STD_USING
                return exp(
...
#pragma warning(pop)
            ...
            template<class T>
            T hypergeometric_pdf_prime_loop_imp(hypergeometric_pdf_prime_loop_data &data,
            ...
            hypergeometric_pdf(unsigned x, unsigned r, unsigned n, unsigned N, const Policy &) {
                ...
                if (N <= boost::math::max_factorial<value_type>::value) {
                    ...
                    //
                    result = detail::hypergeometric_pdf_lanczos_imp(value_type(), x, r, n, N, evaluation_type(),
                                                                    forwarding_policy());
                }
                ...
                return policies::checked_narrowing_cast<result_type, forwarding_policy>(result,

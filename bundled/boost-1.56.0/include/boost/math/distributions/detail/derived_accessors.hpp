...
namespace boost {
    namespace math {
        ...
        hazard(const Distribution &dist, const RealType &x) { // hazard function
        ...
        template<class Distribution, class RealType>
        inline typename Distribution::value_type
        chf(const Distribution &dist, const RealType &x) { // cumulative hazard function.
            // http://www.itl.nist.gov/div898/handbook/eda/section3/eda362.htm#HAZ
        ...
        inline typename Distribution::value_type coefficient_of_variation(const Distribution &dist) {
            ...
            if ((abs(m) < 1) && (d > abs(m) * tools::max_value<value_type>())) { // Checks too that m is not zero,
                return policies::raise_overflow_error<value_type>(
...
*/
        ...
        median(const Dist &d) { // median - default definition for those distributions for which a

    ...
    namespace math {
        ...
        namespace detail {
            ...
            T non_central_t_cdf(T v, T delta, T t, bool invert, const Policy &pol) {
                BOOST_MATH_STD_USING
                if ((boost::math::isinf) (v)) { // Infinite degrees of freedom, so use normal distribution located at delta.
            ...
            T non_central_t_quantile(const char *function, T v, T delta, T p, T q, const Policy &) {
                ...
                value_type guess = 0;
                if (((boost::math::isinf) (v)) || (v > 1 /
                                                       boost::math::tools::epsilon<T>())) { // Infinite or very large degrees of freedom, so use normal distribution located at delta.
                    normal_distribution <T, Policy> n(delta, 1);
                    if (p < q) {
                        return quantile(n, p);
                    } else {
                ...
                } else if (v > 3) { // Use normal distribution to calculate guess.
                    value_type mean = (v > 1 / policies::get_epsilon<T, Policy>()) ? delta : delta * sqrt(v / 2) *
                                                                                             tgamma_delta_ratio(
                                                                                                     (v - 1) * 0.5f,
                                                                                                     T(0.5f));
                    value_type var = (v > 1 / policies::get_epsilon<T, Policy>()) ? value_type(1) : (
            ...
            T non_central_t_pdf(T n, T delta, T t, const Policy &pol) {
                BOOST_MATH_STD_USING
                if ((boost::math::isinf) (n)) { // Infinite degrees of freedom, so use normal distribution located at delta.
        ...
        public:
            ...
            RealType degrees_of_freedom() const { // Private data getter function.
                return v;
            }

            RealType non_centrality() const { // Private data getter function.
                return ncp;
            }
        ...
        range(const non_central_t_distribution<RealType, Policy> & /* dist */) { // Range of permissible values for random variable k.
        ...
        template<class RealType, class Policy>
        inline const std::pair <RealType, RealType>
        support(const non_central_t_distribution<RealType, Policy> & /* dist */) { // Range of supported values for random variable k.
        ...
        template<class RealType, class Policy>
        inline RealType mode(const non_central_t_distribution<RealType, Policy> &dist) { // mode.
        ...
        inline RealType mean(const non_central_t_distribution<RealType, Policy> &dist) {
            ...
            if (v <= 1)
                return policies::raise_domain_error<RealType>(
                        function,
                        "The non-central t distribution has no defined mean for degrees of freedom <= 1: got v=%1%.", v,
                        Policy());
        ...
        template<class RealType, class Policy>
        inline RealType variance(const non_central_t_distribution<RealType, Policy> &dist) { // variance.
            ...
            if (v <= 2)
                return policies::raise_domain_error<RealType>(
                        function,
                        "The non-central t distribution has no defined variance for degrees of freedom <= 2: got v=%1%.",
                        v, Policy());
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
        ...
        template<class RealType, class Policy>
        inline RealType skewness(const non_central_t_distribution<RealType, Policy> &dist) { // skewness = sqrt(l).
            ...
            if (v <= 3)
                return policies::raise_domain_error<RealType>(
                        function,
                        "The non-central t distribution has no defined skewness for degrees of freedom <= 3: got v=%1%.",
                        v, Policy());;
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
        ...
        inline RealType kurtosis_excess(const non_central_t_distribution<RealType, Policy> &dist) {
            ...
            if (v <= 4)
                return policies::raise_domain_error<RealType>(
                        function,
                        "The non-central t distribution has no defined kurtosis for degrees of freedom <= 4: got v=%1%.",
                        v, Policy());;
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
                    detail::kurtosis_excess(static_cast<value_type>(v), static_cast<value_type>(l),
        ...
        template<class RealType, class Policy>
        inline RealType pdf(const non_central_t_distribution<RealType, Policy> &dist,
                            const RealType &t) { // Probability Density/Mass Function.
...
//   was const char* function = "boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
            ...
            if (l == 0) { // NO non-centrality, so use Student's t instead.
        ...
        template<class RealType, class Policy>
        RealType
        cdf(const complemented2_type <non_central_t_distribution<RealType, Policy>, RealType> &c) { // Complemented Cumulative Distribution Function
            // was       const char* function = "boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
            ...
            if (l == 0) { // zero non-centrality so use Student's t distribution.
        ...
        template<class RealType, class Policy>
        inline RealType quantile(const non_central_t_distribution<RealType, Policy> &dist,
                                 const RealType &p) { // Quantile (or Percent Point) function.
        ...
        inline RealType quantile(
                const complemented2_type <non_central_t_distribution<RealType, Policy>, RealType> &c) { // Quantile (or Percent Point) function.

    ...
    namespace math {
        namespace detail {
            ...
            inline bool check_triangular_lower(
                ...
                if ((boost::math::isfinite) (lower)) { // Any finite value is OK.
                    return true;
                } else { // Not finite: infinity or NaN.
            ...
            inline bool check_triangular_mode(
                    ...
                    RealType mode,
                    RealType *result, const Policy &pol) {
                if ((boost::math::isfinite) (mode)) { // any finite value is OK.
                    return true;
                } else { // Not finite: infinity or NaN.
                    *result = policies::raise_domain_error<RealType>(
                            function,
            ...
            inline bool check_triangular(
                ...
                if ((check_triangular_lower(function, lower, result, pol) == false)
                    ...
                    || (check_triangular_upper(function, upper, result, pol) == false)) { // Some parameter not finite.
                    return false;
        ...
        public:
            ...
            { // Evans says 'standard triangular' is lower 0, mode 1/2, upper 1,
                ...
                RealType result;
                detail::check_triangular("boost::math::triangular_distribution<%1%>::triangular_distribution", l_lower,
                                         l_mode, l_upper, &result, Policy());
            }
        ...
        range(const triangular_distribution<RealType, Policy> & /* dist */) { // Range of permissible values for random variable x.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const triangular_distribution<RealType, Policy> &dist) { // Range of supported values for random variable x.
        ...
        RealType pdf(const triangular_distribution<RealType, Policy> &dist, const RealType &x) {
            ...
            if (x == lower) { // (mode - lower) == 0 which would lead to divide by zero!
        ...
        inline RealType mode(const triangular_distribution<RealType, Policy> &dist) {
            ...
            RealType result = 0; // of checks.
            if (false ==
                detail::check_triangular_mode(function, mode, &result, Policy())) { // This should never happen!
                return result;
            }
            return mode;
        } // RealType mode
        ...
        inline RealType skewness(const triangular_distribution<RealType, Policy> &dist) {
            ...
            return root_two<RealType>() * (lower + upper - 2 * mode) * (2 * lower - upper - mode) *
                   (lower - 2 * upper + mode) /
                   (5 * pow((lower * lower + upper + upper + mode * mode - lower * upper - lower * mode - upper * mode),
                            RealType(3) / RealType(2)));
        } // RealType skewness(const triangular_distribution<RealType, Policy>& dist)

        template<class RealType, class Policy>
        inline RealType kurtosis(
                const triangular_distribution<RealType, Policy> &dist) { // These checks may be belt and braces as should have been checked on construction?

...
namespace boost {
    namespace math {
        ...
        typename Dist::value_type find_scale( // For example, normal mean.
            ...
            if (result <= 0) { // If policy isn't to throw, return the scale <= 0.
        ...
        inline typename Dist::value_type find_scale(
                complemented4_type < Real1, Real2, Real3, Policy >
        const& c) {
        ...
        return
        policies::raise_domain_error<typename Dist::value_type>(
                function,
        "Probability parameter was %1%, but must be >= 0 and <= 1!", q, c.param3);
    }
    ...
    policies::raise_domain_error<typename Dist::value_type>(
            function,
    "find_scale z parameter was %1%, but must be finite!", z, c.param3);
}
...
policies::raise_domain_error<typename Dist::value_type>(
        function,
"find_scale location parameter was %1%, but must be finite!", location, c.param3);
}
...
inline typename Dist::value_type find_scale(
        complemented3_type < Real1, Real2, Real3 >
const& c)
{
//cout << "cparam1 q " << c.param1 // q
//  << ", c.dist z " << c.dist // z
...
policies::raise_domain_error<typename Dist::value_type>(
        function,
"Probability parameter was %1%, but must be >= 0 and <= 1!", q,
...
policies::raise_domain_error<typename Dist::value_type>(
        function,
"find_scale z parameter was %1%, but must be finite!", z,
...
policies::raise_domain_error<typename Dist::value_type>(
        function,
"find_scale location parameter was %1%, but must be finite!", location,
...
); // This is only the default policy - also Want a version with Policy here.
}
return
result;

    ...
    namespace math {
        ...
        public:
            ...
            chi_squared_distribution(RealType i) : m_df(i) {
                RealType result;
                detail::check_df(
                        "boost::math::chi_squared_distribution<%1%>::chi_squared_distribution", m_df, &result,
                        Policy());
...
#pragma warning(disable:4127)
        ...
        range(const chi_squared_distribution<RealType, Policy> & /*dist*/) { // Range of permissible values for random variable x.
            if (std::numeric_limits<RealType>::has_infinity) {
                return std::pair<RealType, RealType>(static_cast<RealType>(0),
                                                     std::numeric_limits<RealType>::infinity()); // 0 to + infinity.
            } else {
...
#pragma warning(pop)
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const chi_squared_distribution<RealType, Policy> & /*dist*/) { // Range of supported values for random variable x.
        ...
        template<class RealType, class Policy>
        inline RealType
        mean(const chi_squared_distribution<RealType, Policy> &dist) { // Mean of Chi-Squared distribution = v.
            return dist.degrees_of_freedom();
        } // mean

        template<class RealType, class Policy>
        inline RealType
        variance(const chi_squared_distribution<RealType, Policy> &dist) { // Variance of Chi-Squared distribution = 2v.
...
//
        namespace detail {
            ...
            struct df_estimator {
                df_estimator(RealType a, RealType b, RealType variance, RealType delta)
                        : alpha(a), beta(b), ratio(delta / variance) { // Constructor
                }
        ...
        RealType chi_squared_distribution<RealType, Policy>::find_degrees_of_freedom(
            ...
            if (false ==
                ...
                && detail::check_probability(function, beta, &error_result,
                                             Policy())) { // Either probability is outside 0 to 1.
                return error_result;
            }

            if (hint <= 0) { // No hint given, so guess df = 1.
                hint = 1;
            }

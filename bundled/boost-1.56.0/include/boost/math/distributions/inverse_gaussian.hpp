    ...
    namespace math {
        ...
        public:
            ...
            inverse_gaussian_distribution(RealType l_mean = 1, RealType l_scale = 1)
                    : m_mean(l_mean), m_scale(l_scale) { // Default is a 1,1 inverse_gaussian distribution.
            ...
            RealType mean() const { // alias for location.
                return m_mean; // aka mu
            }

            // Synonyms, provided to allow generic use of find_location and find_scale.
            RealType location() const { // location, aka mu.
                return m_mean;
            }

            RealType scale() const { // scale, aka lambda.
                return m_scale;
            }
        ...
        range(const inverse_gaussian_distribution<RealType, Policy> & /*dist*/) { // Range of permissible values for random variable x, zero to max.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const inverse_gaussian_distribution<RealType, Policy> & /*dist*/) { // Range of supported values for random variable x, zero to max.
        ...
        template<class RealType, class Policy>
        inline RealType pdf(const inverse_gaussian_distribution<RealType, Policy> &dist,
                            const RealType &x) { // Probability Density Function
        ...
        template<class RealType, class Policy>
        inline RealType cdf(const inverse_gaussian_distribution<RealType, Policy> &dist,
                            const RealType &x) { // Cumulative Density Function.
            BOOST_MATH_STD_USING  // for ADL of std functions.
        ...
        struct inverse_gaussian_quantile_functor {
            ...
            inverse_gaussian_quantile_functor(const boost::math::inverse_gaussian_distribution<RealType, Policy> dist,
                                              RealType const &p)
        ...
        namespace detail {
            ...
            inline RealType guess_ig(RealType p, RealType mu = 1,
                                     RealType lambda = 1) { // guess at random variate value x for inverse gaussian quantile.
                ...
                if (phi > 2.) { // Big phi, so starting to look like normal Gaussian distribution.
                ...
                } else { // phi < 2 so much less symmetrical with long tail,
        ...
        inline RealType quantile(const inverse_gaussian_distribution<RealType, Policy> &dist, const RealType &p) {
            ...
            if (p == 1) { // overflow
                result = policies::raise_overflow_error<RealType>(function,
                                                                  "probability parameter is 1, but must be < 1!",
                                                                  Policy());
            ...
            using boost::math::tools::newton_raphson_iterate;
            result =
                    newton_raphson_iterate(inverse_gaussian_quantile_functor<RealType, Policy>(dist, p), guess, min,
                                           max, get_digits, m);
            return result;
        } // quantile
        ...
        quantile(const complemented2_type <inverse_gaussian_distribution<RealType, Policy>, RealType> &c) {
            ...
            using boost::math::tools::newton_raphson_iterate;
            result =
                    newton_raphson_iterate(inverse_gaussian_quantile_complement_functor<RealType, Policy>(c.dist, q),
                                           guess, min, max, get_digits, m);
            return result;
        } // quantile

        template<class RealType, class Policy>
        inline RealType mean(const inverse_gaussian_distribution<RealType, Policy> &dist) { // aka mu
            return dist.mean();
        }

        template<class RealType, class Policy>
        inline RealType scale(const inverse_gaussian_distribution<RealType, Policy> &dist) { // aka lambda
            return dist.scale();
        }

        template<class RealType, class Policy>
        inline RealType shape(const inverse_gaussian_distribution<RealType, Policy> &dist) { // aka phi
            return dist.shape();
        }

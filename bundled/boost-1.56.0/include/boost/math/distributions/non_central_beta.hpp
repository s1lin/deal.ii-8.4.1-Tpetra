...
namespace boost {
    namespace math {
        ...
        namespace detail {
            ...
            template<class RealType, class Policy>
            inline RealType
            non_central_beta_cdf(RealType x, RealType y, RealType a, RealType b, RealType l, bool invert,
                                 const Policy &) {
            ...
            std::pair <T, T>
            bracket_and_solve_root_01(F f, const T &guess, T factor, bool rising, Tol tol, boost::uintmax_t &max_iter,
                                      const Policy &pol) {
                BOOST_MATH_STD_USING
                ...
                if ((fa < 0) == (guess < 0 ? !rising : rising)) {
                    ...
                    while ((boost::math::sign) (fb) == (boost::math::sign) (fa)) {
                        if (count == 0) {
                            b = policies::raise_evaluation_error(function,
                                                                 "Unable to bracket root, last nearest value was %1%",
                                                                 b, pol);
                    ...
                    while ((boost::math::sign) (fb) == (boost::math::sign) (fa)) {
                        ...
                        if (count == 0) {
                            a = policies::raise_evaluation_error(function,
                                                                 "Unable to bracket root, last nearest value was %1%",
                                                                 a, pol);
                            return std::make_pair(a, b);
                        }
            ...
            template<class RealType, class Policy>
            RealType nc_beta_quantile(const non_central_beta_distribution<RealType, Policy> &dist, const RealType &p,
                                      bool comp) {
            ...
            RealType nc_beta_pdf(const non_central_beta_distribution<RealType, Policy> &dist, const RealType &x) {
                ...
                return policies::checked_narrowing_cast<RealType, forwarding_policy>(
                        non_central_beta_pdf(a, b, l, static_cast<value_type>(x),
                                             value_type(1 - static_cast<value_type>(x)), forwarding_policy()),
                        "function");
            }
            ...
            struct hypergeometric_2F2_sum {
                ...
                hypergeometric_2F2_sum(T a1_, T a2_, T b1_, T b2_, T z_) : a1(a1_), a2(a2_), b1(b1_), b2(b2_), z(z_),
                                                                           term(1), k(0) {}
...
#else
                value_type result = boost::math::tools::sum_series(s,
                                                                   boost::math::policies::get_epsilon<value_type, Policy>(),
                                                                   max_iter);
#endif
        ...
        public:
            ...
            RealType alpha() const { // Private data getter function.
                return a;
            }

            RealType beta() const { // Private data getter function.
                return b;
            }
        ...
        range(const non_central_beta_distribution<RealType, Policy> & /* dist */) { // Range of permissible values for random variable k.
        ...
        template<class RealType, class Policy>
        inline const std::pair <RealType, RealType>
        support(const non_central_beta_distribution<RealType, Policy> & /* dist */) { // Range of supported values for random variable k.
        ...
        template<class RealType, class Policy>
        inline RealType mode(const non_central_beta_distribution<RealType, Policy> &dist) { // mode.
        ...
        inline RealType mean(const non_central_beta_distribution<RealType, Policy> &dist) {
            ...
            RealType apb = a + b;
            return exp(-d / 2) * a *
                   detail::hypergeometric_2F2<RealType, Policy>(1 + a, apb, a, 1 + apb, d / 2, Policy()) / apb;
        } // mean
        ...
        inline RealType variance(const non_central_beta_distribution<RealType, Policy> &dist) {
            ...
            RealType d = dist.non_centrality();
            RealType apb = a + b;
            RealType result = detail::hypergeometric_2F2(RealType(1 + a), apb, a, RealType(1 + apb), RealType(d / 2),
                                                         Policy());
            result *= result * -exp(-d) * a * a / (apb * apb);
            result += exp(-d / 2) * a * (1 + a) *
                      detail::hypergeometric_2F2(RealType(2 + a), apb, a, RealType(2 + apb), RealType(d / 2),
                                                 Policy()) / (apb * (1 + apb));
            return result;
        }
        ...
        template<class RealType, class Policy>
        inline RealType
        skewness(const non_central_beta_distribution<RealType, Policy> & /*dist*/) { // skewness = sqrt(l).
        ...
        template<class RealType, class Policy>
        inline RealType pdf(const non_central_beta_distribution<RealType, Policy> &dist,
                            const RealType &x) { // Probability Density/Mass Function.
        ...
        template<class RealType, class Policy>
        RealType
        cdf(const complemented2_type <non_central_beta_distribution<RealType, Policy>, RealType> &c) { // Complemented Cumulative Distribution Function
            const char *function = "boost::math::non_central_beta_distribution<%1%>::cdf(%1%)";
        ...
        template<class RealType, class Policy>
        inline RealType quantile(const non_central_beta_distribution<RealType, Policy> &dist,
        ...
        inline RealType quantile(
                const complemented2_type <non_central_beta_distribution<RealType, Policy>, RealType> &c) { // Quantile (or Percent Point) function.

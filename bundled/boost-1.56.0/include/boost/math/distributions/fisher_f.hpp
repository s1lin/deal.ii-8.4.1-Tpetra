...
namespace boost {
    namespace math {
        ...
        range(const fisher_f_distribution<RealType, Policy> & /*dist*/) { // Range of permissible values for random variable x.
        ...
        template<class RealType, class Policy>
        inline const std::pair<RealType, RealType>
        support(const fisher_f_distribution<RealType, Policy> & /*dist*/) { // Range of supported values for random variable x.
        ...
        template<class RealType, class Policy>
        inline RealType mean(const fisher_f_distribution<RealType, Policy> &dist) { // Mean of F distribution = v.
            ...
            if (df2 <= 2) {
                return policies::raise_domain_error<RealType>(
                        function,
                        "Second degree of freedom was %1% but must be > 2 in order for the distribution to have a mean.",
                        df2, Policy());
            }
            return df2 / (df2 - 2);
        } // mean
        ...
        template<class RealType, class Policy>
        inline RealType variance(const fisher_f_distribution<RealType, Policy> &dist) { // Variance of F distribution.
            ...
            if (df2 <= 4) {
                return policies::raise_domain_error<RealType>(
                        function,
                        "Second degree of freedom was %1% but must be > 4 in order for the distribution to have a valid variance.",
                        df2, Policy());
            }
        ...
        inline RealType mode(const fisher_f_distribution<RealType, Policy> &dist) {
            ...
            if (df2 <= 2) {
                return policies::raise_domain_error<RealType>(
                        function,
                        "Second degree of freedom was %1% but must be > 2 in order for the distribution to have a mode.",
                        df2, Policy());
            }
...
// Now implemented via quantile(half) in derived accessors.
        ...
        inline RealType skewness(const fisher_f_distribution<RealType, Policy> &dist) {
            ...
            if (df2 <= 6) {
                return policies::raise_domain_error<RealType>(
                        function,
                        "Second degree of freedom was %1% but must be > 6 in order for the distribution to have a skewness.",
                        df2, Policy());
            }
        ...
        inline RealType kurtosis_excess(const fisher_f_distribution<RealType, Policy> &dist) {
            ...
            if (df2 <= 8) {
                return policies::raise_domain_error<RealType>(
                        function,
                        "Second degree of freedom was %1% but must be > 8 in order for the distribution to have a kutosis.",
                        df2, Policy());
            }
            RealType df2_2 = df2 * df2;
            RealType df1_2 = df1 * df1;
            RealType n = -16 + 20 * df2 - 8 * df2_2 + df2_2 * df2 + 44 * df1 - 32 * df2 * df1 + 5 * df2_2 * df1 -
                         22 * df1_2 + 5 * df2 * df1_2;
            n *= 12;

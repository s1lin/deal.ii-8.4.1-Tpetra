...
namespace boost {
    namespace math {
        ...
        namespace binomial_detail {
            ...
            template<class RealType, class Policy>
            inline bool
            check_dist_and_k(const char *function, const RealType &N, const RealType &p, RealType k, RealType *result,
                             const Policy &pol) {
            ...
            template<class RealType, class Policy>
            inline bool
            check_dist_and_prob(const char *function, const RealType &N, RealType p, RealType prob, RealType *result,
                                const Policy &pol) {
            ...
            template<class RealType, class Policy>
            RealType
            quantile_imp(const binomial_distribution<RealType, Policy> &dist, const RealType &p, const RealType &q,
                         bool comp) { // Quantile or Percent Point Binomial function.
                ...
                // Special cases:
                //
                if (p == 0) {  // There may actually be no answer to this question,
                    // since the probability of zero successes may be non-zero,
                    // but zero is the best we can do:
                    return 0;
                }
                if (p == 1) {  // Probability of n or fewer successes is always one,
                    // so n is the most sensible answer here:
                    return trials;
                }
                if (p <= pow(1 - success_fraction, trials)) { // p <= pdf(dist, 0) == cdf(dist, 0)
                    return 0; // So the only reasonable result is zero.
                } // And root finder would fail otherwise.
                if (success_fraction == 1) {  // our formulae break down in this case:
                    return p > 0.5f ? trials : 0;
                }

                // Solve for quantile numerically:
                //
                RealType guess = binomial_detail::inverse_binomial_cornish_fisher(trials, success_fraction, p, q,
                                                                                  Policy());
                RealType factor = 8;
                if (trials > 100)
        ...
        public:
            ...
            binomial_distribution(RealType n = 1, RealType p = 0.5) : m_n(n),
                                                                      m_p(p) { // Default n = 1 is the Bernoulli distribution
            ...
            RealType success_fraction() const { // Probability.
                return m_p;
            }

            RealType trials() const { // Total number of trials.
                return m_n;
            }
            ...
            static RealType find_lower_bound_on_p(
                ...
                if (false == binomial_detail::check_dist_and_k(
                    ...
                    binomial_detail::check_dist_and_prob(
                            function, trials, RealType(0), probability, &result, Policy())) { return result; }
                ...
                return (t == clopper_pearson_exact_interval) ? ibeta_inv(successes, trials - successes + 1, probability,
                                                                         static_cast<RealType *>(0), Policy())
                                                             : ibeta_inv(successes + 0.5f, trials - successes + 0.5f,
                                                                         probability, static_cast<RealType *>(0),
                                                                         Policy());
            }
            ...
            static RealType find_upper_bound_on_p(
                ...
                return (t == clopper_pearson_exact_interval) ? ibetac_inv(successes + 1, trials - successes,
                                                                          probability, static_cast<RealType *>(0),
                                                                          Policy())
                                                             : ibetac_inv(successes + 0.5f, trials - successes + 0.5f,
                                                                          probability, static_cast<RealType *>(0),
                                                                          Policy());
            }
                    ...
                    RealType alpha) // risk level
            {
                ...
                if (false == binomial_detail::check_dist_and_k(
                    ...
                    &&
                    binomial_detail::check_dist_and_prob(
                            function, k, p, alpha, &result, Policy())) { return result; }
        ...
        range(const binomial_distribution<RealType, Policy> &dist) { // Range of permissible values for random variable k.
        ...
        template<class RealType, class Policy>
        const std::pair<RealType, RealType>
        support(const binomial_distribution<RealType, Policy> &dist) { // Range of supported values for random variable k.
        ...
        template<class RealType, class Policy>
        inline RealType
        mean(const binomial_distribution<RealType, Policy> &dist) { // Mean of Binomial distribution = np.
            return dist.trials() * dist.success_fraction();
        } // mean

        template<class RealType, class Policy>
        inline RealType
        variance(const binomial_distribution<RealType, Policy> &dist) { // Variance of Binomial distribution = np(1-p).
            return dist.trials() * dist.success_fraction() * (1 - dist.success_fraction());
        } // variance

        template<class RealType, class Policy>
        RealType pdf(const binomial_distribution<RealType, Policy> &dist,
            ...
            if (dist.success_fraction() == 0) {  // probability of zero successes is 1:
                return static_cast<RealType>(k == 0 ? 1 : 0);
            }
            if (dist.success_fraction() == 1) {  // probability of n successes is 1:
            ...
            if (k == 0) { // binomial coeffic (n 0) = 1,
                // n ^ 0 = 1
                return pow(1 - dist.success_fraction(), n);
            }
            if (k == n) { // binomial coeffic (n n) = 1,
                // n ^ 0 = 1
        ...
        template<class RealType, class Policy>
        inline RealType cdf(const binomial_distribution<RealType, Policy> &dist,
                            const RealType &k) { // Cumulative Distribution Function Binomial.
            ...
            if (p == 1) { // This is correct but needs explanation:
                // when k = 1
        ...
        template<class RealType, class Policy>
        inline RealType
        cdf(const complemented2_type <binomial_distribution<RealType, Policy>, RealType> &c) { // Complemented Cumulative Distribution Function Binomial.
            // The random variate k is the number of successes in n trials.
            // k argument may be integral, signed, or unsigned, or floating point.
            // If necessary, it has already been promoted from an integral type.
            ...
            if (k == n) { // Probability of greater than n successes is necessarily zero:
                return 0;
            }
        ...
        template<class RealType, class Policy>
        inline RealType
        median(const binomial_distribution<RealType, Policy> &dist) { // Bounds for the median of the negative binomial distribution

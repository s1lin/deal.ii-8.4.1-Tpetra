    ...
    namespace math {
        ...
        namespace detail {
            ...
            T find_inverse_s(T p, T q) {
                ...
                static const double b[5] = {1, 6.61053765625462, 6.40691597760039, 1.27364489782223,
            ...
            T gamma_p_inv_imp(T a, T p, const Policy &pol) {
                ...
                if ((p < 0) || (p > 1))
                    return policies::raise_domain_error<T>(function,
                                                           "Probabilty must be in the range [0,1] in the incomplete gamma function inverse (got p=%1%).",
                                                           p, pol);
                if (p == 1)
                ...
                BOOST_MATH_INSTRUMENT_VARIABLE(guess);
                if (guess == lower)
                    guess = policies::raise_underflow_error<T>(function,
            ...
            T gamma_q_inv_imp(T a, T q, const Policy &pol) {
                ...
                if ((q < 0) || (q > 1))
                    return policies::raise_domain_error<T>(function,
                                                           "Probabilty must be in the range [0,1] in the incomplete gamma function inverse (got q=%1%).",
                                                           q, pol);
                if (q == 0)
                    return policies::raise_overflow_error<T>(function, 0, Policy());
                if (q == 1)

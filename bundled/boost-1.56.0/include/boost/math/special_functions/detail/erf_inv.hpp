...
//
                ...
                boost::math::tuple<T, T, T> operator()(const T &guess) {
                    ...
                    return boost::math::make_tuple(
                            ((sign > 0) ? static_cast<T>(boost::math::erf(guess, Policy()) - target) :
                             static_cast<T>(boost::math::erfc(guess, Policy())) - target), derivative, derivative2);
                }
                ...
                if (policies::digits<T, Policy>() > 64) {
                    ...
                    if (p <= 0.5) {
                        result = tools::halley_iterate(detail::erf_roots<typename remove_cv<T>::type, Policy>(p, 1),
                                                       guess, static_cast<T>(0), tools::max_value<T>(),
                                                       (policies::digits<T, Policy>() * 2) / 3, max_iter);
                    } else {
        ...
        typename tools::promote_args<T>::type erfc_inv(T z, const Policy &pol) {
            ...
            if ((z < 0) || (z > 2))
                return policies::raise_domain_error<result_type>(function,
                                                                 "Argument outside range [0,2] in inverse erfc function (got p=%1%).",
                                                                 z, pol);
            if (z == 0)
            ...
            mpl::or_ < mpl::less_equal < precision_type, mpl::int_ < 0 > >, mpl::greater < precision_type, mpl::int_ <
                                                                                                           64 > > >,
                    mpl::int_ < 0 >,
                    mpl::int_ < 64 >
                    > ::type
            ...
            return s * policies::checked_narrowing_cast<result_type, forwarding_policy>(
                    detail::erf_inv_imp(static_cast<eval_type>(p), static_cast<eval_type>(q), forwarding_policy(),
                                        static_cast<tag_type const *>(0)), function);
        }
        ...
        typename tools::promote_args<T>::type erf_inv(T z, const Policy &pol) {
            ...
            if ((z < -1) || (z > 1))
                return policies::raise_domain_error<result_type>(function,
                                                                 "Argument outside range [-1, 1] in inverse erf function (got p=%1%).",
                                                                 z, pol);
            if (z == 1)
                return policies::raise_overflow_error<result_type>(function, 0, pol);
            if (z == -1)

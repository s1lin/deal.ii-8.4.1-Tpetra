...
#endif
                policies::check_series_iterations<T>("boost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter,
                                                     pol);
            ...
            int bessel_ik(T v, T x, T *I, T *K, int kind, const Policy &pol) {
                ...
                if (x < 0) {
                    *I = *K = policies::raise_domain_error<T>(function,
                                                              "Got x = %1% but real argument x must be non-negative, complex number result not supported.",
                                                              x, pol);
                    return 1;
                }
                if (x == 0) {
                ...
                if (reflect) {
                    ...
                    if (fact == 0)
                        ...
                        *I = (org_kind & need_i) ? T(
                                sign(fact) * sign(scale) * policies::raise_overflow_error<T>(function, 0, pol)) : T(0);
                    else
                ...
                if (tools::max_value<T>() * scale < Kv)
                    *K = (org_kind & need_k) ? T(
                            sign(Kv) * sign(scale) * policies::raise_overflow_error<T>(function, 0, pol)) : T(0);
                else
                    *K = Kv / scale;

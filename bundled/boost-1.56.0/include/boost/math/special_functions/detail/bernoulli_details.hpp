...
//
            ...
            T t2n_asymptotic(int n) {
                ...
                if (tools::max_value<T>() / p2 < t2n)
                    return policies::raise_overflow_error<T>("boost::math::tangent_t2n<%1%>(std::size_t)", 0, T(n),
                                                             Policy());
                t2n *= p2;
                p2 -= 1;
                if (tools::max_value<T>() / p2 < t2n)
...
//
            ...
            inline std::size_t find_bernoulli_overflow_limit(const mpl::false_ &) {
                ...
                return static_cast<std::size_t>(boost::math::tools::toms748_solve(fun, sqrt(double(t)), double(t), tol,
                                                                                  max_iter).first) / 2;
            }
            ...
            std::size_t b2n_overflow_limit() {
                ...
                typedef mpl::bool_<
                        (bernoulli_imp_variant<T>::value >= 1) && (bernoulli_imp_variant<T>::value <= 3)> tag_type;
...
//
            ...
            inline typename enable_if_c<
                    std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::radix == 2), T>::type
            ...
            public:
                ...
                void tangent_numbers_series(const std::size_t m) {
                    ...
                    for (std::size_t i = old_size; i < m; i++) {
                        ...
                        b /= (power_two - 1);
                        bool overflow_check = (i >= min_overflow_index) && (tools::max_value<T>() /
                ...
                copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy &pol) {
                    ...
                    if (start + n > bn.capacity()) {
                        ...
                        for (; n; ++start, --n) {
                            *out = policies::raise_overflow_error<T>("boost::math::bernoulli_b2n<%1%>(std::size_t)", 0,
                                                                     T(start), pol);
                            ++out;
                        }
                        return out;
                    ...
                    for (std::size_t i = (std::max)(max_bernoulli_b2n<T>::value + 1, start); i < start + n; ++i) {
                        *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>(
                                "boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[i];
                        ++out;
                    }
                ...
                copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy &pol) {
                    ...
                    if (start + n > bn.capacity()) {
                        ...
                        if (start < b2n_overflow_limit<T, Policy>() + 2u) {
                            for (; n; ++start, --n) {
                                *out = t2n_asymptotic<T, Policy>(
                                        static_cast<typename container_type::size_type>(start));
                                ++out;
                            }
                        }
                        for (; n; ++start, --n) {
                        ...
                        if (i >= m_overflow_limit)
                            *out = policies::raise_overflow_error<T>("boost::math::bernoulli_b2n<%1%>(std::size_t)", 0,
                            ...
                            if (tools::max_value<T>() * tangent_scale_factor<T>() <
                                tn[static_cast<typename container_type::size_type>(i)])
                                *out = policies::raise_overflow_error<T>("boost::math::bernoulli_b2n<%1%>(std::size_t)",
                                                                         0, T(i), pol);
                            else
                                *out = tn[static_cast<typename container_type::size_type>(i)] /
                                       tangent_scale_factor<T>();
                        }
                        ++out;
                    }
#elif defined(BOOST_MATH_NO_ATOMIC_INT)

        ...
        namespace detail {
            ...
            int CF1_jy(T v, T x, T *fv, int *sign, const Policy &pol) {
                ...
                for (k = 1; k < policies::get_max_series_iterations<Policy>() * 100; k++) {
                    ...
                    f *= delta;
                    if (D < 0) { s = -s; }
                    if (abs(delta - 1) < tolerance) { break; }
                }
            ...
            int bessel_jy(T v, T x, T *J, T *Y, int kind, const Policy &pol) {
                ...
                if (v > static_cast<T>((std::numeric_limits<int>::max) ())) {
                    *J = *Y = policies::raise_evaluation_error<T>(function,
                                                                  "Order of Bessel function is too large to evaluate: got %1%",
                                                                  v, pol);
                    return 1;
                }
                n = iround(v, pol);
                ...
                if (x == 0) {
                    if (v == 0)
                        ...
                        *J = policies::raise_domain_error<T>(function,
                                                             "Value of Bessel J_v(x) is complex-infinity at %1%", x,
                    ...
                    if ((kind & need_y) == 0)
                        ...
                        *Y = policies::raise_domain_error<T>(function,
                                                             "Value of Bessel Y_v(x) is complex-infinity at %1%", x,
                                                             pol); // complex infinity
                    return 1;
                }
                ...
                if (((kind & need_y) == 0) && ((x < 1) || (v > x * x / 4) || (x < 5))) {
                    ...
                    T chi = boost::math::constants::root_two<T>() / (boost::math::constants::root_pi<T>() *
                ...
                if (reflect) {
                    if ((sp != 0) && (tools::max_value<T>() * fabs(Yv_scale) < fabs(sp * Yv)))
                        *J = org_kind & need_j ? T(-sign(sp) * sign(Yv) * sign(Yv_scale) *
                                                   policies::raise_overflow_error<T>(function, 0, pol)) : T(0);
                    else
                    ...
                    *J = Jv;
                    if (tools::max_value<T>() * fabs(Yv_scale) < fabs(Yv))
                        *Y = org_kind & need_y ? T(
                                sign(Yv) * sign(Yv_scale) * policies::raise_overflow_error<T>(function, 0, pol)) : T(0);
                    else
                        *Y = Yv / Yv_scale;
                }

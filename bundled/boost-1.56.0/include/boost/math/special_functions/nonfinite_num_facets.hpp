       ...
       */
        ...
        private:
            ...
            void put_impl(
                ...
                switch ((boost::math::fpclassify) (val)) {
                    ...
                    case FP_INFINITE:
                        ...
                        } else if ((boost::math::signbit) (val)) { // negative infinity.
                            put_num_and_fill(it, iosb, prefix_minus, body_inf, fill, val);
                        } else if (iosb.flags() & std::ios_base::showpos) { // Explicit "+inf" wanted.
                            put_num_and_fill(it, iosb, prefix_plus, body_inf, fill, val);
                        } else { // just "inf" wanted.
                    ...
                    case FP_NAN:
                        ...
                        } else if ((boost::math::signbit) (val)) { // negative so "-nan".
                            put_num_and_fill(it, iosb, prefix_minus, body_nan, fill, val);
                        } else if (iosb.flags() & std::ios_base::showpos) { // explicit "+nan" wanted.
                    ...
                    case FP_ZERO:
                        if ((flags_ & signed_zero) &&
                            ((boost::math::signbit) (val))) { // Flag set to distinguish between positive and negative zero.
                        ...
                        } else { // Output the platform default for positive and negative zero.
            ...
            void put_num_and_fill(
                ...
                if (body || prefix) { // adjust == std::ios_base::right, so leading fill needed.
                    if (adjust != std::ios_base::internal && adjust != std::ios_base::left)
                        put_fill(it, iosb, fill, width);
                }

                if (prefix) { // Adjust width for prefix.
                    while (*prefix)
                        *it = *(prefix++);
                ...
                if (body) { //
                    if (adjust == std::ios_base::internal) { // Put fill between sign and digits.
                        put_fill(it, iosb, fill, width);
                    }
            ...
            void put_fill(
                    OutputIterator &it, std::ios_base &iosb, CharType fill, int width) const { // Insert fill chars.
        ...
        private:
            ...
            void get_signed
                ...
                if (negative || c == '+') {
                    ...
                    if (c == '-' || c == '+') { // Without this check, "++5" etc would be accepted.
            ...
            void get_i
                ...
                ++it;
                if (!match_string(it, end, ct, "nity")) { // Expected "infinity"
                    state |= std::ios_base::failbit;
                    return;
                }
                ...
                switch (peek_char(it, end, ct)) {
                    ...
                    case '(':   // Optional payload field in (...) follows.
                    {
                        ...
                        while ((c = peek_char(it, end, ct))
                               && c != ')' && c != ' ' && c != '\n' && c != '\t')
                            ++it;
                        if (c != ')') { // Optional payload field terminator missing!
                            state |= std::ios_base::failbit;
                            return;
                        }
                        ++it;
                ...
                switch (peek_char(it, end, ct)) {
                    ...
                    case 's': // from SNAN - treated the same as QNAN.
                        if (std::numeric_limits<ValType>::has_quiet_NaN
                            && !(flags_ & trap_nan)) {
                            ++it;
                            if (match_string(it, end, ct, "nan")) { // "1.#QNAN", "1.#SNAN"

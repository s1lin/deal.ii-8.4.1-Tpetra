...
//
            ...
            T gamma_imp(T z, const Policy &pol, const Lanczos &l) {
                ...
                if (z <= 0) {
                    if (floor(z) == z)
                        return policies::raise_pole_error<T>(function,
                                                             "Evaluation of tgamma at a negative integer %1%.", z, pol);
                    if (z <= -20) {
                        result = gamma_imp(T(-z), pol, l) * sinpx(z);
                        BOOST_MATH_INSTRUMENT_VARIABLE(result);
                        if ((fabs(result) < 1) &&
                            (tools::max_value<T>() * fabs(result) < boost::math::constants::pi<T>()))
                            return -boost::math::sign(result) * policies::raise_overflow_error<T>(function,
                                                                                                  "Result of tgamma is too large to represent.",
                                                                                                  pol);
                        result = -boost::math::constants::pi<T>() / result;
                        if (result == 0)
                            return policies::raise_underflow_error<T>(function,
                                                                      "Result of tgamma is too small to represent.",
                                                                      pol);
                        if ((boost::math::fpclassify) (result) == (int) FP_SUBNORMAL)
                            return policies::raise_denorm_error<T>(function, "Result of tgamma is denormalized.",
                                                                   result, pol);
                        BOOST_MATH_INSTRUMENT_VARIABLE(result);
                        return result;
                    }
...
//
            ...
            T lgamma_imp(T z, const Policy &pol, const Lanczos &l, int *sign = 0) {
                ...
                if (z <= -tools::root_epsilon<T>()) {
                    ...
                    if (floor(z) == z)
                        return policies::raise_pole_error<T>(function,
                                                             "Evaluation of lgamma at a negative integer %1%.", z, pol);
...
//
            ...
            inline T lower_gamma_series(T a, T z, const Policy &pol, T init_value = 0) {
                ...
                policies::check_series_iterations<T>("boost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter,
                                                     pol);
                return result;
            }
            ...
            T gamma_imp(T z, const Policy &pol, const lanczos::undefined_lanczos &) {
                ...
                if (b_neg) {
                    ...
                    if (result_is_too_large_to_represent)
                        return policies::raise_overflow_error<T>(function,
                    ...
                    if (gamma_value == 0)
                        return policies::raise_underflow_error<T>(function,
                                                                  "Result of tgamma is too small to represent.", pol);
...
//
            ...
            T full_igamma_prefix(T a, T z, const Policy &pol) {
                ...
                if ((boost::math::fpclassify) (prefix) == (int) FP_INFINITE)
                    return policies::raise_overflow_error<T>("boost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)",
...
//
            template<class T, class Policy>
            inline T
            tgamma_small_upper_part(T a, T x, const Policy &pol, T *pgam = 0, bool invert = false, T *pderivative = 0) {
                ...
                result = -p * tools::sum_series(s, boost::math::policies::get_epsilon<T, Policy>(), max_iter,
                                                (init_value - result) / p);
                policies::check_series_iterations<T>("boost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter,
                                                     pol);
                if (invert)
                    result = -result;
                return result;
...
//
            ...
            T gamma_incomplete_imp(T a, T x, bool normalised, bool invert,
                ...
                switch (eval_method) {
                    ...
                    case 2: {
                        // Compute P:
                        result = normalised ? regularised_gamma_prefix(a, x, pol, lanczos_type()) : full_igamma_prefix(
                                a, x, pol);
                        if (p_derivative)
                            *p_derivative = result;
                        if (result != 0) {
            ...
            T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy &pol, const Lanczos &l) {
                ...
                if (z < tools::epsilon<T>()) {
                    ...
                    if (boost::math::max_factorial<T>::value<delta) {
                        T ratio = tgamma_delta_ratio_imp_lanczos(delta, T(boost::math::max_factorial<T>::value - delta),
                                                                 pol, l);
                        ratio *= z;
...
//
            ...
            T tgamma_ratio_imp(T x, T y, const Policy &pol) {
                ...
                if ((x <= 0) || (boost::math::isinf) (x))
                    return policies::raise_domain_error<T>("boost::math::tgamma_ratio<%1%>(%1%, %1%)",
            ...
            T gamma_p_derivative_imp(T a, T x, const Policy &pol) {
                ...
                //
                if (a <= 0)
                    return policies::raise_domain_error<T>("boost::math::gamma_p_derivative<%1%>(%1%, %1%)",
                                                           "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).",
                                                           a, pol);
                if (x < 0)
                    return policies::raise_domain_error<T>("boost::math::gamma_p_derivative<%1%>(%1%, %1%)",
...
// ratios of gamma functions:
        ...
        tgamma_delta_ratio(T1 z, T2 delta, const Policy & /* pol */) {
            ...
            return policies::checked_narrowing_cast<result_type, forwarding_policy>(
                    detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta),
                                                   forwarding_policy()),
        ...
        tgamma_ratio(T1 a, T2 b, const Policy &) {
            ...
            return policies::checked_narrowing_cast<result_type, forwarding_policy>(
                    detail::tgamma_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(b),
                                             forwarding_policy()), "boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
        }
        ...
        gamma_p_derivative(T1 a, T2 x, const Policy &) {
            ...
            return policies::checked_narrowing_cast<result_type, forwarding_policy>(
                    detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x),
                                                   forwarding_policy()),

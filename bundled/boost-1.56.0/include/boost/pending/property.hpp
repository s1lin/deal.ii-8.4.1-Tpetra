...
#undef BGL_DO_ONE_BUNDLE_TYPE
    ...
    struct lookup_one_property_internal<boost::property<Tag, T, Base>, Tag> {
        ...
        typedef T type;
        template<typename U>
        static typename enable_if<is_same < prop, U>, T&>

        ::type
        lookup(U &prop, const Tag &) { return prop.m_value; }

        template<typename U>
        static typename enable_if<is_same < prop, U>, const T&>
...
struct lookup_one_property<const T, Tag> {
    ...
    add_reference<const typename lookup_one_property_internal<T, Tag>::type> >

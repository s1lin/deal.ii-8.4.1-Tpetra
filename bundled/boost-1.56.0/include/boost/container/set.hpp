...
namespace boost {
    ...
    template<class InputIterator>
    set( ordered_unique_range_t, InputIterator
    first,
    InputIterator last
    ,
    const Compare &comp = Compare(),
...
this->priv_insert, const_iterator, const_iterator)
#endif
...
//!
//! <b>Complexity</b>: log(size())+count(k)
size_type count(const key_type &x) { return static_cast<size_type>(this->base_t::find(x) != this->base_t::end()); }
...
//!
//! <b>Complexity</b>: Logarithmic
std::pair<iterator, iterator> equal_range(const key_type &x) { return this->base_t::lower_bound_range(x); }
...
//! <b>Note</b>: Non-standard extension.
template<class InputIterator>
multiset( ordered_range_t, InputIterator
first,
InputIterator last
,
const Compare &comp = Compare()
...
x)
{
return static_cast<multiset&>(this->
...
template<class KeyType>
iterator priv_insert(BOOST_FWD_REF(KeyType) x) { return this->base_t::insert_equal(::boost::forward<KeyType>(x)); }

    ...
    namespace container {
        ...
        namespace container_detail {
            ...
            struct less_func<SizeType, adaptive_pool_flag::none> {
                static bool less(SizeType, SizeType, const void *, const void *) { return true; }
            };

            template<class SizeType>
            struct less_func<SizeType, adaptive_pool_flag::size_ordered> {
                static bool less(SizeType ls, SizeType rs, const void *, const void *) { return ls < rs; }
            };

            template<class SizeType>
            struct less_func<SizeType, adaptive_pool_flag::address_ordered> {
                static bool less(SizeType, SizeType, const void *la, const void *ra) { return &la < &ra; }
            };
            ...
            struct block_container_traits<VoidPointer, SizeType, false> {
                ...
                struct container {
                    typedef typename bi::make_list
                            <T, bi::base_hook <
                                hook_t>, bi::size_type <SizeType>, bi::constant_time_size<false> >::type type;
                };
            ...
            struct adaptive_pool_types {
                ...
                static const bool ordered =
                        (Flags & (adaptive_pool_flag::size_ordered | adaptive_pool_flag::address_ordered)) != 0;
                ...
                static const unsigned int order_flags =
                        Flags & (adaptive_pool_flag::size_ordered | adaptive_pool_flag::address_ordered);
                ...
                struct block_info_t
                    ...
                    friend bool operator==(const block_info_t &l, const block_info_t &r) { return &l == &r; }
                };
            ...
            inline size_type calculate_alignment
                    (size_type overhead_percent, size_type real_node_size, size_type hdr_size,
                     size_type hdr_offset_size, size_type payload_per_allocation) {
            ...
            inline void calculate_num_subblocks
                    (size_type alignment, size_type real_node_size, size_type elements_per_block,
                     size_type &num_subblocks, size_type &real_num_node, size_type overhead_percent, size_type hdr_size,
                     size_type hdr_offset_size, size_type payload_per_allocation) {
                const size_type hdr_subblock_elements =
                        (alignment - hdr_size - payload_per_allocation) / real_node_size;
                ...
                bool overhead_satisfied = false;
                while (!overhead_satisfied) {
                    const size_type total_data =
                            (elements_per_subblock * (possible_num_subblock - 1) + hdr_subblock_elements) *
                            real_node_size;
            ...
            public:
                ...
                private_adaptive_node_pool_impl
                        (segment_manager_base_type *segment_mngr_base, size_type node_size, size_type nodes_per_block,
                         size_type max_free_blocks, unsigned char overhead_percent
                        )
                        ...
                        , m_real_block_alignment
                                  (AlignOnly ?
                                   ...
                                   calculate_alignment((size_type) overhead_percent, m_real_node_size, HdrSize,
                                                       HdrOffsetSize, PayloadPerAllocation))
                        //This is the real number of nodes per block
                        , m_num_subblocks(0), m_real_num_node(
                                AlignOnly ? (m_real_block_alignment - PayloadPerAllocation - HdrSize) / m_real_node_size
                                          : 0)
                        //General purpose allocator
                        , mp_segment_mngr_base(segment_mngr_base), m_block_container(), m_totally_free_blocks(0) {
                    if (!AlignOnly) {
                        calculate_num_subblocks
                                (m_real_block_alignment, m_real_node_size, nodes_per_block, m_num_subblocks,
                                 m_real_num_node, (size_type) overhead_percent, HdrSize, HdrOffsetSize,
                                 PayloadPerAllocation);
                    }
                }

                //!Destructor. Deallocates all allocated blocks. Never throws
                ~private_adaptive_node_pool_impl() { this->priv_clear(); }

                size_type get_real_num_node() const { return m_real_num_node; }
                            ...
                            while (i != n){
                                ...
                                if (m_block_container.empty()) {
                                    ...
                                    BOOST_ASSERT(m_block_container.empty() ||
                                                 (++m_block_container.cbegin() == m_block_container.cend()));
                                ...
                                const size_type num_elems = (num_left < free_nodes_count_before) ? num_left
                                                                                                 : free_nodes_count_before;
                ...
                void deallocate_nodes(multiallocation_chain &nodes) {
                        ...
                        while (itf != ite) {
                            ...
                            if (&block_info != prev_block_info) {
                                if (prev_block_info) { //Make sure we skip the initial "dummy" cache
                                    ...
                                    prev_block_info->free_nodes.incorporate_after(prev_block_info->free_nodes.last(),
                                                                                  &*it, &*itbf, splice_node_count);
                    ...
                    if (prev_block_info) {
                        ...
                        nodes.clear();
                        prev_block_info->free_nodes.incorporate_after(prev_block_info->free_nodes.last(), &*itfirst,
                                                                      &*itlast, splice_node_count);
                        this->priv_reinsert_block(*prev_block_info, prev_block_was_empty);
                        this->priv_invariants();
                        this->priv_deallocate_free_blocks(m_max_free_blocks);
                    }
                ...
                void deallocate_free_blocks() { this->priv_deallocate_free_blocks(0); }
                ...
                void deallocate_free_chunks() { this->priv_deallocate_free_blocks(0); }

            private:

                void priv_deallocate_free_blocks(size_type max_free_blocks) {  //Trampoline function to ease inlining
                ...
                private:
                    ...
                    void do_destroy(typename block_container_t::pointer to_deallocate, AlignOnlyFalse) {
                        ...
                        hdr_offset_holder *hdr_off_holder =
                                mp_impl->priv_first_subblock_from_block(
                                        container_detail::to_raw_pointer(to_deallocate));
...
#undef BOOST_CONTAINER_ADAPTIVE_NODE_POOL_CHECK_INVARIANTS
                ...
                block_info_t *priv_block_from_node(void *node, AlignOnlyFalse) const {
                    hdr_offset_holder *hdr_off_holder =
                            reinterpret_cast<hdr_offset_holder *>((std::size_t) node &
                                                                  size_type(~(m_real_block_alignment - 1)));
                ...
                hdr_offset_holder *priv_first_subblock_from_block(block_info_t *block, AlignOnlyFalse) const {
                    ...
                    BOOST_ASSERT(hdr_off_holder->hdr_offset ==
                                 size_type(reinterpret_cast<char *>(block) - reinterpret_cast<char *>(hdr_off_holder)));
                    BOOST_ASSERT(0 == ((std::size_t) hdr_off_holder & (m_real_block_alignment - 1)));
                    BOOST_ASSERT(0 == (hdr_off_holder->hdr_offset & (m_real_block_alignment - 1)));
                ...
                void priv_dispatch_block_chain_or_free
                        (multiallocation_chain &chain, block_info_t &c_info, size_type num_node, char *mem_address,
                         size_type total_elements, bool insert_block_if_free) {
                    ...
                    mem_address = static_cast<char *>(container_detail::to_raw_pointer
                            (chain.incorporate_after(chain.last(), void_pointer(mem_address), m_real_node_size,
                                                     max_chain)));
                    ...
                    if (const size_type max_free = num_node - max_chain) {
                        ...
                        free_nodes.incorporate_after(free_nodes.last(), void_pointer(mem_address), m_real_node_size,
                                                     max_free);
                ...
                void priv_append_from_new_blocks(size_type min_elements, multiallocation_chain &chain, AlignOnlyTrue) {
                    ...
                    for (size_type i = 0; i != n; ++i) {
                        ...
                        if (i != (n - 1)) {
                            ...
                            this->priv_dispatch_block_chain_or_free(chain, c_info, m_real_num_node, mem_address,
                                                                    total_elements, true);
                        }
                    }
                }
                ...
                void priv_append_from_new_blocks(size_type min_elements, multiallocation_chain &chain, AlignOnlyFalse) {
                    ...
                    const size_type hdr_subblock_elements =
                            (m_real_block_alignment - HdrSize - PayloadPerAllocation) / m_real_node_size;
                    const size_type total_elements = chain.size() + min_elements;
                    ...
                    for (size_type i = 0; i != n; ++i) {
                        ...
                        if (i != (n - 1)) {
                            for (size_type subblock = 0, maxsubblock = m_num_subblocks - 1;
                                 subblock < maxsubblock; ++subblock, mem_address += m_real_block_alignment) {
                                //Initialize header offset mark
                                new(mem_address) hdr_offset_holder(size_type(hdr_addr - mem_address));
                                chain.incorporate_after
                                        (chain.last(), void_pointer(mem_address + HdrOffsetSize), m_real_node_size,
                                         elements_per_subblock);
                            }
                            chain.incorporate_after(chain.last(), void_pointer(hdr_addr + HdrSize), m_real_node_size,
                            ...
                            for (size_type subblock = 0, maxsubblock = m_num_subblocks - 1;
                                ...
                                this->priv_dispatch_block_chain_or_free
                                        (chain, c_info, elements_per_subblock, mem_address + HdrOffsetSize,

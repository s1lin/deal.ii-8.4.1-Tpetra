    ...
    namespace container {
        ...
        namespace container_detail {
            ...
            public:
                ...
                const Compare &get_comp() const { return *this; }

                Compare &get_comp() { return *this; }
            };
            ...
            private:
                ...
                public:
                    ...
                    Data (BOOST_RV_REF(Data)

                    d)
                    :

                    value_compare (boost::move(

                    static_cast<value_compare&>(d))),
                    ...
                    Data(BOOST_RV_REF(Data) d, const A &a)
                            : value_compare(boost::move(static_cast<value_compare &>(d))),
                              m_vect(boost::move(d.m_vect), a) {}
            ...
            public:
                ...
                flat_tree (BOOST_RV_REF(flat_tree)
                ...
                flat_tree(ordered_range_t, InputIterator first, InputIterator last, const Compare &comp = Compare(),
                          const allocator_type &a = allocator_type())
                        : m_data(comp, a) { this->m_data.m_vect.insert(this->m_data.m_vect.end(), first, last); }
            ...
            public:
                // accessors:
                Compare key_comp() const { return this->m_data.get_comp(); }

                value_compare value_comp() const { return this->m_data; }

                allocator_type get_allocator() const { return this->m_data.m_vect.get_allocator(); }

                const stored_allocator_type &
                get_stored_allocator() const { return this->m_data.m_vect.get_stored_allocator(); }

                stored_allocator_type &get_stored_allocator() { return this->m_data.m_vect.get_stored_allocator(); }

                iterator begin() { return this->m_data.m_vect.begin(); }

                const_iterator begin() const { return this->cbegin(); }

                const_iterator cbegin() const { return this->m_data.m_vect.begin(); }

                iterator end() { return this->m_data.m_vect.end(); }

                const_iterator end() const { return this->cend(); }

                const_iterator cend() const { return this->m_data.m_vect.end(); }
                ...
                bool empty() const { return this->m_data.m_vect.empty(); }

                size_type size() const { return this->m_data.m_vect.size(); }

                size_type max_size() const { return this->m_data.m_vect.max_size(); }

                void swap(flat_tree &other) { this->m_data.swap(other.m_data); }
            ...
            public:
                ...
                void insert_unique(ordered_unique_range_t, BidirIt first, BidirIt last
                    ...
                    while (len && !back_insert) {
                        ...
                        if (unique_burst) {
                            //Insert all in a single step in the precalculated positions
                            this->m_data.m_vect.insert_ordered_at(unique_burst, positions + unique_burst,
                                                                  skips + unique_burst, first);
                            //Next search position updated, iterator still valid because we've preserved the vector
                            pos += unique_burst;
                        }
                    }
...
#include BOOST_PP_LOCAL_ITERATE()
                ...
                iterator erase(const_iterator position) { return this->m_data.m_vect.erase(position); }
                ...
                void clear() { this->m_data.m_vect.clear(); }
                ...
                void shrink_to_fit() { this->m_data.m_vect.shrink_to_fit(); }
                ...
                equal_range(const key_type &k) const { return this->priv_equal_range(this->cbegin(), this->cend(), k); }
                ...
                size_type capacity() const { return this->m_data.m_vect.capacity(); }

                void reserve(size_type cnt) { this->m_data.m_vect.reserve(cnt); }
                ...
                friend bool operator!=(const flat_tree &x, const flat_tree &y) { return !(x == y); }

                friend bool operator>(const flat_tree &x, const flat_tree &y) { return y < x; }

                friend bool operator<=(const flat_tree &x, const flat_tree &y) { return !(y < x); }

                friend bool operator>=(const flat_tree &x, const flat_tree &y) { return !(x < y); }

                friend void swap(flat_tree &x, flat_tree &y) { x.swap(y); }
            ...
            private:
                ...
                bool priv_insert_unique_prepare
                    ...
                    if (pos == cend_it || val_cmp(val, *pos)) { //Check if val should go before end
                        ...
                        --prev;
                        if (val_cmp(*prev,
                                    val)) {   //If previous element was less, then it should go between prev and pos
                            return true;
                ...
                iterator priv_insert_commit
                    ...
                    return this->m_data.m_vect.insert
                            (commit_data.position, boost::forward<Convertible>(convertible));
                }

...
//!(memory usable for nodes / total memory allocated from the memory allocator)
        ...
        template<class T, std::size_t NodesPerBlock, std::size_t MaxFreeBlocks, std::size_t OverheadPercent, unsigned Version
        >
#endif
        class adaptive_pool {
        ...
        public:
            ...
            struct rebind {
                typedef adaptive_pool
                        <T2, NodesPerBlock, MaxFreeBlocks, OverheadPercent
#ifndef BOOST_CONTAINER_DOXYGEN_INVOKED
                        , Version
#endif
                > other;
        ...
        public:
            ...
            BOOST_CONTAINER_NOEXCEPT { return size_type(-1) / sizeof(T); }
            ...
            BOOST_CONTAINER_NOEXCEPT { return boost_cont_size(p); }
            ...
            void allocate_individual(std::size_t num_elements, multiallocation_chain &chain) {
                typedef container_detail::shared_adaptive_node_pool
                        <sizeof(T), NodesPerBlock, MaxFreeBlocks, OverheadPercent> shared_pool_t;
                typedef container_detail::singleton_default <shared_pool_t> singleton_t;
                singleton_t::instance().allocate_nodes(num_elements,
                                                       static_cast<typename shared_pool_t::multiallocation_chain &>(chain));
      ...
      chain.incorporate_after(chain.before_begin()
                ...
                if (!boost_cont_multialloc_nodes(n_elements, elem_size * sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS,
                                                 reinterpret_cast<boost_cont_memchain *>(&chain))) {
                    boost::container::throw_bad_alloc();
                }
            }
      ...
      chain.incorporate_after(chain.before_begin()
                             ...
                             ,(T*)BOOST_CONTAINER_MEMCHAIN_LASTMEM(&ch)
                             ,BOOST_CONTAINER_MEMCHAIN_SIZE(&ch) );*/
                if (!boost_cont_multialloc_arrays(n_elements, elem_sizes, sizeof(T), DL_MULTIALLOC_DEFAULT_CONTIGUOUS,
                                                  reinterpret_cast<boost_cont_memchain *>(&chain))) {
                    boost::container::throw_bad_alloc();
                }
            }
      ...
      boost_cont_multidealloc(&ch);*/
            ...
            friend void swap(adaptive_pool &, adaptive_pool &)

            BOOST_CONTAINER_NOEXCEPT {}

            //!An allocator always compares to true, as memory allocated with one
            //!instance can be deallocated by another instance
            friend bool operator==(const adaptive_pool &, const adaptive_pool &)

            BOOST_CONTAINER_NOEXCEPT { return true; }

            //!An allocator always compares to false, as memory allocated with one
            //!instance can be deallocated by another instance
            friend bool operator!=(const adaptive_pool &, const adaptive_pool &)

            BOOST_CONTAINER_NOEXCEPT { return false; }

        private:
            std::pair<pointer, bool> priv_allocation_command
                    (allocation_type command, std::size_t limit_size, std::size_t preferred_size,

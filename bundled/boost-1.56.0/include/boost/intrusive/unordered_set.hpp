...
//! inserting or erasing elements. Iterators are only invalidated when rehasing.
        ...
        public:
            ...
            //!   *this is disposed.
            explicit unordered_set_impl(const bucket_traits &b_traits, const hasher &hash_func = hasher(),
            ...
            unordered_set_impl(Iterator b, Iterator e, const bucket_traits &b_traits,
                               const hasher &hash_func = hasher(), const key_equal &equal_func = key_equal(),
                               const value_traits &v_traits = value_traits())
                    :  table_type(b_traits, hash_func, equal_func, v_traits) { table_type::insert_unique(b, e); }

            //! <b>Effects</b>: to-do
            //!
            unordered_set_impl (BOOST_RV_REF(unordered_set_impl)

            x)
            :

            table_type (::boost::move(

            static_cast<table_type&>(x)))
            {}

            //! <b>Effects</b>: to-do
            //!
            unordered_set_impl &operator=(BOOST_RV_REF(unordered_set_impl)x) {
                return static_cast<unordered_set_impl &>(table_type::operator=(
                        ::boost::move(static_cast<table_type &>(x))));
            }
            ...
            std::pair<iterator, bool> insert(reference value) { return table_type::insert_unique(value); }
            ...
            //!   No copy-constructors are called.
            template<class Iterator>
            void insert(Iterator b, Iterator e) { table_type::insert_unique(b, e); }
            ...
            std::pair<iterator, bool> insert_check
                    (const KeyType &key, KeyHasher hasher, KeyValueEqual key_value_equal,
...
//! same options (either explicitly or implicitly) are used.
        ...
        public:
            ...
            explicit unordered_set(const bucket_traits &b_traits, const hasher &hash_func = hasher(),
                                   const key_equal &equal_func = key_equal(),
                                   const value_traits &v_traits = value_traits())
                    : Base(b_traits, hash_func, equal_func, v_traits) {}

            template<class Iterator>
            unordered_set(Iterator b, Iterator e, const bucket_traits &b_traits, const hasher &hash_func = hasher(),
...
//! inserting or erasing elements. Iterators are only invalidated when rehasing.
        ...
        public:
            ...
            //! <b>Notes</b>: buckets array must be disposed only after
            //!   *this is disposed.
            explicit unordered_multiset_impl(const bucket_traits &b_traits, const hasher &hash_func = hasher(),
                                             const key_equal &equal_func = key_equal(),
                                             const value_traits &v_traits = value_traits())
                    : table_type(b_traits, hash_func, equal_func, v_traits) {}
            ...
            //!   *this is disposed.
            template<class Iterator>
            unordered_multiset_impl(Iterator b, Iterator e, const bucket_traits &b_traits,
                                    const hasher &hash_func = hasher(), const key_equal &equal_func = key_equal(),
                                    const value_traits &v_traits = value_traits())
                    :  table_type(b_traits, hash_func, equal_func, v_traits) { table_type::insert_equal(b, e); }

            //! <b>Effects</b>: to-do
            //!
            unordered_multiset_impl (BOOST_RV_REF(unordered_multiset_impl)
            ...
            unordered_multiset_impl &operator=(BOOST_RV_REF(unordered_multiset_impl)x) {
                return static_cast<unordered_multiset_impl &>(table_type::operator=(
                        ::boost::move(static_cast<table_type &>(x))));
            }
            ...
            //! <b>Note</b>: Does not affect the validity of iterators and references.
            //!   No copy-constructors are called.
            iterator insert(reference value) { return table_type::insert_equal(value); }
            ...
            //!   No copy-constructors are called.
            template<class Iterator>
            void insert(Iterator b, Iterator e) { table_type::insert_equal(b, e); }
...
//! same options (either explicitly or implicitly) are used.
        ...
        public:
            ...
            explicit unordered_multiset(const bucket_traits &b_traits, const hasher &hash_func = hasher(),
                                        const key_equal &equal_func = key_equal(),
                                        const value_traits &v_traits = value_traits())
                    : Base(b_traits, hash_func, equal_func, v_traits) {}

...
/// @cond
        ...
        struct unordered_node_traits
            ...
            static void set_next(const node_ptr &n, const node_ptr &next) { n->next_ = next; }

            static node_ptr get_prev_in_group(const const_node_ptr &n) { return n->prev_in_group_; }

            static void set_prev_in_group(const node_ptr &n, const node_ptr &prev) { n->prev_in_group_ = prev; }

            static std::size_t get_hash(const const_node_ptr &n) { return n->hash_; }

            static void set_hash(const node_ptr &n, std::size_t h) { n->hash_ = h; }
        };
        ...
        struct unordered_group_adapter {
            ...
            static node_ptr get_next(const const_node_ptr &n) { return NodeTraits::get_prev_in_group(n); }

            static void set_next(const node_ptr &n, const node_ptr &next) { NodeTraits::set_prev_in_group(n, next); }
        };
        ...
        struct get_uset_node_algo {
            typedef typename detail::if_c
                    <(StoreHash || OptimizeMultiKey), unordered_node_traits<VoidPointer, StoreHash, OptimizeMultiKey>,
                            slist_node_traits < VoidPointer>
            >::type node_traits_type;
            typedef typename detail::if_c
                    <OptimizeMultiKey, unordered_algorithms<node_traits_type>,
...
//! type when the same options (either explicitly or implicitly) are used.
        ...
        struct make_unordered_set_base_hook {
            ...
            typedef generic_hook
                    <get_uset_node_algo<typename packed_options::void_pointer, packed_options::store_hash, packed_options::optimize_multikey
                    >, typename packed_options::tag, packed_options::link_mode, HashBaseHookId

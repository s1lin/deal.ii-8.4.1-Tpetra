...
//! <tt>static balance positive();</tt>
        ...
        private:
                ...
                for (node_ptr root = NodeTraits::get_parent(header); x != root; root = NodeTraits::get_parent(header)) {
                    ...
                    if (x_parent_balance == NodeTraits::zero()) {
                        NodeTraits::set_balance(x_parent,
                                                x == x_parent_right ? NodeTraits::negative() : NodeTraits::positive());
            ...
            static void rebalance_after_insertion(const node_ptr &header, node_ptr x) {
                ...
                for (node_ptr root = NodeTraits::get_parent(header); x != root; root = NodeTraits::get_parent(header)) {
                    ...
                    if (x_parent_balance == NodeTraits::zero()) {
                        ...
                        NodeTraits::set_balance(x_parent,
                                                x_is_leftchild ? NodeTraits::negative() : NodeTraits::positive());
                        x = x_parent;
                    } else if (x_parent_balance == NodeTraits::positive()) {
            ...
            static node_ptr avl_rotate_left_right(const node_ptr a, const node_ptr a_oldleft,
                                                  const node_ptr &hdr) {  // [note: 'a_oldleft' is 'b']
                //             |                               |         //
                //             a(-2)                           c         //
                //            / \                             / \        //
                //           /   \        ==>                /   \       //
            ...
            static node_ptr avl_rotate_right_left(const node_ptr a, const node_ptr a_oldright,
                                                  const node_ptr &hdr) {  // [note: 'a_oldright' is 'b']
                //              |                               |           //
                //              a(pos)                          c           //
                //             / \                             / \          //
                //            /   \                           /   \         //

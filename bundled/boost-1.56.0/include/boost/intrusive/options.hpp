...
#undef BOOST_INTRUSIVE_DEFAULT_HOOK_MARKER_DEFINITION
            ...
            struct concrete_hook_base_value_traits {
                ...
                typedef bhtraits
                        <T, typename tags::node_traits, tags::link_mode, typename tags::tag, tags::type> type;
            };
            ...
            struct any_hook_base_value_traits {
                ...
                typedef bhtraits
                        <T, typename proto_value_traits::node_traits, proto_value_traits::hooktags::link_mode, typename proto_value_traits::hooktags::tag, proto_value_traits::hooktags::type
                        > type;
            };
            ...
            struct get_base_value_traits {
                typedef typename detail::eval_if_c
                        <internal_any_hook_bool_is_true<BaseHook>::value, any_hook_base_value_traits<T, BaseHook>, concrete_hook_base_value_traits<T, BaseHook>
                        >::type type;
            };
            ...
            struct get_base_node_traits {
                typedef typename detail::eval_if_c
                        <internal_any_hook_bool_is_true<BaseHook>::value, any_hook_base_node_traits<BaseHook>, concrete_hook_base_node_traits<BaseHook>
                        >::type type;
            };
            ...
            struct get_value_traits {
                typedef typename detail::eval_if_c
                        <detail::is_convertible<SupposedValueTraits *, detail::default_hook_tag *>::value, detail::apply<SupposedValueTraits, T>, detail::identity<SupposedValueTraits>
                ...
                typedef typename detail::eval_if_c
                        <internal_base_hook_bool_is_true<supposed_value_traits>::value
                                ...
                                , detail::eval_if_c
                                <internal_member_value_traits<supposed_value_traits>::value, get_member_value_traits<T, supposed_value_traits>, detail::identity<supposed_value_traits>
                                >
                        >::type type;
            };
            ...
            struct get_node_traits {
                ...
                typedef typename detail::eval_if_c
                        <internal_base_hook_bool_is_true<supposed_value_traits>::value
                                ...
                                //(member hooks and custom value traits are in this group)
                                , detail::eval_if_c
                                <internal_member_value_traits<supposed_value_traits>::value, get_member_node_traits<supposed_value_traits>, get_explicit_node_traits<supposed_value_traits>
                                >
                        >::type type;
            };
...
//!obtain constant-time size() member.
        BOOST_INTRUSIVE_OPTION_CONSTANT(constant_time_size,
        bool, Enabled, constant_time_size)
...
//!and division operations when rebalancing the tree.
        BOOST_INTRUSIVE_OPTION_CONSTANT(floating_point,
        bool, Enabled, floating_point)
...
//!should be optimized for size instead of for speed.
        BOOST_INTRUSIVE_OPTION_CONSTANT(optimize_size,
        bool, Enabled, optimize_size)

//!This option setter specifies if the slist container should
//!use a linear implementation instead of a circular one.
        BOOST_INTRUSIVE_OPTION_CONSTANT(linear,
        bool, Enabled, linear)
...
//!possible new functions like push_back(reference) and back().
        BOOST_INTRUSIVE_OPTION_CONSTANT(cache_last,
        bool, Enabled, cache_last)
...
//!rehashing might throw or the value is heavy to hash.
        BOOST_INTRUSIVE_OPTION_CONSTANT(store_hash,
        bool, Enabled, store_hash)
...
//!with the same key.
        ...
        bool, Enabled, optimize_multikey)
...
//!will be checked to through assertions to assure the bucket length is power of two.
        BOOST_INTRUSIVE_OPTION_CONSTANT(power_2_buckets,
        bool, Enabled, power_2_buckets)
...
//!stored hash values.
        BOOST_INTRUSIVE_OPTION_CONSTANT(compare_hash,
        bool, Enabled, compare_hash)
...
//!(rehashing the whole bucket array) is not admisible.
        BOOST_INTRUSIVE_OPTION_CONSTANT(incremental,
        bool, Enabled, incremental)

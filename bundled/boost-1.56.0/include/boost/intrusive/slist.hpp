...
//! an iterator that points one less the beginning of the list: ++before_begin() == begin()
        ...
        private:
            ...
            node_ptr get_end_node() { return node_ptr(linear ? node_ptr() : this->get_root_node()); }
            ...
            node_ptr get_root_node() { return data_.root_plus_size_.header_holder_.get_node(); }

            const_node_ptr get_root_node() const { return data_.root_plus_size_.header_holder_.get_node(); }

            node_ptr get_last_node() { return this->get_last_node(detail::bool_<cache_last>()); }

            const_node_ptr get_last_node() const { return this->get_last_node(detail::bool_<cache_last>()); }

            void set_last_node(const node_ptr &n) { return this->set_last_node(n, detail::bool_<cache_last>()); }
            ...
            node_ptr get_last_node(detail::bool_<true>) { return node_ptr(data_.root_plus_size_.last_); }

            const_node_ptr get_last_node(detail::bool_<true>) const {
                return const_node_ptr(data_.root_plus_size_.last_);
            }

            void set_last_node(const node_ptr &n, detail::bool_<true>) { data_.root_plus_size_.last_ = n; }
            ...
            struct root_plus_size
                    : public size_traits, public header_holder_plus_last_t {
            };
            ...
            size_traits &priv_size_traits() { return data_.root_plus_size_; }

            const size_traits &priv_size_traits() const { return data_.root_plus_size_; }

            const value_traits &priv_value_traits() const { return data_; }

            value_traits &priv_value_traits() { return data_; }
        ...
        public:
            ...
            slist_impl(const node_ptr &f, const node_ptr &before_l, size_type n,
            ...
            explicit slist_impl(const value_traits &v_traits = value_traits())
                    : data_(v_traits) { this->set_default_constructed_state(); }
            ...
            //! <b>Effects</b>: to-do
            //!
            slist_impl (BOOST_RV_REF(slist_impl)
            ...
            void pop_front() { return this->pop_front_and_dispose(detail::null_disposer()); }
            ...
            //!
            //! <b>Complexity</b>: Constant.
            iterator end() { return iterator(this->get_end_node(), this->priv_value_traits_ptr()); }
            ...
            //!
            //! <b>Complexity</b>: Constant.
            iterator before_begin() { return iterator(this->get_root_node(), this->priv_value_traits_ptr()); }
            ...
            //!
            //! <b>Complexity</b>: Constant.
            const_iterator cbefore_begin() const { return this->before_begin(); }
            ...
            //!
            //! <b>Complexity</b>: Constant.
            static const slist_impl &container_from_end_iterator(
                    const_iterator end_iterator) { return slist_impl::priv_container_from_end_iterator(end_iterator); }
            ...
            //!
            //! <b>Note</b>: Does not affect the validity of iterators and references.
            bool empty() const { return node_algorithms::unique(this->get_root_node()); }
            ...
            void shift_backwards(size_type n = 1) { this->priv_shift_backwards(n, detail::bool_<linear>()); }
            ...
            //!
            //! <b>Note</b>: Does not affect the validity of iterators and references.
            void shift_forward(size_type n = 1) { this->priv_shift_forward(n, detail::bool_<linear>()); }
            ...
            //!
            //! <b>Note</b>: Does not affect the validity of iterators and references.
            iterator insert(const_iterator p, reference value) { return this->insert_after(this->previous(p), value); }
            ...
            //! <b>Note</b>: Invalidates the iterators (but not the references) to the
            //!   erased element.
            iterator erase(const_iterator i) { return this->erase_after(this->previous(i)); }
            ...
            //! <b>Note</b>: Invalidates the iterators (but not the references) to the
            //!   erased elements.
            iterator erase(const_iterator f, const_iterator l) { return this->erase_after(this->previous(f), l); }
            ...
            //!   list. Iterators of this list and all the references are not invalidated.
            void
            splice_after(const_iterator prev_pos, slist_impl &x, const_iterator before_f, const_iterator before_l) {
                if (constant_time_size)
            ...
            void sort(Predicate p) {
                if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
                    ...
                    while (!this->empty()) {
                        ...
                        if (constant_time_size) {
                            counter[i].splice_after(counter[i].cbefore_begin(), carry, carry.cbefore_begin(),
                                                    last_element, carry.size());
                        } else {
                            counter[i].splice_after(counter[i].cbefore_begin(), carry, carry.cbefore_begin(),
                                                    last_element);
                        }
            ...
            //!
            //! <b>Note</b>: Iterators and references are not invalidated
            void merge(slist_impl &x) { this->merge(x, std::less<value_type>()); }
            ...
            void remove(const_reference value) { this->remove_if(detail::equal_to_value<const_reference>(value)); }
            ...
            //! <b>Note</b>: The relative order of elements that are not removed is unchanged,
            //!   and iterators to elements that are not removed remain valid.
            void unique() { this->unique_and_dispose(std::equal_to<value_type>(), detail::null_disposer()); }
            ...
            //!   and iterators to elements that are not removed remain valid.
            template<class Disposer>
            void unique_and_dispose(Disposer disposer) { this->unique(std::equal_to<value_type>(), disposer); }
            ...
            static const_iterator s_iterator_to(const_reference value) {
                BOOST_STATIC_ASSERT((!stateful_value_traits));
                reference r = *pointer_traits<pointer>::const_cast_from(
            ...
            iterator previous(iterator i) { return this->previous(this->cbefore_begin(), i); }
            ...
            //! <b>Complexity</b>: Linear to the number of elements before i.
            //!   Constant if cache_last<> is true and i == end().
            const_iterator previous(const_iterator i) const { return this->previous(this->cbefore_begin(), i); }
        ...
        private:
            void priv_splice_after(const node_ptr &prev_pos_n, slist_impl &x, const node_ptr &before_f_n,
            ...
            void
            priv_incorporate_after(const node_ptr &prev_pos_n, const node_ptr &first_n, const node_ptr &before_l_n) {
                if (cache_last) {
                    if (prev_pos_n == this->get_last_node()) {
                        this->set_last_node(before_l_n);
                    }
            ...
            void priv_reverse(detail::bool_<false>) { node_algorithms::reverse(this->get_root_node()); }
            ...
            //circular version
            static void priv_swap_lists(const node_ptr &this_node, const node_ptr &other_node,
                                        detail::bool_<false>) { node_algorithms::swap_nodes(this_node, other_node); }
...
//! same options (either explicitly or implicitly) are used.
        ...
        struct make_slist {
            ...
            typedef slist_impl
                    <value_traits, typename packed_options::size_type,
                            ...
                            | (std::size_t(packed_options::cache_last) *
                               slist_bool_flags::cache_last_pos), header_holder_type
        ...
        public:
            ...
            slist(const node_ptr &f, const node_ptr &before_l, size_type n,
                  const value_traits &v_traits = value_traits())

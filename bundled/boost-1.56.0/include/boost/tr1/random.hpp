...
namespace std {
    namespace tr1 {
        ...
        public:
            ...
            void seed(unsigned long x0 = 1) { m_gen.seed(x0); }
            ...
            BOOST_PREVENT_MACRO_SUBSTITUTION() const { return (m_gen.min)(); }

            result_type max

            BOOST_PREVENT_MACRO_SUBSTITUTION() const { return (m_gen.max)(); }

            result_type operator()() {
                return m_gen();
            }

            bool operator==(const linear_congruential &that) const { return m_gen == that.m_gen; }
        ...
        public:
            ...
            void seed() { m_gen.seed(); }

            void seed(unsigned long value) { m_gen.seed(value == 0 ? 5489UL : value); }

            template<class Gen>
            void seed(Gen &g) { init2(g, ::boost::is_fundamental<Gen>()); }
            ...
            bool operator==(const mersenne_twister &that) const { return m_gen == that.m_gen; }
        ...
        public:
            ...
            template<class Gen>
            subtract_with_carry(Gen &g) { init1(g, ::boost::is_same<Gen, subtract_with_carry<IntType, m, s, r> >()); }

            void seed(unsigned long value = 19780503ul) { m_gen.seed(value == 0 ? 19780503UL : value); }
            ...
            bool operator==(const subtract_with_carry &that) const { return m_gen == that.m_gen; }
        ...
        public:
            ...
            bool operator==(const subtract_with_carry_01 &that) const { return m_gen == that.m_gen; }
        ...
        public:
            ...
            xor_combine(Gen &g) {
                init_minmax();
                init1(g,
                      ::boost::is_same<Gen, xor_combine<UniformRandomNumberGenerator1, s1, UniformRandomNumberGenerator2, s2> >());
            }
            ...
            const base1_type &base1() const { return m_b1; }

            const base2_type &base2() const { return m_b2; }

            result_type min

            BOOST_PREVENT_MACRO_SUBSTITUTION() const { return m_min; }

            result_type max

            BOOST_PREVENT_MACRO_SUBSTITUTION() const { return m_max; }

            result_type operator()() { return (m_b1() << s1) ^ (m_b2() << s2); }

            bool operator==(const xor_combine &that) const { return (m_b1 == that.m_b1) && (m_b2 == that.m_b2); }

            bool operator!=(const xor_combine &that) const { return !(*this == that); }
        ...
        public:
            ...
            double p() const { return m_dist.p(); }

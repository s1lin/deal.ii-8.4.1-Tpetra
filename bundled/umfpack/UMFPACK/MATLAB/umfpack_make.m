...
%
See also
: umfpack, umfpack2, umfpack_details, umfpack_report, umfpack_demo,
%
and umfpack_simple
.
...
[M, kk] =
make (M,
'%s -DCONJUGATE_SOLVE -c %sumf_%s.c', 'umf_%s.%s', ...
...
rule, umfch {
k}, umfch {
k}, kind, obj, umfdir, kk, details);
end

[M, kk] =
make (M,
'%s -DWSOLVE -c %sumfpack_%s.c', 'umfpack_%s.%s', ...
...
'%s -output luflop luflopmex.c', mx), kk, details);
...
function [M, kk] =
make (M, s, src, dst, rule, file1, file2, kind, obj, ...
        srcdir, kk, details
)
% make:
execute a

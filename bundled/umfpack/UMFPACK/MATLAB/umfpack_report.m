...
%
See also
umfpack, umfpack2, umfpack_make, umfpack_details,
% umfpack_demo,
and umfpack_simple
.
...
'    Control (15): AMD dense row/column parameter: %g\n', Control (15));
...
'    Control (18): frontal matrix allocation ratio: %g\n', Control (18));
...
fprintf ('ERROR    n <= 0\n');
elseif (status
<= -7 & status >= -12 | status == -14)                %#

ok
fprintf(

'ERROR    matrix A is corrupted\n');
elseif (status
== -13)
...
'    (a -1 means the entry has not been computed):\n');
...
'    Info (32): %d, Q fixed during numeric factorization: ', Info (32));
...
'    mflops in numeric factorization phase: %.2f\n', 1e-6 * Info (43) / Info (66));

end
fprintf(

'    Info (67): %d, nnz (diag (U))\n', Info (67));
...
'    Info (70): %g, min. scale factor of rows of A\n', Info (70));

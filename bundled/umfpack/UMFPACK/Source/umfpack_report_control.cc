...
GLOBAL void UMFPACK_report_control
    ...
    PRINTF (("    "
                    ID
                    ": block size for dense matrix kernels: "
                    ID
                    "\n",
    ...
    PRINTF (("    "
                    ID
                    ": Q fixed during numerical factorization: %g ",
    ...
    PRINTF (("    "
                    ID
                    ": scaling: "
                    ID, (Int) INDEX(UMFPACK_SCALE), scale));
    ...
    if (front_alloc_init >= 0) {
        PRINTF (("    "
                        ID
                        ": frontal matrix allocation ratio: %g\n",
        ...
        PRINTF (("    "
                        ID
                        ": initial frontal matrix size (# of Entry's): "
                        ID
                        "\n",
    ...
    PRINTF (("    "
                    ID
                    ": AMD and COLAMD aggressive absorption: %g",
...
#else
    PRINTF (("Fortran BLAS.  size of BLAS integer: "
                    ID
                    "\n",
                            (Int)(sizeof(BLAS_INT))));
#endif

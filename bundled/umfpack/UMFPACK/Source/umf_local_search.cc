...
#define blas_ok FALSE
    ...
    DEBUG2 (("Max candidates %d, Work->ncand "
                    ID
                    " jmax "
                    ID
                    "\n",
...
#pragma ivdep
        ...
        if (pivrow[IN][OUT] != EMPTY) {
            ...
            if (do_extend) {
                ...
                do_update = fnpiv > 0;
                fnzeros = 0;
                DEBUG2 (("IN-OUT do_update forced true: "
                                ID
                                "\n", do_update));
    ...
    if (search_pivcol_out) {
        ...
        if (status == UMFPACK_WARNING_singular_matrix) {
            ...
            DEBUG1 (("Prune tuples of dead outcol: "
                            ID
                            "\n", pivcol[OUT]));
        ...
        if (pivrow[OUT][IN] != EMPTY) {
            ...
            if (do_extend) {
                ...
                do_update = fnpiv > 0;
                fnzeros = 0;
                DEBUG2 (("OUT-IN do_update forced true: "
                                ID
                                "\n", do_update));
        ...
        if (pivrow[OUT][OUT] != EMPTY) {
            ...
            if (do_extend) {
                ...
                do_update = fnpiv > 0;
                fnzeros = 0;
                DEBUG2 (("OUT-OUT do_update forced true: "
                                ID
                                "\n", do_update));
    ...
    DEBUG2 (("local search, best option "
                    ID
                    ", best cost "
                    ID
                    "\n",
    ...
    */
    ...
    DEBUG2 (("LOCAL SEARCH DONE: pivot column "
                    ID
                    " pivot row: "
                    ID
                    "\n",

...
PRIVATE Int packsp    /* returns new value of pnew */
    ...
    *p_len = len_new;
    Bi2 = (Int *) (Memory + pnew);
    pnew += UNITS (Int, len_new);
    Bx2 = (Entry *) (Memory + pnew);
    pnew += UNITS (Entry, len_new);
    DEBUGm4 (("  pnew "
                     ID
                     " len_new "
                     ID
...
GLOBAL Int UMF_kernel_init
    ...
    DEBUG1 (("Kernel init head usage, before allocs: "
                    ID
                    "\n", Numeric->ihead));
    ...
    for (k = n1; k < n_col - nempty_col; k++) {
        ...
        if (esize > 0) {
            ...
            E[e] = 0;
            empty_elements = TRUE;
            DEBUG0 (("column element e is empty "
                            ID
                            "\n", e));
        }
    }
    ...
    for (k = n1; k < n_col; k++) {
        ...
        for (pa = Ap[oldcol]; pa < p2; pa++) {
            ...
            if (newrow < n1 || rdeg > dense_row_threshold) {
                ...
                DEBUG1 (("Singleton/dense row of U: k "
                                ID
                                " newrow "
                                ID
                                "\n",

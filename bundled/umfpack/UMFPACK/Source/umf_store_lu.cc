...
GLOBAL Int UMF_store_lu
    ...
    for (kk = 0; kk < fnpiv; kk++) {
        ...
        DEBUG2 (("Removing pivrow from Lpattern, k = "
                        ID
                        "\n", k));
    ...
    for (kk = 0; kk < fnpiv; kk++) {
        ...
        if (pivcol_position != EMPTY) {
            unz2x--;
            DEBUG2 (("(exclude pivcol) unz2x is now "
                            ID
                            "\n", unz2x));
        }
        ...
        if (newUchain) {
            ...
            DEBUG2 (("Removing pivcol from Upattern, k = "
                            ID
                            "\n", k));

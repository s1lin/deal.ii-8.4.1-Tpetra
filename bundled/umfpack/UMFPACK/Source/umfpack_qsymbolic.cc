...
"**** Debugging enabled (UMFPACK will be exceedingly slow!) *****************\n"
    ...
    *SymbolicHandle = (void *) NULL;
    ...
    DEBUG0 (("Symbolic UMF_malloc_count - init_count = "
                    ID
                    "\n",
    ...
    DEBUG0 (("Strategy: ordering:   "
                    ID
                    "\n", Symbolic->ordering));
    DEBUG0 (("Strategy: fixQ:       "
                    ID
                    "\n", fixQ));
    DEBUG0 (("Strategy: prefer diag "
                    ID
                    "\n", prefer_diagonal));
    ...
    if (do_UMF_analyze) {
        ...
        DEBUG1 (("Front (EMPTY), singleton nrows "
                        ID
                        " ncols "
                        ID
                        "\n", k, k));
    ...
    for (i = 0; i < nfr; i++) {
        ...
        DEBUG2 (("Walk up front tree from "
                        ID
                        "\n", i));
        j = i;
        while (j != EMPTY && Front_leftmostdesc[j] == EMPTY) {
            DEBUG3 (("  Leftmost desc of "
                            ID
                            " is "
                            ID
                            "\n", j, i));
    ...
    DEBUGm1 (("dmaxfrsize %30.20g Int_MAX "
                     ID
                     "\n", dmaxfrsize, Int_MAX));
    ...
    DEBUG1 (("Symbolic usage after Rpi/Rpx allocation: head "
                    ID
                    " tail "
                    ID
                    "\n",
    ...
    DEBUG1 (("Symbolic init head usage: "
                    ID
                    " for LU singletons\n", head_usage));
    ...
    *SymbolicHandle = (void *) Symbolic;
    ...
    DEBUG0 (("(3)Symbolic UMF_malloc_count - init_count = "
                    ID
                    "\n",
                            UMF_malloc_count - init_count));
